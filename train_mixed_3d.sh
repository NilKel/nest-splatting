#!/bin/bash
# Script to train Nest-Splatting with --method mixed_3d
#   (textured = beta_scaled 2DGS surfels, untextured = EWA 3D ellipsoids
#    with --kernel2 gaussian by default — overridable via extra_args)
#
# Usage: ./train_mixed_3d.sh <dataset> <base_name> [scene_names] [iterations] [extra_args]
#
# Equivalent to one of:
#   python train.py -s <scene> -m <expname> --yaml <cfg> --eval --iterations N \
#       <-i imagesX> --method mixed_3d --kernel beta_scaled --kernel2 gaussian \
#       --texsplit 5000 --hybrid_levels 2 --disable_c2f false --aabb rect \
#       <extra_args>
#
# Examples:
#   ./train_mixed_3d.sh mip_360 GS2_SV_30thr_005w25gLP4lev_FRP5k10_C2F_Jac_5ksp all 35000 \
...
# --- Defaults for --method mixed_3d. Override any of these via extra_args. ---
METHOD_DEFAULTS="--method mixed_3d --kernel beta_scaled --kernel2 gaussian \
                 --texsplit 5000 --hybrid_levels 2 --disable_c2f true --aabb rect"
...
if [ $# -lt 2 ]; then
    ...
    echo ""
    echo "Output path: outputs/<dataset>/<scene>/mixed_3d/<base_name>"
    exit 1
fi
...
echo "════════════════════════════════════════════════════════════════════"
echo "  Nest-Splatting - mixed_3d Training"
echo "════════════════════════════════════════════════════════════════════"
...
LOG_DIR="logs"
mkdir -p $LOG_DIR
GLOBAL_LOG_FILE="${LOG_DIR}/train_mixed_3d_${BASE_NAME}_${TIMESTAMP}.log"
...
run_training() {
    ...
    OUTPUT_PATH="outputs/${DATASET_PATH}/${scene_name}/mixed_3d/${experiment_name}"
