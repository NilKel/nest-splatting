#!/usr/bin/env python3
"""
Compute LPIPS for all DTU experiments (baseline + cat) and update test_metrics.txt.
"""
...
def main():
    ...
    experiments = []
    scenes = sorted([d for d in os.listdir(BASE_DIR) if (BASE_DIR / d).is_dir() and d.startswith("scan")])
    for scene in scenes:
        for mode in MODES:
