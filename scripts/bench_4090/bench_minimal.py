...
def _lpips_factory(device):
    """Use lpipsPyTorch (the impl shipped alongside nest-splatting) so the
    4090 numbers match the 5090 reference exactly. Same call signature as
    benchmark_baked.py: lpips(x, y, net_type='vgg')."""
    import sys, os
    ...
    @torch.no_grad()
    def _go(a, b):
        return _lpips(a.unsqueeze(0).clamp(0, 1),
                      b.unsqueeze(0).clamp(0, 1),
                      net_type='vgg').item()
    return _go
...
def run_bench(bundle_dir, num_warmup=10, num_benchmark=200, beta=0.0,
              aabb_mode=5, sort_mode=0, no_atlas=False):
    ...
    if no_atlas:
        # SH/SV-only render: bind no atlas, pass atlas_rects=None to skip the
        # per-pixel tex2D lookup in renderBakedCUDA.
        from diff_surfel_bake_render import clear_atlas_bc7, clear_atlas_cache, \
            set_activation_bias, set_compact_mult, set_residual_mode
        clear_atlas_bc7(); clear_atlas_cache()
        set_activation_bias(float(bake_meta.get("sh_bias", 0.5)),
                            float(bake_meta.get("res_bias", 0.0)))
        set_compact_mult(float(bake_meta.get("compact_mult", 1.0)))
        set_residual_mode(int(bake_meta.get("residual_mode", 0)))
...
def main():
    ...
    ap.add_argument("--no_atlas", action="store_true",
                    help="Skip BC7 residual-atlas lookup; render with per-Gaussian SH/SV color only")
