// CUDA MLP test that compares against PyTorch reference
// Loads test data from mlp_test_data.npz and verifies gradients match
// Build: nvcc -o mlp_compare mlp_compare.cu -lcudart -arch=sm_86
// Run: ./mlp_compare
...
// Collaborative GEMM - EXACTLY as in mode_3d_direct_fused.cu
...
int main() {
    printf("=== CUDA MLP Comparison Test ===\n");
    printf("Testing with UNIT weights and input=1\n\n");

    // Allocate
    float *h_W1, *h_b1, *h_W2, *h_b2, *h_W3, *h_b3;
    float *h_dL_db1, *h_dL_db2, *h_dL_db3;
    float *h_inputs;

    h_W1 = (float*)calloc(32 * 40, sizeof(float));
    h_b1 = (float*)calloc(32, sizeof(float));
    h_W2 = (float*)calloc(32 * 32, sizeof(float));
    h_b2 = (float*)calloc(32, sizeof(float));
    h_W3 = (float*)calloc(3 * 32, sizeof(float));
    h_b3 = (float*)calloc(3, sizeof(float));

    h_dL_db1 = (float*)calloc(32, sizeof(float));
    h_dL_db2 = (float*)calloc(32, sizeof(float));
    h_dL_db3 = (float*)calloc(3, sizeof(float));

    int N = 256;
    h_inputs = (float*)malloc(N * 40 * sizeof(float));

    // Unit weights
    for (int i = 0; i < 32; i++) h_W1[i * 40 + i] = 1.0f;
    for (int i = 0; i < 32; i++) h_W2[i * 32 + i] = 1.0f;
    for (int i = 0; i < 3; i++) h_W3[i * 32 + i] = 1.0f;
    ...
    printf("dL_db3: ");
    int nz3 = 0;
    for (int i = 0; i < 3; i++) { if (fabsf(h_dL_db3[i]) > 1e-10) nz3++; printf("%.4f ", h_dL_db3[i]); }
    printf("(%d/3 nz)\n", nz3);
    ...
    printf("\nExpected: 3/3, 3/32, 3/32 all at [0,1,2]\n");
    bool pass = (nz3 == 3) && (nz2 == 3) && (nz1 == 3);
    printf("Result: %s\n", pass ? "PASS" : "FAIL");

    // Cleanup
    free(h_W1); free(h_b1); free(h_W2); free(h_b2); free(h_W3); free(h_b3);
    free(h_dL_db1); free(h_dL_db2); free(h_dL_db3); free(h_inputs);
