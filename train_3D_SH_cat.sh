#!/bin/bash
# Script to train Nest-Splatting with 3D_SH_cat mode
# Usage: ./train_3D_SH_cat.sh <dataset> <base_name> [scene_names] [iterations] [extra_args]
#
# Example:
#   ./train_3D_SH_cat.sh nerf_synthetic exp1              # Run all nerf_synthetic scenes
#   ./train_3D_SH_cat.sh DTU exp1 scan24,scan37           # Run specific DTU scenes
#   ./train_3D_SH_cat.sh mip_360 exp1 all 30000           # Run all mip_360 scenes with 30k iters
#   ./train_3D_SH_cat.sh nerf_synthetic exp1 all 30000 "--kernel betascaled"  # With extra args
...
if [ $# -lt 2 ]; then
    ...
    echo "  extra_args    Optional. Extra arguments to pass to train.py (e.g., '--kernel betascaled')"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 nerf_synthetic exp1 all 30000 \"--kernel betascaled\"  # With betascaled kernel"
    echo ""
    echo "Default args per scene: --method 3D_SH_cat --hybrid_levels 5 --disable_c2f --aabb rect --warmup gaussian --kernel gaussian"
    echo "                        --mcmc_fps --cap_max 1000000 --opacity_reg 0.01 --scale_reg 0.0 --noise_lr 1e3 --bce --bce_iter 10000"
    echo ""
    echo "Datasets:"
...
echo "════════════════════════════════════════════════════════════════════"
echo "  Nest-Splatting - 3D_SH_cat Training"
echo "════════════════════════════════════════════════════════════════════"
...
LOG_DIR="logs"
mkdir -p $LOG_DIR
GLOBAL_LOG_FILE="${LOG_DIR}/train_3D_SH_cat_${BASE_NAME}_${TIMESTAMP}.log"
...
run_training() {
    ...
    OUTPUT_PATH="outputs/${DATASET_PATH}/${scene_name}/3D_SH_cat/${experiment_name}"
    ...
    CMD="python train.py -s $scene_path -m $experiment_name --yaml $scene_yaml --eval --iterations $ITERATIONS $scene_resolution --method 3D_SH_cat --hybrid_levels 5 --disable_c2f --mcmc_fps --cap_max 1000000 --opacity_reg 0.01 --scale_reg 0.0 --noise_lr 1e3 --aabb rect --warmup gaussian --bce --bce_iter 10000 --kernel gaussian $extra_args $EXTRA_ARGS"
