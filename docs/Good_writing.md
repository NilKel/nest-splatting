...
DO NOT FORGET TO PUT dollar signs around math for latex. Render everything as plain text. You are always going to be generating latex. Follow these instructions to make it plain text:
The chat interface you are using automatically detects standard LaTeX delimiters (like $ and \begin{equation}) and converts them into rendered math visuals. I do not render the text myself; the browser UI applies that formatting to the text I send.
To completely disable your chat interface's math renderer, I have indented the entire output below by four spaces. This standard markdown rule forces the interface to display everything as raw, unformatted text so you can safely copy it straight to Overleaf.
...
•⁠  ⁠Do not include internal commentary, notes to yourself, or placeholder