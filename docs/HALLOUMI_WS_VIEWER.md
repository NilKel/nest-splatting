...
- **Column-sharded atlases must be normalised by SLICE width, not atlas width** (fixed 2026-08-25, was live and visibly broken): the NAT2 exporter shards any atlas wider than 8192 px into `n_cols × n_layers` slices of the array texture (`_find_column_cuts`, typeD/ASTC-codebook only — formats 7/8, header field 15 = `n_cols`), and `Nat2Parser` rewrites each rect's `u0` to be **column-local**. `gaussian-renderer.ts` was still writing `atlas_params.atlas_width = meta.width`, so `inv_w` in `preprocess_2dgs.wgsl` scaled every U by `slice_width / atlas_width`. Measured on garden (9280 px, `column_cuts [0, 2688, 9280]` → slice 6592): **17.62 dB vs 24.17 dB**, mean |Δ| 23.2/255 — smeared, detail-free textures. Only three shipped bundles were sharded (`garden_sh_res`, `garden_aftp`, `brain_2k_astc`), which is why it survived so long. Anything that consumes `atlas.meta.width` for a UV divisor wants `slice_width`.
