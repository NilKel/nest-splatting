...
- **Atlas dimension cap**: WebGPU on Adreno/Mali exposes `max_texture_dimension_2d = 8192`. The NAT2 packer slices the atlas into 8192-row stripes (`atlas_bc7_padded_h` may be larger; viewer reads `layer_cuts` to split). If you bake with `atlas_height > 8192 × max_layers`, the texture array creation will fail. Bake config typically uses 6 layers max → 49152 atlas rows.
