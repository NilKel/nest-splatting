...
__device__ void mlp_backward(
    ...
    for (int o = 0; o < OUT_DIM; o++) {
        ...
        for (int h = 0; h < HIDDEN_DIM; h++) {
            ...
            dL_dh2_post[h] += dz * __half2float(mlp.W3_rgb[o * HIDDEN_DIM + h]);
        }
    }
...
renderCUDAsurfelBackward(
	...
	// MLP gradient buffers for fused modes (render_mode=5, bias-free)
	float* __restrict__ dL_dmlp_W1 = nullptr,    // [32 * 32] = W1_SIZE
	float* __restrict__ dL_dmlp_W2 = nullptr,    // [32 * 32] = W2_SIZE
	float* __restrict__ dL_dmlp_W3 = nullptr,    // [48 * 32] = W3_SIZE
	// MLP weight pointers for fused modes (bias-free, FP16)
	const __half* __restrict__ mlp_W1_ptr = nullptr,
	const __half* __restrict__ mlp_W2_ptr = nullptr,
	const __half* __restrict__ mlp_W3_sh_ptr = nullptr,
	// Pre-encoded view directions (unused in SH mode, kept for API compatibility)
	const float* __restrict__ viewdirs_enc = nullptr)
{
	// Create MLP weights struct from parameters (bias-free)
	...
	// Per-tile MLP gradient accumulators for 3D_direct_fused mode (render_mode=5, bias-free)
	// Accumulate to shared memory first, then flush to global once per tile
	__shared__ float tile_dL_dW1[W1_SIZE];   // 32*32 = 4KB
	__shared__ float tile_dL_dW2[32 * 32];   // 4KB
	__shared__ float tile_dL_dW3[W3_SIZE];   // 48*32 = 6KB

	// Shared memory cache for MLP weights (FP16, ~7.1KB total)
	...
	__shared__ __half smem_mlp_W2[32 * 32];   // 2,048 bytes
	...
	// SH mode: view direction computed per-intersection from xyz and cam_pos
	// No pre-cached encoded view directions needed
	...
	if ((render_mode & 0xFF) == 5 && dL_dmlp_W1 != nullptr) {
		...
		for (int idx = tid; idx < 32 * 32; idx += BLOCK_SIZE)
		...
		for (int idx = tid; idx < W3_SIZE; idx += BLOCK_SIZE)
			smem_mlp_W3[idx] = mlp_weights.W3_rgb[idx];
		block.sync();
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if ((render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
			// DEBUG: Verify WMMA backward path is entered
			if (block.thread_rank() == 0 && block.group_index().x == 0 && block.group_index().y == 0) {
				printf("[BW_DEBUG] WMMA path entered: render_mode=%d, toDo=%d, inside=%d, done=%d, dL_dpixel=[%f,%f,%f]\n",
					render_mode, toDo, (int)inside, (int)done, dL_dpixel[0], dL_dpixel[1], dL_dpixel[2]);
			}
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				// Kernel-type variables (needed across alpha + geometry gradient phases)
				...
				// Beta kernel variables (kernel_type 1, 4)
				...
				if (participates) {
					...
					if (p.z != 0.0f) {
						...
						if (c_d >= near_n) {
							// Compute alpha using the same kernel type as forward
							bool alpha_ok = false;
							if (kernel_type == 3) {
								// General/beta kernel: G = exp(-0.5 * (rho^(beta/2)))
								...
								if (power <= 0.0f) {
									...
									alpha_ok = (alpha >= 1.0f / 255.0f);
								}
							} else if (kernel_type == 1 || kernel_type == 4) {
								// Beta kernel with max-pool handoff
								// kernel_type 1: k²=1, kernel_type 4: k²=9
								if (rho3d < k_sq + 1e-6f) {
								...
								// Flex beta kernel: G = (1+beta)*G_gauss / (1+beta*G_gauss)
								float power = -0.5f * rho;
								if (power <= 0.0f) {
									G_raw = expf(power);
								...
								// Default Gaussian kernel (type 0) with optional global beta
								float power = -0.5f * rho;
								if (power <= 0.0f) {
									G = expf(power);
									if (beta > 0.0f)
										G = (1.0f + beta) * G / (1.0f + beta * G);
									alpha = fminf(0.99f, opa * G);
									alpha_ok = (alpha >= 1.0f / 255.0f);
								}
							}
				...
				// Staged layer-by-layer backward interleaved with collaborative GEMM
				// Reduces peak register pressure from ~281 to ~139 floats
				...
				// --- Phase 1: Forward recomputation + SH backward → dL_dz3 ---
				// No h_pre needed: ReLU derivative = (h_post > 0)
				float my_input[TC_INPUT_DIM] = {0};
				float my_h1_post[32] = {0};
				float my_h2_post[32] = {0};
				float my_output[TC_OUTPUT_DIM] = {0};  // 48 SH coefficients
				float my_dL_dz3[TC_OUTPUT_DIM] = {0};
				float my_rgb[3] = {0};  // RGB from SH eval (for geometry gradients)
				...
				if (participates) {
					...
					// 3. Set implicit L1 bias (no view encoding in SH mode)
					my_input[24] = 1.0f;  // Implicit bias (positions 25-31 already zero)

					// 4. Recompute MLP forward → 48D SH coefficients (no activation)
					MlpWeights smem_mlp = {smem_mlp_W1, smem_mlp_W2, smem_mlp_W3};
					mlp_forward_inline(my_input, my_output, my_h1_post, my_h2_post, false, smem_mlp);

					// 5. Evaluate SH at view direction → RGB (for geometry gradients)
					glm::vec3 cp = *cam_pos;
					...
					// 6. SH backward → dL_dz3[48]
					// dL_dz3[ch*16+l] = dL_drgb[ch] * clamp_mask * sh_basis[l]
					// (no activation derivative — MLP output is identity for SH)
					for (int ch = 0; ch < 3; ch++) {
						float sh_val = eval_sh_channel(&my_output[ch * 16], sh_basis);
						my_rgb[ch] = fmaxf(0.0f, sh_val + 0.5f);
						float dL_drgb = dL_dpixel[ch] * w;
						float mask = (sh_val + 0.5f > 0.0f) ? 1.0f : 0.0f;
						for (int l = 0; l < 16; l++) {
							my_dL_dz3[ch * 16 + l] = dL_drgb * mask * sh_basis[l];
						}
					}
				// DEBUG: Print SH backward results for first participating thread
				if (participates && block.thread_rank() == 0 && block.group_index().x == 0 && block.group_index().y == 0 && j == 0) {
					printf("[BW_DEBUG] SH backward: w=%f, dL_dpixel=[%f,%f,%f], sh_basis[0]=%f\n",
						w, dL_dpixel[0], dL_dpixel[1], dL_dpixel[2], sh_basis[0]);
					printf("[BW_DEBUG] my_output[0..2]=[%f,%f,%f], my_rgb=[%f,%f,%f]\n",
						my_output[0], my_output[1], my_output[2], my_rgb[0], my_rgb[1], my_rgb[2]);
					float dz3_sum = 0;
					for (int i = 0; i < 48; i++) dz3_sum += my_dL_dz3[i] * my_dL_dz3[i];
					printf("[BW_DEBUG] dL_dz3 norm=%f, dL_dz3[0..2]=[%f,%f,%f]\n",
						sqrtf(dz3_sum), my_dL_dz3[0], my_dL_dz3[1], my_dL_dz3[2]);
				}
				}
				...
				if (participates) {
					...
					for (int h = 0; h < 32; h++) {
						...
						#pragma unroll
						for (int i = 0; i < 32; i++) {
							dL_dh1 += my_dL_dz2[i] * __half2float(smem_mlp_W2[i * 32 + h]);
						}
				...
				// ======== Phase 4: dL_dinput + feature & geometry gradients ========
				if (participates) {
					// dL_dinput = W1^T @ dL_dz1 (only first 24 elements: 20 gauss + 4 hash)
					float my_dL_dinput[24];
					#pragma unroll
					for (int i = 0; i < 24; i++) {
						float sum = 0;
						#pragma unroll
					...
					float dL_dalpha = 0.0f;
					float feat[C];
					for (int ch = 0; ch < C; ch++) feat[ch] = my_rgb[ch];
					...
					if (kernel_type == 1 || kernel_type == 4) {
						...
						// dL_dG stays as opa * dL_dalpha for position gradient propagation
					...
					if (rho3d <= rho2d) {
						...
						if (kernel_type == 1 || kernel_type == 4) {
							...
							if (beta_wins && base > 1e-7f) {
								...
								// Gaussian low-pass won, rho3d branch: gradient is 0
								// (alpha_lp depends on rho2d, not rho3d)
								dG_factor = 0.0f;
							}
						} else if (kernel_type == 3) {
							// General kernel: dG/drho * 2 = -0.5 * beta * G * pow_term / rho
							dG_factor = -0.5f * general_beta_val * G * general_pow_term_val / general_rho_safe_val;
						} else if (kernel_type == 2) {
							// Flex kernel: use G_raw (dL_dG already adjusted)
							dG_factor = -G_raw;
						} else {
							// Gaussian kernel (and global beta, where dL_dG not adjusted)
							dG_factor = -G;
						}
						float2 dL_ds = {
						...
						if (kernel_type == 1 || kernel_type == 4) {
							...
							if (!beta_wins) {
								...
								// Beta branch won, rho2d branch: gradient is 0
								// (alpha_beta depends on rho3d, not rho2d)
								dG_factor_2d = 0.0f;
							}
						} else if (kernel_type == 3) {
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 3:
				...
				break;
			case 5: {
				// 3D_SH_TC backward (legacy per-pixel path): MLP → SH → RGB
				if (pix_id == 0 && j == 0) {
					printf("[BW_DEBUG] Legacy case 5: dL_dmlp_W1=%p, dL_dpixel=[%f,%f,%f], w=%f\n",
						dL_dmlp_W1, dL_dpixel[0], dL_dpixel[1], dL_dpixel[2], w);
				}
				...
				// 1. Build MLP input: [gauss(20) | hash(4) | 1.0 bias | zeros(7)] = 32D
				float mlp_input[TC_INPUT_DIM] = {0};
				for (int i = 0; i < per_gaussian_dim && i < 20; i++) {
					mlp_input[i] = colors[global_id * per_gaussian_dim + i];
				}
				float hash_feat[4] = {0};
				if (active_hashgrid_levels > 0 && l_dim == 4) {
					query_feature<false, 4, 4>(hash_feat, xyz, voxel_min, voxel_max, collec_offsets,
				...
				mlp_input[24] = 1.0f;  // Implicit L1 bias

				// 2. MLP forward → 48D SH coefs (no activation)
				float h1_post[32], h2_post[32];
				float mlp_output[TC_OUTPUT_DIM];
				MlpWeights smem_mlp2 = {smem_mlp_W1, smem_mlp_W2, smem_mlp_W3};
				mlp_forward_inline(mlp_input, mlp_output, h1_post, h2_post, false, smem_mlp2);

				// 3. Evaluate SH at viewdir → RGB, store in feat for geometry grads
				glm::vec3 cp2 = *cam_pos;
				float3 view_dir = {xyz.x - cp2.x, xyz.y - cp2.y, xyz.z - cp2.z};
				float inv_len = rsqrtf(view_dir.x*view_dir.x + view_dir.y*view_dir.y + view_dir.z*view_dir.z + 1e-7f);
				view_dir.x *= inv_len; view_dir.y *= inv_len; view_dir.z *= inv_len;
				...
				for (int ch = 0; ch < 3; ch++) {
					float sh_val = eval_sh_channel(&mlp_output[ch * 16], sh_basis);
					feat[ch] = fmaxf(0.0f, sh_val + 0.5f);
				}

				// 4. SH backward → dL_dsh[48]
				float dL_dsh[TC_OUTPUT_DIM];
				for (int ch = 0; ch < 3; ch++) {
					float sh_val = eval_sh_channel(&mlp_output[ch * 16], sh_basis);
					float dL_drgb = dL_dchannels[ch] * w;
					float mask = (sh_val + 0.5f > 0.0f) ? 1.0f : 0.0f;
					for (int l = 0; l < 16; l++) {
						dL_dsh[ch * 16 + l] = dL_drgb * mask * sh_basis[l];
					}
				}

				// 5. MLP backward (legacy per-pixel atomic path)
				float dL_dinput[TC_INPUT_DIM];
				mlp_backward<TC_INPUT_DIM, TC_HIDDEN_DIM, TC_OUTPUT_DIM>(
					mlp_input, mlp_output, dL_dsh,
					h1_post, h2_post,
					dL_dinput,
					tile_dL_dW1,
					tile_dL_dW2,
					tile_dL_dW3,
					smem_mlp2,
					false  // No sigmoid — identity activation for SH
				);

				// 6. Backprop to per-Gaussian features (first 20D)
				for (int i = 0; i < per_gaussian_dim && i < 20; i++) {
					atomicAdd(&(dL_dcolors[global_id * per_gaussian_dim + i]), dL_dinput[i]);
				}

				// 7. Backprop to hash features (next 4D)
				if (active_hashgrid_levels > 0 && l_dim == 4) {
					float dL_dhash[4];
					for (int i = 0; i < 4; i++) dL_dhash[i] = dL_dinput[20 + i];
	...
	const float* viewdirs_enc)
{
	...
	__half *mlp_W3_sh_ptr = nullptr;
	__half *mlp_W3_rgb_ptr = nullptr;
	if (render_mode == 5) {
	...
	// Dynamic shared memory for mode 5 (3D_SH_TC) collaborative GEMM (FP16 buffers)
	// Layer 3 is largest: 256*(48+32)*2 = 40,960 bytes (FP16, single batch)
	size_t smem_size = 0;
	...
	if (render_mode == 5 && dL_dmlp_W1 != nullptr) {
		...
		if (!force_disable) {
			// Try to enable collaborative GEMM with ~40KB FP16 shared memory (single batch of 256)
			// Layer 3 (largest) needs 256*(48+32)*2 = 40960 bytes
	...
	renderCUDAsurfelBackward<3, 0> <<<grid, block, smem_size>>>(
			...
			dL_dmlp_W1, dL_dmlp_W2, dL_dmlp_W3,
			mlp_W1_ptr, mlp_W2_ptr, mlp_W3_sh_ptr, viewdirs_enc);
