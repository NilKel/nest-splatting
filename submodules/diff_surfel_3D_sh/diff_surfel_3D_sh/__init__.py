...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    @staticmethod
    def get_mlp_grads():
        """Get MLP gradients from last backward pass (for 3D_direct_fused mode, bias-free).
    ...
    def forward(
        ...
        ctx.viewdirs_enc = viewdirs_enc  # Save separately (no gradient needed)
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_intersection_buffer, grad_intersection_count, grad_geomBuffer):
        ...
        viewdirs_enc = ctx.viewdirs_enc  # Pre-encoded view directions (no gradient)
...
def set_mlp_weights(W1, W2, W3, is_sh_mode=False):
    """
    Copy MLP weights to CUDA global memory for fused in-kernel MLP evaluation (bias-free).
    Call this before each render call if weights have changed.

    Layer 1 uses input padding (input[24]=1.0) for implicit bias via W1[:, 24].
    Layers 2 and 3 have no bias.

    Args:
        W1: [32, 32] Layer 1 weights (WMMA-padded from 32×25, col 24 = implicit bias)
        W2: [32, 32] Layer 2 weights (hidden1 → hidden2)
        W3: [48, 32] Layer 3 weights (48 SH coefficients)
        is_sh_mode: True (always True for this SH submodule)
    """
...
def get_mlp_grads():
    ...
    Returns:
        ...
        - grad_W1: [32, 32] Layer 1 weight gradients (col 24 = implicit bias grad)
        - grad_W2: [32, 32] Layer 2 weight gradients
        - grad_W3: [48, 32] Layer 3 weight gradients (48 SH coefficients)
    """
