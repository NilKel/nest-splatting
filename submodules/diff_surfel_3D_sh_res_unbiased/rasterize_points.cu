	...
	const torch::Tensor& metric_map)
{
  ...
  // Channel layout (19 total): DEPTH(0), ALPHA(1), NORMAL(2..4), MIDDEPTH(5),
  // DISTORTION(6), NUM(7), POS(8..10), VIS(11..13), OVERDRAW(14), MAXDEPTH(15),
  // WSQUARE(16), DIFFUSE_RGB(17), CONVERGE(18 — Unbiased Depth, Peng et al.)
  int out_dim = 3+3+1+1 + 3 + 3 + 1 + 1 + 1 + 1 + 1; // base 17 + w_square_sum (+1) + converge (+1) = 19
...
// Unbiased Depth: set per-pair depth-difference cutoff for the convergence loss.
// Patches BOTH forward and backward TU copies in lockstep.
void SetConvergeThresholdCUDA(float val) {
    FORWARD::setConvergeThreshold(val);
    BACKWARD::setConvergeThreshold(val);
}
