...
Usage:
    from diff_surfel_bake_render_lean_occ import GaussianRasterizationSettings, GaussianRasterizer
...
def set_occluder_depth(depth):
    """Per-Gauss proxy-mesh Z-cull. Pass a CUDA fp32 [H, W] cam-Z depth map
    (non-hits = +inf). Preprocess drops any surfel whose centre projects into
    a pixel where centre_depth > occluder_depth. Caller keeps the tensor
    alive across the render call."""
    _C.set_occluder_depth(depth)


def clear_occluder_depth():
    """Disable the proxy-mesh Z-cull (revert to byte-identical no-op)."""
    _C.clear_occluder_depth()


def set_backface_cull(cos_thr, centroid):
    """Per-Gauss backface cull in preprocess (train.py --backface_cull
    semantics): disc normal = quat 3rd axis, sign-oriented outward via
    `centroid` (len-3 sequence / tensor, world space); a surfel is dropped
    when dot(view_dir_to_surfel, outward_normal) > cos_thr. View-dependent
    but state is view-independent — set once, valid for every camera."""
    cx, cy, cz = (float(centroid[0]), float(centroid[1]), float(centroid[2]))
    _C.set_backface_cull(float(cos_thr), cx, cy, cz)


def clear_backface_cull():
    """Disable the backface cull (byte-identical no-op)."""
    _C.clear_backface_cull()
