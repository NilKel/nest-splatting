...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_occluder_depth", &SetOccluderDepthBakeCUDA,
        ...
        "(non-hits = +inf). In preprocess, surfels whose centres are behind "
        "the mesh at their projected pixel are dropped from all downstream "
        "stages (sort/tile-assign/render). Caller must keep the tensor alive.");
  m.def("clear_occluder_depth", &ClearOccluderDepthBakeCUDA,
        "Disable the proxy-mesh Z-cull (revert to byte-identical no-op).");
  m.def("set_backface_cull", &SetBackfaceCullBakeCUDA,
        "Per-Gauss backface cull in preprocess: disc normal (quat 3rd axis) "
        "oriented outward via the given cloud centroid; cull when "
        "dot(view_dir, outward_normal) > cos_thr. Matches train.py "
        "--backface_cull semantics. Args: (cos_thr, cx, cy, cz).");
  m.def("clear_backface_cull", &ClearBackfaceCullBakeCUDA,
