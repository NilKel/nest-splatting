...
// Per-Gauss proxy-mesh Z-cull. When d_occluder_depth != nullptr, preprocess
// projects each Gauss centre, looks up the mesh cam-Z at that pixel, and
// early-culls the surfel (radii=0 / tiles_touched=0) if its centre is behind
// the mesh. Fragment-level cull is not implemented for the LEAN CONIC path
// because CONIC discards per-fragment depth (it stores uv0 / J⁻¹ / dw only,
// not the 3×3 T matrix from which depth is reconstructed). Per-Gauss is
// enough for the FPS study — a surfel that fails the check is dropped from
// every downstream stage (preprocess, sort, tile-assign, render).
// nullptr / zeros = no-op → byte-identical to diff_surfel_bake_render_lean.
__device__ const float* d_occluder_depth = nullptr;
__device__ int d_occluder_W = 0;
__device__ int d_occluder_H = 0;

// Per-Gauss backface cull (BFC). When d_bfc_cos <= 1.0f, preprocess drops
// surfels whose outward-oriented disc normal faces away from the camera:
//   n      = 3rd column of quat rotation (disc normal, arbitrary sign)
//   n_out  = n * sign(dot(n, p - centroid))    (centroid orientation)
//   cull if dot(normalize(p - cam), n_out) > d_bfc_cos
// Matches train.py's --backface_cull (same centroid trick + threshold) so a
// BFC-trained checkpoint renders under its training semantics. Sentinel
// 2.0f = off (dot can never exceed 1). One dot product on registers already
// loaded for transMat — effectively free.
__device__ float  d_bfc_cos = 2.0f;
__device__ float3 d_bfc_centroid = {0.f, 0.f, 0.f};
	...
	float* __restrict__ conic_uv = nullptr)
{
	...
	// Per-Gauss proxy-mesh Z-cull. Project surfel centre → pixel, look up
	// mesh cam-Z at that pixel, kill the whole Gauss if its centre is behind
	// the mesh. Downstream stages (sort/tile-assign/render) never see it.
	// Only per-Gauss (not per-fragment) because the LEAN CONIC render path
	// discards per-fragment depth to save fp16 storage.
	if (d_occluder_depth != nullptr) {
		float4 _p_hom = transformPoint4x4(p_orig, projmatrix);
		float  _p_w  = 1.0f / (_p_hom.w + 1e-7f);
		float  _px   = ndc2Pix(_p_hom.x * _p_w, W);
		float  _py   = ndc2Pix(_p_hom.y * _p_w, H);
		int    _ix   = (int)_px;
		int    _iy   = (int)_py;
		if (_ix >= 0 && _ix < d_occluder_W && _iy >= 0 && _iy < d_occluder_H) {
			float _occ = d_occluder_depth[_iy * d_occluder_W + _ix];
			if (isfinite(_occ) && p_view.z > _occ) return;
		}
	}

	// Per-Gauss backface cull (see d_bfc_cos above). Cheap: quat→R 3rd
	// column + two dots; runs before transMat/AABB so culled surfels cost
	// nothing downstream.
	if (d_bfc_cos <= 1.0f) {
		glm::mat3 _R = quat_to_rotmat(rotations[idx]);
		glm::vec3 _n(_R[2][0], _R[2][1], _R[2][2]);
		glm::vec3 _rel(p_orig.x - d_bfc_centroid.x,
		               p_orig.y - d_bfc_centroid.y,
		               p_orig.z - d_bfc_centroid.z);
		if (glm::dot(_n, _rel) < 0.0f) _n = -_n;   // orient outward
		glm::vec3 _vd(p_orig.x - cam_pos->x,
		              p_orig.y - cam_pos->y,
		              p_orig.z - cam_pos->z);
		float _vl = sqrtf(glm::dot(_vd, _vd)) + 1e-12f;
		if (glm::dot(_vd, _n) / _vl > d_bfc_cos) return;
	}
...
// Per-Gauss proxy-mesh Z-cull. Same semantics as diff_surfel_bake_render's
// setter, but this one drops the SURFEL in preprocess (not just fragments in
// render), matching what the LEAN CONIC path can do given its dropped depth.
...
// Backface cull setter (see d_bfc_cos / preprocess block). cos > 1 disables.
__global__ void setBakeBackfaceCullKernel(float cos_thr, float cx, float cy, float cz) {
	d_bfc_cos = cos_thr;
	d_bfc_centroid = make_float3(cx, cy, cz);
}
void FORWARD::setBackfaceCull(float cos_thr, float cx, float cy, float cz) {
	setBakeBackfaceCullKernel<<<1, 1>>>(cos_thr, cx, cy, cz);
}
void FORWARD::clearBackfaceCull() {
	setBakeBackfaceCullKernel<<<1, 1>>>(2.0f, 0.f, 0.f, 0.f);
}
