...
namespace FORWARD
{
	...
	// Per-Gauss proxy-mesh Z-cull. `ptr` = CUDA fp32 [H*W] cam-Z depth (non-hits
	// = +inf). In preprocess, each surfel's centre is projected → looked up →
	// dropped if its cam-Z exceeds occluder[pix]. nullptr = check is a no-op.
	void setOccluderDepth(const float* ptr, int W, int H);
	void clearOccluderDepth();
	void setBackfaceCull(float cos_thr, float cx, float cy, float cz);
	void clearBackfaceCull();
