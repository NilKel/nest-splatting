	...
	const torch::Tensor& scaling_z)
{
  ...
  // `--method res_3d_paired` SLIM out_others: only DEPTH(0) + ALPHA(1) +
  // NORMAL(2..4) are written by the kernel. Channels 5..17 of the original
  // mixed_3d layout (MIDDEPTH, DISTORTION, NUM, POS, VIS, OVERDRAW, MAXDEPTH,
  // W_SQUARE, BETA_SUM) are SKIPPED entirely — for res_3d_paired's typical
  // config (no --lambda_dist / --overdraw_reg / --w_lambda_perpix / mini
  // reinit / 3D mode POS), they are populated but never read. At 4K image
  // resolution this saves ~870 MB of per-pixel forward output and the
  // matching backward gradient tensor.
  // If a future workflow needs one of the trimmed channels, allocate
  // out_dim = 18 and remove the `slot < 5` guards in the kernel.
  int out_dim = 5;
...
// `--method GEStex`: point the forward-read + backward-scatter at the explicit
// per-surfel RGB atlas + its grad accumulator. Called each render; the tensors'
// data must stay alive for the forward/backward. Pass an empty atlas to disable.
void SetGestexAtlasCUDA(torch::Tensor atlas, torch::Tensor atlas_grad, int res, float uv_extent) {
    const float* atlas_ptr = (atlas.numel() > 0) ? atlas.contiguous().data_ptr<float>() : nullptr;
    float* grad_ptr = (atlas_grad.numel() > 0) ? atlas_grad.contiguous().data_ptr<float>() : nullptr;
    FORWARD::setGestexAtlas(atlas_ptr, res, uv_extent);
    BACKWARD::setGestexAtlas(atlas_ptr, grad_ptr, res, uv_extent);
}

void ClearGestexAtlasCUDA() {
    FORWARD::setGestexAtlas(nullptr, 0, 4.0f);
    BACKWARD::setGestexAtlas(nullptr, nullptr, 0, 4.0f);
}

// `--method GEStex`: enable/disable the frontmost-first two-pass blend (both TUs).
void SetGestexFrontmostCUDA(int v) {
    FORWARD::setGestexFrontmost(v);
    BACKWARD::setGestexFrontmost(v);
}
