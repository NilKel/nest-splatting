...
// ===================== --method GEStex explicit texture atlas =====================
// At the joint stage the hashgrid+MLP residual is baked into an explicit per-surfel
// RGB atlas [N, R, R, 3] (u-major, v-minor, channel-last). When d_gestex_atlas is
// non-null, the textured (surfel) residual is a bilinear atlas lookup at the ray-disc
// local coord (s.x, s.y) instead of hash+MLP. Set via set_gestex_atlas each render;
// the backward atomic-scatters into d_gestex_atlas_grad (Python owns that buffer).
__device__ const float* d_gestex_atlas = nullptr;   // [N,R,R,3] atlas values (forward read)
__device__ int   d_gestex_res = 0;                  // atlas resolution R
__device__ float d_gestex_uv_extent = 4.0f;         // UV half-extent (matches bake)

// `--method GEStex` frontmost-first blend (GES sec 4.1: "once w_i > 30 ... the surfel
// with the minimum depth is selected for the first blending computation, while the
// blending order of the others is not adjusted"). When d_gestex_frontmost != 0 the
// forward runs a two-pass blend: a pass-1 min-depth scan finds the frontmost covering
// surfel per pixel, blends it first at T=1, then the main loop blends the rest in tile
// order (skipping the frontmost). 0 (default) → ordinary tile-order blend (byte-identical).
__device__ int d_gestex_frontmost = 0;

// Bilinear-sample the per-Gauss atlas at local (u, v) in [-extent, extent]. Texel-center
// convention matches the bake: texel i centered at (i+0.5)*step - extent, step = 2*extent/R.
__device__ inline void ges_sample_atlas(int gid, float u, float v, float* out3) {
	const float* atlas = d_gestex_atlas;
	const int R = d_gestex_res;
	const float ext = d_gestex_uv_extent;
	if (atlas == nullptr || R <= 0) { out3[0] = out3[1] = out3[2] = 0.0f; return; }
	const float step = 2.0f * ext / (float)R;
	float fu = (u + ext) / step - 0.5f;
	float fv = (v + ext) / step - 0.5f;
	...
	float* __restrict__ out_others)
{
	...
	if (inside)
	{
		...
		// [slim out_others] out_others[pix_id + MIDDEPTH_OFFSET * H * W] = median_depth;
		// [slim out_others] out_others[pix_id + DISTORTION_OFFSET * H * W] = distortion;
		// [slim out_others] out_others[pix_id + NUM_OFFSET * H * W] = render_number;
		// [slim out_others] out_others[pix_id + MAXDEPTH_OFFSET * H * W] = max_depth;
	...
	const float4* __restrict__ ewa_conic = nullptr)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		case 5: {
			...
			bool skip_hash = (d_contrib_thresh > 0.0f && w < d_contrib_thresh)
			                 ...
			                 // `--method GEStex`: textured residual is the baked atlas, not
			                 // the hash+MLP → skip the hash query entirely for these surfels.
			                 || (d_gestex_atlas != nullptr && tex_fwd);
			...
			// `--method GEStex` joint stage: the textured residual is a bilinear lookup
			// into the baked per-surfel RGB atlas at the ray-disc local coord (s.x, s.y).
			// NO hash query, NO MLP (the atlas fully replaces them). Otherwise run the
			// normal hash+MLP residual. Only textured surfels reach here (untextured EWA
			// rows took the !tex_fwd fast path above).
			if (d_gestex_atlas != nullptr && tex_fwd) {
				ges_sample_atlas(gauss_id, s.x, s.y, residual);
			} else {
	...
	if (inside)
	{
		...
		// [slim out_others] out_others[pix_id + (POS_OFFSET + 0) * H * W] = pos_x;
		// [slim out_others] out_others[pix_id + (POS_OFFSET + 1) * H * W] = pos_y;
		// [slim out_others] out_others[pix_id + (POS_OFFSET + 2) * H * W] = pos_z;

		// [slim out_others] out_others[pix_id + (VIS_OFFSET + 0) * H * W] = vis_appearance[0];
		// [slim out_others] out_others[pix_id + (VIS_OFFSET + 1) * H * W] = vis_appearance[1];
		// [slim out_others] out_others[pix_id + (VIS_OFFSET + 2) * H * W] = vis_appearance[2];
		// [slim out_others] out_others[pix_id + OVERDRAW_OFFSET * H * W] = overdraw_sum;
		// [slim out_others] out_others[pix_id + MAXDEPTH_OFFSET * H * W] = max_depth;
		// [slim out_others] out_others[pix_id + WSQUARE_OFFSET * H * W] = w_square_sum;
		// [slim out_others] out_others[pix_id + BETA_SUM_OFFSET * H * W] = beta_sum;
...
// `--method GEStex`: set the explicit texture-atlas pointer + geometry (forward TU).
__global__ void setGestexAtlasFwdKernel(const float* atlas, int res, float uv_extent) {
	d_gestex_atlas = atlas;
	d_gestex_res = res;
	d_gestex_uv_extent = uv_extent;
}
void FORWARD::setGestexAtlas(const float* atlas, int res, float uv_extent) {
	setGestexAtlasFwdKernel<<<1, 1>>>(atlas, res, uv_extent);
}

// `--method GEStex`: enable/disable the frontmost-first two-pass blend (forward TU).
__global__ void setGestexFrontmostFwdKernel(int v) { d_gestex_frontmost = v; }
void FORWARD::setGestexFrontmost(int v) {
	setGestexFrontmostFwdKernel<<<1, 1>>>(v);
}
