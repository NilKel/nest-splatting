...
// ===================== --method GEStex explicit texture atlas (backward) =====================
// Mirrors forward.cu. The textured residual is a bilinear atlas lookup; the backward
// recomputes `residual` from the atlas (for the α-grad feat) and atomic-scatters the
// residual gradient into d_gestex_atlas_grad (Python owns that buffer, reads it back as
// the atlas leaf's .grad). d_gestex_atlas == nullptr → hash+MLP path (unchanged).
__device__ const float* d_gestex_atlas = nullptr;    // [N,R,R,3] atlas values (feat recompute)
__device__ float*       d_gestex_atlas_grad = nullptr; // [N,R,R,3] grad accumulator (atomic scatter)
__device__ int   d_gestex_res_bw = 0;
__device__ float d_gestex_uv_extent_bw = 4.0f;

// `--method GEStex` frontmost-first blend flag (mirror of forward's d_gestex_frontmost).
// When set the backward defers the frontmost (min-depth) surfel and replays its full
// gradient recurrence AFTER the reverse-traversal loop (it was blended first in the
// forward → processed last in the reverse). 0 (default) → ordinary reverse traversal.
__device__ int d_gestex_frontmost = 0;

__device__ inline void ges_sample_atlas_bw(int gid, float u, float v, float* out3) {
	const float* atlas = d_gestex_atlas;
	const int R = d_gestex_res_bw;
	const float ext = d_gestex_uv_extent_bw;
	if (atlas == nullptr || R <= 0) { out3[0] = out3[1] = out3[2] = 0.0f; return; }
	const float step = 2.0f * ext / (float)R;
	float fu = (u + ext) / step - 0.5f, fv = (v + ext) / step - 0.5f;
	int iu0 = (int)floorf(fu), iv0 = (int)floorf(fv);
	float au = fu - (float)iu0, av = fv - (float)iv0;
	int iu1 = iu0 + 1, iv1 = iv0 + 1;
	iu0 = min(max(iu0, 0), R - 1); iu1 = min(max(iu1, 0), R - 1);
	iv0 = min(max(iv0, 0), R - 1); iv1 = min(max(iv1, 0), R - 1);
	const float* base = atlas + (size_t)gid * R * R * 3;
	#pragma unroll
	for (int ch = 0; ch < 3; ch++) {
		float c00 = base[(iu0 * R + iv0) * 3 + ch];
		float c01 = base[(iu0 * R + iv1) * 3 + ch];
		float c10 = base[(iu1 * R + iv0) * 3 + ch];
		float c11 = base[(iu1 * R + iv1) * 3 + ch];
		float c0 = c00 * (1.0f - av) + c01 * av;
		float c1 = c10 * (1.0f - av) + c11 * av;
		out3[ch] = c0 * (1.0f - au) + c1 * au;
	}
}

__device__ inline void ges_scatter_atlas_grad(int gid, float u, float v, const float* dres3) {
	float* grad = d_gestex_atlas_grad;
	const int R = d_gestex_res_bw;
	const float ext = d_gestex_uv_extent_bw;
	if (grad == nullptr || R <= 0) return;
	const float step = 2.0f * ext / (float)R;
	float fu = (u + ext) / step - 0.5f, fv = (v + ext) / step - 0.5f;
	int iu0 = (int)floorf(fu), iv0 = (int)floorf(fv);
	float au = fu - (float)iu0, av = fv - (float)iv0;
	...
	iu0 = min(max(iu0, 0), R - 1); iu1 = min(max(iu1, 0), R - 1);
	iv0 = min(max(iv0, 0), R - 1); iv1 = min(max(iv1, 0), R - 1);
	float w00 = (1.0f - au) * (1.0f - av), w01 = (1.0f - au) * av;
	float w10 = au * (1.0f - av),          w11 = au * av;
	float* base = grad + (size_t)gid * R * R * 3;
	#pragma unroll
	for (int ch = 0; ch < 3; ch++) {
		float g = dres3[ch];
		atomicAdd(&base[(iu0 * R + iv0) * 3 + ch], w00 * g);
		atomicAdd(&base[(iu0 * R + iv1) * 3 + ch], w01 * g);
		atomicAdd(&base[(iu1 * R + iv0) * 3 + ch], w10 * g);
		atomicAdd(&base[(iu1 * R + iv1) * 3 + ch], w11 * g);
	}
}
// =============================================================================================
	...
	const float* __restrict__ scaling_z = nullptr)
{
	...
	if (inside) {
		...
		dL_dmedian_depth = 0.0f /*[slim]*/;
		// dL_dmax_dweight = dL_depths[MEDIAN_WEIGHT_OFFSET * H * W + pix_id];
		dL_doverdraw_px = 0.0f /*[slim]*/;
		dL_dbeta_sum_px = 0.0f /*[slim]*/;
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (!tex_j && ewa_conic != nullptr) {
					...
					if (ok) {
						...
						T = T / (1.f - alpha_e);
						const float dchannel_dcolor = alpha_e * T;
						...
						for (int ch = 0; ch < C; ch++) {
							...
							last_color[ch] = cval;
							dL_dalpha += (cval - accum_rec[ch]) * dL_dpixel[ch];
				...
				if (participates && !tex_j) {
					for (int ch = 0; ch < 3; ch++) {
						acc_dL_dcolors[ch] += dL_dpixel[ch] * w;
					}
				}
				if (participates && tex_j) {
...
const float dL_dmd = 2.0f * (T * alpha) * (m_d * final_A_tex - final_D) * dL_dreg;
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 5: {
				...
				// 3. Recompute residual. `--method GEStex`: the textured residual IS the
				// baked atlas (bilinear at ray-disc UV) — recompute it from the atlas (no
				// hash query, no MLP). The MLP/hash backward below is skipped entirely for
				// these surfels (gated on !ges_atlas_bw), so gestex needs no MLP weights.
				// Otherwise recompute the MLP forward → residual (identity, no sigmoid).
				...
				const bool ges_atlas_bw = (d_gestex_atlas != nullptr && tex_bw);
				if (ges_atlas_bw) {
					ges_sample_atlas_bw(global_id, s.x, s.y, residual);
				} else {
				...
				// `--method GEStex`: dL/dresidual flows ONLY into the explicit atlas texels
				// (bilinear scatter). The entire MLP + hash backward (sections 6/7) is
				// skipped for these surfels — the atlas fully replaces the hash/MLP, so no
				// MLP weights are even read. Untextured/normal surfels take the else path.
				if (ges_atlas_bw) {
					if (d_gestex_atlas_grad != nullptr)
						ges_scatter_atlas_grad(global_id, s.x, s.y, dL_drgb);
				} else {
				...
				}  // end else (!ges_atlas_bw): MLP + hash backward skipped for GEStex atlas surfels
...
// `--method GEStex`: set the atlas value pointer + grad-accumulator pointer + geometry
// (backward TU). atlas/atlas_grad = nullptr disables (hash+MLP backward path).
__global__ void setGestexAtlasBwKernel(const float* atlas, float* atlas_grad, int res, float uv_extent) {
	d_gestex_atlas = atlas;
	d_gestex_atlas_grad = atlas_grad;
	d_gestex_res_bw = res;
	d_gestex_uv_extent_bw = uv_extent;
}
// Host-side mirror of "atlas active" so BACKWARD::render can force the scalar path
// (the collab-GEMM backward has no atlas hooks and the atlas case has no MLP GEMM).
bool h_gestex_atlas_active = false;
void BACKWARD::setGestexAtlas(const float* atlas, float* atlas_grad, int res, float uv_extent) {
	setGestexAtlasBwKernel<<<1, 1>>>(atlas, atlas_grad, res, uv_extent);
	h_gestex_atlas_active = (atlas != nullptr);
}

// `--method GEStex`: enable/disable the frontmost-first two-pass blend (backward TU).
__global__ void setGestexFrontmostBwKernel(int v) { d_gestex_frontmost = v; }
// Host-side mirror so BACKWARD::render can force the scalar path (the collab-GEMM
// backward has no deferred-minGeo hooks — the reorder lives only in the scalar path).
bool h_gestex_frontmost_active = false;
void BACKWARD::setGestexFrontmost(int v) {
	setGestexFrontmostBwKernel<<<1, 1>>>(v);
	h_gestex_frontmost_active = (v != 0);
}
	...
	const float* scaling_z)
{
	...
	if (((render_mode & 0xFF) == 5 || (render_mode & 0xFF) == 6) && dL_dmlp_W1 != nullptr) {
		...
		// `--method GEStex`: atlas active → force scalar backward (atlas hooks are scalar-
		// only; the atlas case has no MLP GEMM to collaborate on).
		if (!force_disable && !h_gestex_atlas_active && !h_gestex_frontmost_active) {
