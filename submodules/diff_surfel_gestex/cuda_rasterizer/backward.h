...
namespace BACKWARD
{
	...
	// `--method res_3d_paired`: backward mirror of FORWARD::setTexturedBiasGate.
	void setTexturedBiasGate(int v);
	// `--lru`: leaky-ReLU slope α for the outer per-Gauss ReLU (mode 0 only).
	// α == 0 (default) → standard ReLU. α > 0 → grad scaled by α at clamped sites.
	...
	// `--method GEStex`: set atlas value + grad-accumulator pointers + R + uv_extent.
	void setGestexAtlas(const float* atlas, float* atlas_grad, int res, float uv_extent);
