...
namespace FORWARD
{
	...
	// `--method res_3d_paired`: when 1, kernel forces sh_color = 0 for
	// textured Gauss → feat = residual only (no +0.5 SH-bias baseline).
	// Default 0 = unchanged behavior.
	void setTexturedBiasGate(int v);

	// `--lru`: leaky-ReLU slope α for the outer per-Gauss activation (mode 0).
	// α == 0 (default) → standard ReLU. α > 0 → forward+backward leak.
	void setLruSlope(float v);

	// `--method GEStex`: set the explicit per-surfel RGB texture-atlas pointer + R +
	// uv_extent (forward-read). atlas=nullptr disables (hash+MLP residual path).
	void setGestexAtlas(const float* atlas, int res, float uv_extent);

	// `--method GEStex`: enable (1) / disable (0) the frontmost-first two-pass blend.
	void setGestexFrontmost(int v);
