...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_intersection_buffer, grad_intersection_count, grad_geomBuffer, grad_out_index, grad_metric_counts, grad_out_color_untex):
        ...
        # Debug print removed (forced a per-500-backward sync; masked async CUDA errors).
...
def set_gestex_atlas(atlas, atlas_grad, res, uv_extent=4.0):
    """`--method GEStex`: point the textured residual at an explicit per-surfel RGB
    atlas [N,R,R,3] (bilinear sample at the ray-disc UV) instead of the hash+MLP.
    `atlas` is read in the forward; `atlas_grad` (same shape) accumulates the residual
    gradient in the backward (Python reads it back as the atlas leaf's .grad). Both must
    stay alive across the forward+backward. Pass empty tensors to disable."""
    _C.set_gestex_atlas(atlas, atlas_grad, int(res), float(uv_extent))

def clear_gestex_atlas():
    """`--method GEStex`: disable the atlas (revert to the hash+MLP residual path)."""
