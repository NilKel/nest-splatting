...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_textured_bias_gate", &SetTexturedBiasGateCUDA,
        "`--method res_3d_paired`: when 1, force sh_color = 0 for textured "
        "Gauss → feat = residual (no +0.5 SH-bias floor on tex carriers). "
  ...
  m.def("set_gestex_atlas", &SetGestexAtlasCUDA,
        "`--method GEStex`: point the textured residual at an explicit per-surfel "
        "RGB atlas [N,R,R,3] (bilinear at ray-disc UV) + its grad buffer.");
  m.def("clear_gestex_atlas", &ClearGestexAtlasCUDA,
        "`--method GEStex`: disable the atlas (revert to hash+MLP residual path).");
