...
RasterizeGaussiansCUDA(
	...
	// `--method mixed`: per-Gauss bool flag [P]. Empty tensor → all-true (textured)
	// behavior (kernel branch falls through to standard 3D_SH_res path).
	const torch::Tensor& is_textured,
	// `--method mixed_3d`: per-Gauss activated 3rd-axis scale [P]. Empty tensor →
	// untextured surfels keep 2DGS ray-splat geometry (bit-identical to mixed).
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
 ...
 RasterizeGaussiansBackwardCUDA(
	...
	// `--method mixed`: per-Gauss bool flag [P]. Empty tensor → all-true.
	const torch::Tensor& is_textured,
	// `--method mixed_3d`: activated 3rd-axis scale [P] (empty → 2DGS path).
	const torch::Tensor& scaling_z);
...
// `--method res_3d_paired`: per-Gauss bias gate. When 1, kernel forces
// sh_color = 0 for textured Gauss → feat = residual (no +0.5 floor).
// Default 0 = byte-identical to plain mixed_3d.
...
// `--method GEStex`: set / clear the explicit per-surfel RGB texture-atlas pointers.
