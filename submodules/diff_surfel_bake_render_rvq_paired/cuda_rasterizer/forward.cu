...
// ---- PAIRED RVQ: collapse L=4 stages → L=2 paired codebooks. -------------
// pair_cb[p][k_hi, k_lo] = cb[2p][k_hi] + cb[2p+1][k_lo]
// Per-block: 1 uint32 load = (pair1_idx << 16) | pair0_idx; each pair_idx
// is uint16 = (code_2p << 8) | code_2p+1, so c_hi = idx >> 8, c_lo = idx & 0xFF.
// 2D pair-cb texture layout: 1024 cols × (2 * 1024) rows.
//   codeword (c_hi, c_lo) of pair p at:  u = c_lo*B + intra_u
//                                        v = p*(K*B) + c_hi*B + intra_v
__device__ cudaTextureObject_t d_rvq_pair_cb_tex = 0;
__device__ const uint32_t* d_rvq_pair_indices = nullptr;   // [N_used] packed
__device__ int   d_rvq_pair_K = 256;                       // original K (per stage)
__device__ float d_rvq_pair_scale[2]  = {1.0f, 1.0f};
__device__ float d_rvq_pair_offset[2] = {0.0f, 0.0f};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (atlas_rects != nullptr) {
				...
				{
					// PAIRED RVQ — L=2 stages instead of L=4. Per fragment:
					//   1 uint32 load (packed pair indices)  →  2 tex2D fetches.
					// Halves the dependency-chain depth vs the L=4 path.
					const int B = d_rvq_B;
					...
					const cudaTextureObject_t pair_tex = d_rvq_pair_cb_tex;
					const int K = d_rvq_pair_K;
					const int K_B = K * B;                       // row stride for pair stacking
					...
					auto sample_at = [&](float au_in, float av_in, float* rgb) {
						...
						// Single packed-index load.
						uint32_t packed = d_rvq_pair_indices[bid];
						uint32_t p0 = packed & 0xFFFFu;
						uint32_t p1 = packed >> 16;
						// Each pair uint16 = (code_stage_2p << 8) | code_stage_2p+1
						int c0_hi = (int)(p0 >> 8),  c0_lo = (int)(p0 & 0xFF);
						int c1_hi = (int)(p1 >> 8),  c1_lo = (int)(p1 & 0xFF);

						float4 rgba0 = tex2D<float4>(pair_tex,
							(float)(c0_lo * B + intra_u) + 0.5f,
							(float)(0       + c0_hi * B + intra_v) + 0.5f);
						float4 rgba1 = tex2D<float4>(pair_tex,
							(float)(c1_lo * B + intra_u) + 0.5f,
							(float)(K_B     + c1_hi * B + intra_v) + 0.5f);

						float s0 = d_rvq_pair_scale[0],  o0 = d_rvq_pair_offset[0];
						float s1 = d_rvq_pair_scale[1],  o1 = d_rvq_pair_offset[1];
						rgb[0] = rgba0.x * s0 + o0 + rgba1.x * s1 + o1;
						rgb[1] = rgba0.y * s0 + o0 + rgba1.y * s1 + o1;
						rgb[2] = rgba0.z * s0 + o0 + rgba1.z * s1 + o1;
					};
					// Bilinear sampling. Convention: the 4 taps are at floor(au), floor(au)+1
					// in u and same in v. If all 4 fall in the same 4×4 atlas block AND
					// stay inside the surfel rect, take the SAME-BLOCK fast path: 1
					// packed-index load + 2 hw-bilinear tex2D fetches (sampler does the
					// 4-tap interpolation within the codeword). Otherwise fall back to
					// explicit 4-tap with sample_at.
					float au0_f = floorf(au); float av0_f = floorf(av);
					float fu = au - au0_f; float fv = av - av0_f;
					int lu0 = (int)au0_f - u0i;
					int lv0 = (int)av0_f - v0i;
					// Same-block check + in-rect check. Both bilinear taps in [intra, intra+1]
					// must stay within the same 4×4 block AND inside the surfel.
					bool inside_rect = (lu0 >= 0) && (lv0 >= 0)
					                 && (lu0 + 1 < (int)u_span) && (lv0 + 1 < (int)v_span);
					int intra_u0 = lu0 & (B - 1);            // local pos within block (B=4 ⇒ & 3)
					int intra_v0 = lv0 & (B - 1);
					bool same_block_uv = (intra_u0 < B - 1) && (intra_v0 < B - 1);

					if (inside_rect && same_block_uv) {
						int bu0 = lu0 / B;
						int bv0 = lv0 / B;
						long long bid = surfel_base + (long long)bv0 * bw_g + bu0;
						uint32_t packed = d_rvq_pair_indices[bid];
						uint32_t p0 = packed & 0xFFFFu;
						uint32_t p1 = packed >> 16;
						int c0_hi = (int)(p0 >> 8),  c0_lo = (int)(p0 & 0xFF);
						...
						// Sub-pixel coord WITHIN the codeword: (intra_u0 + fu, intra_v0 + fv).
						// With cudaFilterModeLinear the tex2D samples 4 neighbouring
						// texels and bilinear-blends — exactly the bilinear we'd do
						// in software, done in the sampler unit.
						float u0_ = (float)(c0_lo * B) + (float)intra_u0 + fu + 0.5f;
						float v0_ = (float)(c0_hi * B) + (float)intra_v0 + fv + 0.5f;
						float u1_ = (float)(c1_lo * B) + (float)intra_u0 + fu + 0.5f;
						float v1_ = (float)(K_B + c1_hi * B) + (float)intra_v0 + fv + 0.5f;
						float4 rgba0 = tex2D<float4>(pair_tex, u0_, v0_);
						float4 rgba1 = tex2D<float4>(pair_tex, u1_, v1_);
						float s0 = d_rvq_pair_scale[0],  o0 = d_rvq_pair_offset[0];
						float s1 = d_rvq_pair_scale[1],  o1 = d_rvq_pair_offset[1];
						feat[0] += rgba0.x * s0 + o0 + rgba1.x * s1 + o1;
						feat[1] += rgba0.y * s0 + o0 + rgba1.y * s1 + o1;
						feat[2] += rgba0.z * s0 + o0 + rgba1.z * s1 + o1;
					} else {
						// Cross-block / boundary fragment: 4 explicit taps. Each tex2D
						// here is also hw-bilinear (Linear sampler) but we only need
						// the texel value (sub-pixel is 0.5) so it returns the texel
						// — same as point sampling. The explicit fu/fv combine below
						// gives the correct cross-block bilinear.
...
__global__ void setAtlasRVQPairedKernel(
	cudaTextureObject_t pair_tex, const uint32_t* packed_indices, int K,
	float s0, float o0, float s1, float o1
) {
	d_rvq_pair_cb_tex   = pair_tex;
	d_rvq_pair_indices  = packed_indices;
	d_rvq_pair_K        = K;
	d_rvq_pair_scale[0] = s0; d_rvq_pair_offset[0] = o0;
	d_rvq_pair_scale[1] = s1; d_rvq_pair_offset[1] = o1;
}
void FORWARD::setAtlasRVQPaired(
	cudaTextureObject_t pair_tex, const uint32_t* packed_indices, int K,
	float s0, float o0, float s1, float o1
) {
	setAtlasRVQPairedKernel<<<1, 1>>>(pair_tex, packed_indices, K, s0, o0, s1, o1);
}
