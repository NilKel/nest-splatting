...
namespace FORWARD
{
	...
	void clearAtlasRVQ();
	// Install 2D codebook texture object + per-stage min/max dequant params.
	void setAtlasRVQTex2D(cudaTextureObject_t tex);
	void setAtlasRVQDequant(int l, float scale, float offset);
	// PAIRED RVQ: collapsed L=4 → L=2. Pair tex is K_orig*B cols × 2*K_orig*B
	// rows (stacked vertically). packed_idx[bid] = (pair1_idx<<16)|pair0_idx,
	// each pair_idx = (code_high_stage<<8) | code_low_stage.
	void setAtlasRVQPaired(cudaTextureObject_t pair_tex,
	                       const uint32_t* packed_indices, int K_orig,
