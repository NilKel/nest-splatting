...
// `--densfix` (d_exclude_hash_from_densify): backward-only. When 1, the AbsGS
// densification proxy (the abs Tu.z/Tv.z channels of dL_dmean2D) is computed
// from the FOOTPRINT-ONLY dL_ds — i.e. EXCLUDING the hash-query-point term
// (dL_duv from dL_dxyz). The full (hash-inclusive) dL_ds still drives transMat /
// mean3D, so surfels keep repositioning on both SV + hash gradients; only the
// densification score drops the hash boost. Default 0 = byte-identical.
__device__ int d_exclude_hash_from_densify = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (participates) {
					...
					if (rho3d <= rho2d) {
						...
						// `--densfix`: footprint-only dL_ds (BEFORE the hash dL_duv term) so the
						// AbsGS densify proxy below can optionally exclude the hash-query-point
						// contribution while transMat/mean3D still gets the full gradient.
						const float2 dL_ds_fp = {dL_ds.x, dL_ds.y};
						dL_ds.x += dL_duv.x;
						dL_ds.y += dL_duv.y;
						...
						// `--densfix`: Tu.z/Tv.z depend only on dL_ds.{x,y} (via dL_dp.{x,y}), so
						// recompute the .z from footprint-only dL_ds to drop the hash boost.
						if (d_exclude_hash_from_densify) {
							const float dsx_pz_fp = dL_ds_fp.x / p_stored.z;
							const float dsy_pz_fp = dL_ds_fp.y / p_stored.z;
							acc_dL_dmean2D[2] += fabsf(-(l_stored.x * dsy_pz_fp - l_stored.y * dsx_pz_fp)); // -dL_dk.z
							acc_dL_dmean2D[3] += fabsf(-(dsx_pz_fp * k_stored.y - dsy_pz_fp * k_stored.x)); // -dL_dl.z
						} else {
		...
		if (alpha < 1.0f / 255.0f)
			...
			if (is_aa) {
				...
				// `--densfix`: footprint-only dL_ds (BEFORE the hash dL_duv term).
				const float2 dL_ds_fp = {dL_ds.x, dL_ds.y};
				dL_ds.x += dL_duv.x;
				dL_ds.y += dL_duv.y;
				...
				atomicAdd(&dL_dtransMat[global_id * 9 + 8],  dL_dTw.z);
				// AbsGS: accumulate absolute Tu.z / Tv.z for cancellation-free densification.
				// `--densfix`: recompute .z from footprint-only dL_ds to drop the hash boost.
				if (d_exclude_hash_from_densify) {
					const float dsx_pz_fp = dL_ds_fp.x / p.z;
					const float dsy_pz_fp = dL_ds_fp.y / p.z;
					atomicAdd(&dL_dmean2D[global_id].z, fabsf(-(l.x * dsy_pz_fp - l.y * dsx_pz_fp))); // -dL_dk.z
					atomicAdd(&dL_dmean2D[global_id].w, fabsf(-(dsx_pz_fp * k.y - dsy_pz_fp * k.x))); // -dL_dl.z
				} else {
					atomicAdd(&dL_dmean2D[global_id].z, fabsf(dL_dTu.z));
					atomicAdd(&dL_dmean2D[global_id].w, fabsf(dL_dTv.z));
				}
...
// `--densfix`: exclude the hash-query-point term from the densification proxy.
__global__ void setExcludeHashFromDensifyBwKernel(int v) { d_exclude_hash_from_densify = v; }
void BACKWARD::setExcludeHashFromDensify(int v) {
	setExcludeHashFromDensifyBwKernel<<<1, 1>>>(v);
}
