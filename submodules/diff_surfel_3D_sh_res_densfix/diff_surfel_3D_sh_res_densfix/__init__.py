...
def set_exclude_hash_from_densify(v=0):
    """`--densfix`: backward-only. When v=1, the AbsGS densification proxy (the
    abs Tu.z/Tv.z channels of dL_dmean2D that FastGS gates on under `--grads abs`)
    is computed from the FOOTPRINT-ONLY dL_ds — excluding the hash-query-point
    term (dL_duv from the hash's dL_dxyz). The full (hash-inclusive) dL_ds still
    drives transMat/mean3D, so surfels keep repositioning on both SV + hash
    gradients; only the densification score drops the hash-inflated boost. v=0 =
    byte-identical to the base rasterizer."""
    _C.set_exclude_hash_from_densify(int(v))
