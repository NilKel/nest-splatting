...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_exclude_hash_from_densify", &SetExcludeHashFromDensifyCUDA,
        "`--densfix`: backward-only. 1 = the AbsGS densification proxy (abs "
        "Tu.z/Tv.z channels of dL_dmean2D) is computed from footprint-only "
        "dL_ds, EXCLUDING the hash-query-point term; transMat/mean3D optimizer "
        "gradient still gets the full (hash-inclusive) gradient. 0 = default.");
