	...
	float4* __restrict__ ewa_conic = nullptr)
{
	...
	if (!untex_ewa) {
		...
		// SnugBox+AccuTile enabled for ALL rect modes (2, 3, 5). On mode 3
		// (AdR), the smaller cutoff feeds into compute_conic_from_transmat()
		// the same way; AdR shrinks the per-Gauss ellipse, AccuTile then
		// enumerates only the tiles where that smaller ellipse actually
		// intersects. The rect-AABB code path below is still reached as a
