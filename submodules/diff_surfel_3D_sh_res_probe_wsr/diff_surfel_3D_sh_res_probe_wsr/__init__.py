...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_intersection_buffer, grad_intersection_count, grad_geomBuffer, grad_out_index, grad_metric_counts):
        ...
        if raster_settings.debug:
            ...
            try:
                (grad_features, grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D,
                 ...
                 grad_mlp_W1, grad_mlp_W2, grad_mlp_W3, grad_tex
        ...
        grads = (
            ...
            None,  # offsets
            None,  # gridrange
            grad_features_diffuse,  # PROBERES: dL/dprobes [N,6] (probes ride this slot)
            None,  # offsets_diffuse (PROBERES: tex dims {Ht,Wt}, int — no grad)
            # PROBERES: gridrange_diffuse carries the texture image → return its grad
            grad_tex if (render_mode & 0x1000) else None,
...
def set_wsr(mode, occ=None, occ_grad=None, aux=None):
    """WSR (sort-free weighted-sum) mode.

    mode=1 switches the surfel render kernel to the order-independent composite
        out = (1-P) * (Σ w_i c_i / Σ w_i),  P = Π(1-α_i),  w_i = α_i·occ_i
    with `occ` a [N] fp32 CUDA tensor of ACTIVATED per-surfel occlusions in
    [0,1] (e.g. torch.sigmoid of the raw parameter — chain the sigmoid
    derivative manually when applying `occ_grad`). `occ_grad` is a [N] fp32
    zero-initialized accumulator the backward atomicAdds dL/docc into (the
    device-global grad pattern, cf. get_mlp_grads / the gestex atlas).
    `aux` is a [4, H, W] fp32 scratch the forward fills (C̄ rgb + weight sum)
    and the backward reads — allocate per render, keep alive through backward.

    Raw pointers are installed into device globals: re-call before EVERY
    forward while active, keep the tensors alive, and call set_wsr(0) to
    disable (eval/sorted renders)."""
    global _WSR_EMPTY
    if _WSR_EMPTY is None:
        _WSR_EMPTY = torch.empty(0, dtype=torch.float32, device="cuda")
    e = _WSR_EMPTY
    _C.set_wsr(int(mode),
               e if occ is None else occ,
               e if occ_grad is None else occ_grad,
               e if aux is None else aux)

def set_wsr_gate(tau, bins=16, zmin=0.2, zmax=120.0, tbin=None):
    """WSR transmittance gate (the 2-pass saturation gate).

    tau > 0 arms a lean pre-pass that walks every fragment (alpha + depth
    only), bins Σlog(1−α) into `bins` (≤16) log-depth bins per pixel over
    [zmin, zmax], and writes the EXCLUSIVE prefix transmittance per bin to
    `tbin` (a [bins, H, W] fp32 CUDA tensor, allocated per render and kept
    alive through backward). Forward AND backward then fully discard any
    fragment whose bin transmittance < tau — "once accumulated opacity
    saturates, gate the remaining surfels", restated as a depth relation so
    it is order-independent and reproducible by the WebGPU 2-pass renderer
    (same zmin/zmax/bins constants go in the WGSL uniforms).

    Only active while set_wsr(mode>0). Call set_wsr_gate(0.0) to disable."""
    global _WSR_EMPTY
    if _WSR_EMPTY is None:
        _WSR_EMPTY = torch.empty(0, dtype=torch.float32, device="cuda")
    _C.set_wsr_gate(float(tau), int(bins), float(zmin), float(zmax),
                    _WSR_EMPTY if tbin is None else tbin)

def set_wsr_dgate(margin, dbuf=None):
    """WSR mean-depth gate (?wsr=3, "D-gate").

    margin > 0 arms a SORTED pre-pass that computes per pixel the
    alpha-weighted mean depth D̄ = Σ(αTd)/Σ(αT) and saturation A = Σ(αT)
    into `dbuf` (a [2, H, W] fp32 CUDA tensor, allocated per render, alive
    through backward). Forward AND backward then fade the occ weight of
    fragments behind D̄ over the relative window [D̄(1+m/2), D̄(1+3m/2)],
    scaled by g(A) = smoothstep(0.6, 0.9, A) so unsaturated pixels never
    gate their own background. The saturation depth is an INTEGRAL of
    opacity — robust to the local order swaps that make sorted color
    compositing pop — so the sort is used only for this scalar anchor and
    color stays order-independent. Only active while set_wsr(mode>0);
    set_wsr_dgate(0.0) disables."""
    global _WSR_EMPTY
    if _WSR_EMPTY is None:
        _WSR_EMPTY = torch.empty(0, dtype=torch.float32, device="cuda")
    _C.set_wsr_dgate(float(margin), _WSR_EMPTY if dbuf is None else dbuf)
