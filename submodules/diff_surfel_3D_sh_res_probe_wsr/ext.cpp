...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_wsr", &SetWsrCUDA,
        "WSR (sort-free weighted-sum) mode. (mode, occ[N], occ_grad[N], aux[4*H*W]) — "
        "installs pointers into forward+backward device globals; re-call before every "
        "render while active; (0, empty, empty, empty) disables.");
  m.def("set_wsr_dgate", &SetWsrDGateCUDA,
        "WSR mean-depth gate (?wsr=3). (margin, dbuf[2*H*W]) — margin>0 arms a "
        "SORTED pre-pass computing per-pixel (mean depth, saturation); render+"
        "backward fade occ behind the mean depth. (0, empty) disables.");
  m.def("set_wsr_gate", &SetWsrGateCUDA,
        "WSR transmittance gate (2-pass). (tau, bins<=16, zmin, zmax, tbin[bins*H*W]) — "
        "tau>0 arms a depth-binned pre-pass; render+backward discard fragments whose "
        "bin's exclusive-prefix T < tau. (0, 16, ., ., empty) disables.");
