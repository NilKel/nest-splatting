...
// ======================= WSR (sort-free weighted-sum) mode =======================
// d_wsr_mode = 1 switches the surfel render kernel from sorted alpha-blending to
// the order-independent weighted-sum composite
//     out = (1 - P) * (Σ w_i c_i / Σ w_i),   P = Π(1 - α_i),  w_i = α_i · occ_i
// where occ_i (d_wsr_occ, [N], activated to [0,1]) is a learned per-surfel
// occlusion factor approximating the surfel's mean transmittance. With
// occ_i == T_i (the true per-fragment transmittance) this coincides exactly with
// sorted blending (Σ α_i T_i = 1 - P telescopes). Early-out is DISABLED under
// WSR: the sort-free operator includes every fragment, so training must too —
// occluded-but-visible-elsewhere surfels are exactly the leakage the finetune
// has to learn to suppress.
// d_wsr_aux ([4*H*W], allocated per-render by the Python side) carries the
// per-pixel normalized color C̄ (ch 0..2) and the weight sum den (ch 3) from
// forward to backward. Pointers installed via FORWARD::setWsr each render
// (device-global pointer pattern, cf. set_mlp_weights / gestex atlas).
__device__ int d_wsr_mode = 0;
__device__ const float* d_wsr_occ = nullptr;   // [N] activated occlusion in [0,1]
__device__ float* d_wsr_aux = nullptr;         // [4*H*W]: 0..2 = C̄, 3 = den

// --------------------- WSR transmittance gate (2-pass) ----------------------
// "Once accumulated opacity saturates, gate the remaining surfels" — restated
// as a DEPTH relation so it stays order-independent (deployable in the WebGPU
// 2-pass renderer, which has no sorted per-pixel walk): a fragment at depth z
// participates iff T_before(z) = Π_{z' in strictly nearer BINS} (1−α') ≥ tau.
// A lean pre-pass (wsrGatePrepassCUDA, launched from FORWARD::render before
// the main kernel) walks every fragment computing ONLY alpha + depth, bins
// log(1−α) into WSR_GATE_MAX_BINS log-depth bins per pixel, and writes the
// EXCLUSIVE prefix product T_before(bin) to d_wsr_tbin [bins*H*W]. Fragments
// in the same bin never gate each other (bin-width leakage floor), and the
// frontmost occupied bin always survives (T_before = 1).
// Bin mapping (must byte-match the viewer's WGSL):
//   d = clamp((log2(1+z) − d_wsr_gate_lo) * d_wsr_gate_invr, 0, 1)
//   bin = min(bins−1, floor(d·bins)),  lo = log2(1+zmin), invr = 1/(log2(1+zmax)−lo)
// NOTE: not the viewer's ht_depth (log2(1+z)/22) — that mapping collapses a
// room-scale scene into ~4 of 16 bins; the parametric range keeps ~1.33×
// relative depth per bin at (0.2, 120, 16).
#define WSR_GATE_MAX_BINS 16
__device__ float  d_wsr_gate_tau  = 0.0f;      // 0 = gate off
__device__ int    d_wsr_gate_bins = WSR_GATE_MAX_BINS;
__device__ float  d_wsr_gate_lo   = 0.0f;      // log2(1+zmin)
__device__ float  d_wsr_gate_invr = 0.0f;      // 1/(log2(1+zmax) − lo)
__device__ float* d_wsr_tbin      = nullptr;   // [bins*H*W] exclusive-prefix T

__device__ __forceinline__ int wsr_gate_bin(float depth) {
	float d = (log2f(1.0f + fmaxf(depth, 0.0f)) - d_wsr_gate_lo) * d_wsr_gate_invr;
	d = fminf(fmaxf(d, 0.0f), 1.0f);
	return min(d_wsr_gate_bins - 1, (int)(d * (float)d_wsr_gate_bins));
}

// Continuous soft gate factor s ∈ [0,1]. The tbin buffer stores the
// EXCLUSIVE cumulative optical depth per bin (od_before, positive); the
// fragment's od is linearly interpolated INSIDE its bin (deep-shadow-map
// style, assumes own-bin mass uniform in depth) so T(z) = exp(−od(z)) is
// C0-continuous in depth — a piecewise-constant T stepped at bin edges,
// and bin edges are camera-attached iso-depth planes, so every camera
// move made whole bands of fragments flip at once (the "rippling lines
// that move with the camera" artifact). The gate itself is a smoothstep
// over T ∈ [0.5τ, 1.5τ] folded into the occ weight (see call sites), not
// a hard discard — contributions fade to zero continuously.
__device__ __forceinline__ float wsr_gate_soft(float depth, int H, int W, uint32_t pix_id) {
	float d = (log2f(1.0f + fmaxf(depth, 0.0f)) - d_wsr_gate_lo) * d_wsr_gate_invr;
	d = fminf(fmaxf(d, 0.0f), 1.0f) * (float)d_wsr_gate_bins;
	// Evaluate the interpolated prefix ONE BIN NEARER than the fragment.
	// This is the crux: interpolating at the fragment's own position mixes
	// in its OWN bin's mass, so a surface stack inside one bin partially
	// gates ITSELF (frac-dependent → catastrophic, and camera-attached).
	// One bin back, co-surface mass (hard-binned at the fragment's bin)
	// never counts, and the lerp endpoints meet exactly at bin crossings —
	// the gate value is continuous under any camera motion. Occluders must
	// be ≥1 bin (~1.33× relative depth) nearer to start gating (guard band).
	float dg = fmaxf(d - 1.0f, 0.0f);
	int b = min(d_wsr_gate_bins - 1, (int)dg);
	float frac = fminf(fmaxf(dg - (float)b, 0.0f), 1.0f);
	float od = d_wsr_tbin[b * H * W + pix_id];
	if (b + 1 < d_wsr_gate_bins)
		od += frac * (d_wsr_tbin[(b + 1) * H * W + pix_id] - od);
	const float T = expf(-od);
	float s = (T - 0.5f * d_wsr_gate_tau) / d_wsr_gate_tau;   // fade over [0.5τ, 1.5τ]
	s = fminf(fmaxf(s, 0.0f), 1.0f);
	return s * s * (3.0f - 2.0f * s);
}

// ---------------- WSR mean-depth gate (?wsr=3, "D-gate") --------------------
// The user-correct observation behind this variant: the SATURATION DEPTH is an
// INTEGRAL of opacity along the ray — robust to the local order swaps that
// make sorted COLOR compositing pop. So use the sorted order only for the
// scalar visibility anchor and blend color order-independently:
//   pre-pass (sorted walk): D̄ = Σ(α·T·d)/Σ(α·T)  (the sorted mean depth) and
//                            A  = Σ(α·T) = 1 − T_final (how saturated).
//   render:  w = α·occ·s,  s = 1 − g(A)·smoothstep(D̄(1+m/2), D̄(1+3m/2), d)
// g(A) = smoothstep(0.6, 0.9, A) disables the gate on unsaturated pixels
// (windows / soft edges — gating behind the MEAN of a translucent mix would
// cut genuinely visible background). m = d_wsr_dgate_margin (relative depth
// margin, default 0.15) — "slightly behind the actual saturation". No bins ⇒
// nothing camera-attached to ripple; D̄ moves continuously with the camera.
// Viewer twin: sorted premultiplied over-blend of (d·α, α) — the blend unit
// computes both sums in hardware — then fs_wsr_gated_d. Coverage keeps raw α
// (occ-fold pattern, cf. the binned gate v2).
__device__ float  d_wsr_dgate_margin = 0.0f;   // 0 = off
__device__ float* d_wsr_dbuf = nullptr;        // [2*H*W]: 0 = D̄, 1 = A

// Host shadows so FORWARD::render can decide whether to launch the pre-pass.
static int    h_wsr_mode_fw   = 0;
static float  h_wsr_gate_tau  = 0.0f;
static float* h_wsr_tbin      = nullptr;
static float  h_wsr_dgate_margin = 0.0f;
static float* h_wsr_dbuf      = nullptr;
...
// WSR gate pre-pass: alpha-only walk over every fragment (no color, no hash,
// no probe fetch), accumulating log(1−α) into per-pixel log-depth bins, then
// storing the exclusive-prefix transmittance per bin to d_wsr_tbin. The alpha
// / depth math below MUST mirror renderCUDAsurfelForward's scalar path exactly
// (same kernel_type branches, same 0.99 clamp, same 1/255 cull, same pix_off)
// so the gate decision is consistent with what the render pass then blends.
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
wsrGatePrepassCUDA(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	...
	const int render_mode)
{
	...
	__shared__ float3 collected_Tv[BLOCK_SIZE];
	__shared__ float3 collected_Tw[BLOCK_SIZE];
	__shared__ float2 collected_shapes[BLOCK_SIZE];

	float binlog[WSR_GATE_MAX_BINS] = { 0 };
	const int bins = min(d_wsr_gate_bins, WSR_GATE_MAX_BINS);
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (kernel_type == 1 || kernel_type == 4) {
				...
				alpha = fminf(0.99f, opa * fmaxf(alpha_beta, alpha_lp));
				...
				float power = -0.5f * powf(rho_safe, 0.5f * beta_param);
				if (power > 0.0f) continue;
				...
				const float rho_aa = ((s.x*s.x + s.y*s.y) + ks * (term1*term1 + term2*term2)) * det_V_inv;
			...
			binlog[wsr_gate_bin(depth)] += -logf(1.0f - alpha);   // optical depth
		}
	}

	if (inside && d_wsr_tbin != nullptr) {
		// EXCLUSIVE cumulative optical depth per bin (od_before, positive);
		// the gate interpolates within the bin (wsr_gate_soft).
		float acc = 0.0f;
		for (int b = 0; b < bins; b++) {
			d_wsr_tbin[b * H * W + pix_id] = acc;
			acc += binlog[b];
		}
	}
}

// WSR mean-depth gate pre-pass: SORTED walk (fragment lists are depth-sorted
// here) accumulating the alpha-weighted mean depth D̄ = Σ(αTd)/Σ(αT) and the
// saturation A = Σ(αT). Alpha/depth math mirrors the render kernel's scalar
// path (same kernel branches, clamps, culls). Early-out at T < 1e-4 — D̄ is a
// sorted quantity, so termination past saturation is exact, not a shortcut.
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
wsrDGatePrepassCUDA(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	...
	const int render_mode)
{
	...
	float T = 1.0f, dsum = 0.0f, wsum = 0.0f;
	...
	if (inside && d_wsr_dbuf != nullptr) {
		d_wsr_dbuf[pix_id] = dsum / fmaxf(wsum, 1e-8f);   // D̄
		d_wsr_dbuf[H * W + pix_id] = wsum;                // A = 1 − T_final
	}
}
	...
	int* __restrict__ metric_counts = nullptr)
{
	...
	uint32_t last_contributor = 0;
	float C[CHANNELS] = { 0 };
	float wsr_den = 0.0f;  // WSR: Σ w_i = Σ α_i·occ_i (weight-sum denominator)
	// WSR composite (d_wsr_mode == 2): running frontmost fragment by exact
	// per-pixel intersection depth (the CUDA analogue of the viewer's ht=1
	// z-buffer core). The accumulators above keep ALL fragments; write-out
	// removes the winner algebraically (num − w_F·feat_F, den − w_F,
	// P_tail = P/(1−α_F)) so the pass stays order-independent.
	float wsr_front_depth = 1e30f;
	float wsr_front_alpha = 0.0f;
	float wsr_front_w = 0.0f;
	float wsr_front_feat[CHANNELS] = { 0 };
	int wsr_front_id = -1;
	// WSR mean-depth gate: per-pixel fade anchors from the sorted pre-pass
	// (D̄, A). t0/t1 = fade window edges; g = gate strength (0 on unsaturated
	// pixels so translucent content never gates its own background).
	float wsr_dg_t0 = 1e30f, wsr_dg_inv = 0.0f, wsr_dg_g = 0.0f;
	if (d_wsr_mode && d_wsr_dgate_margin > 0.0f && d_wsr_dbuf != nullptr && inside) {
		const float Dbar = d_wsr_dbuf[pix_id];
		const float A = d_wsr_dbuf[H * W + pix_id];
		wsr_dg_t0 = Dbar * (1.0f + 0.5f * d_wsr_dgate_margin);
		const float t1 = Dbar * (1.0f + 1.5f * d_wsr_dgate_margin);
		wsr_dg_inv = 1.0f / fmaxf(t1 - wsr_dg_t0, 1e-6f);
		float gg = fminf(fmaxf((A - 0.6f) / 0.3f, 0.0f), 1.0f);   // smoothstep(0.6, 0.9, A)
		wsr_dg_g = gg * gg * (3.0f - 2.0f * gg);
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (test_T < 0.0001f)
		{
			// WSR: no early-out — every fragment participates in the
			// sort-free composite (see d_wsr_mode doc at top of file).
			if (d_wsr_mode == 0)
			{
				done = true;
				continue;
		...
		float w = alpha * T;
		if (d_wsr_mode) {
			float occ_v = (d_wsr_occ != nullptr) ? d_wsr_occ[collected_id[j]] : 1.0f;
			// Soft transmittance gate: fold the continuous saturation factor
			// into the occ weight (w only) — the coverage product keeps raw α
			// so nothing pops as camera-attached bin planes sweep the scene.
			if (d_wsr_gate_tau > 0.0f && d_wsr_tbin != nullptr)
				occ_v *= wsr_gate_soft(depth, H, W, pix_id);
			// Mean-depth gate (D-gate): smooth relative-depth fade behind the
			// sorted mean depth, blended by saturation strength g.
			if (d_wsr_dgate_margin > 0.0f && d_wsr_dbuf != nullptr) {
				float x = fminf(fmaxf((depth - wsr_dg_t0) * wsr_dg_inv, 0.0f), 1.0f);
				occ_v *= 1.0f - wsr_dg_g * (x * x * (3.0f - 2.0f * x));
			}
			w = alpha * occ_v;
			wsr_den += w;
		}
		...
		case 5: {
			// ================= PROBERES branch (flag 0x1000) =================
			// residual = bilinear(tex, A·uv + t) — probe-mapped shared texture.
			// No 3D hash query, no MLP. probes [N,6] ride in the
			// hash_features_diffuse slot, tex [Ht,Wt,3] in gridrange_diffuse,
			// dims {Ht,Wt} in level_offsets_diffuse. SH base + activation
			// cascade identical to the hash path below.
			if (render_mode & 0x1000) {
				const int gauss_id_p = collected_id[j];
				float sh_color_p[3];
				for (int ch = 0; ch < 3; ch++)
					sh_color_p[ch] = RGB_TO_FLOAT(rgb[gauss_id_p * 3 + ch]);

				// Same residual-skip gates as the hash path.
				const bool skip_res = (d_contrib_thresh > 0.0f && w < d_contrib_thresh)
				                      || (d_count_thresh > 0 && contributor > (uint32_t)d_count_thresh)
				                      || (d_opacity_thresh > 0.0f && alpha < d_opacity_thresh)
				                      || dropout_skip((unsigned int)gauss_id_p);

				float residual_p[3] = {0.0f, 0.0f, 0.0f};
				if (!skip_res && hash_features_diffuse != nullptr && gridrange_diffuse != nullptr
				    && level_offsets_diffuse != nullptr) {
					// uv = ray-splat intersection in surfel σ units; degenerate
					// (low-pass) branch samples the probe center, matching the
					// hash path's xyz = pk convention.
					const float2 uv = (rho3d <= rho2d) ? s : float2{0.0f, 0.0f};
					const float* pr = hash_features_diffuse + gauss_id_p * 6;
					const int th = level_offsets_diffuse[0];
					const int tw = level_offsets_diffuse[1];
				...
				// Activation cascade — identical to the hash-path tail below.
				for (int ch = 0; ch < 3; ch++) {
					if (d_residual_mode == 1) {
						feat[ch] = sh_color_p[ch] + fmaxf(0.0f, residual_p[ch] + d_res_bias);
					} else if (d_residual_mode == 2) {
						feat[ch] = sh_color_p[ch] + residual_p[ch] + d_res_bias;
					} else {
						const float _pre = sh_color_p[ch] + residual_p[ch] + d_res_bias;
						feat[ch] = (_pre > 0.0f) ? _pre : d_lru_slope * _pre;
					}
				}
				break;
			}
			// ============== end PROBERES branch (hash path below) ==============
		...
		// Accumulate hashgrid features
				...
				// WSR composite: track the frontmost fragment (exact per-pixel
				// intersection depth, center-depth fallback for low-pass wins —
				// same key the viewer's ht z-buffer uses).
				if (d_wsr_mode == 2 && depth < wsr_front_depth) {
					wsr_front_depth = depth;
					wsr_front_alpha = alpha;
					wsr_front_w = w;
					wsr_front_id = collected_id[j];
					for (int ch = 0; ch < CHANNELS; ch++)
						wsr_front_feat[ch] = feat[ch];
				}
			...
			if(record_transmittance){
				// WSR distill targets (semantics CHANGED vs the base module, where
				// cover_pixel was a pixel COUNT): cover_pixel = Σα (denominator),
				// trans_avg = Σ(α·T) (numerator) → occ_init = trans_avg / cover_pixel,
				// the visibility-weighted mean transmittance of the surfel. α·T is
				// written explicitly so the dump is correct regardless of d_wsr_mode.
				atomicAdd(&(cover_pixel[collected_id[j]]), alpha);
				atomicAdd(&(trans_avg[collected_id[j]]), alpha * T);
			}
	...
	if (inside)
	{
		...
		if (d_wsr_mode == 2 && wsr_front_id >= 0) {
			// WSR COMPOSITE (ht=1-style): exact frontmost fragment F (depth-
			// argmin, matching the viewer's z-buffer core) composited over the
			// occ-weighted mean of the REST:
			//   out = α_F·c_F + (1−α_F)·(1−P_t)·C̄_t
			//   A   = α_F + (1−α_F)·(1−P_t),   P_t = Π_tail(1−α)
			// The all-fragment accumulators have F removed algebraically.
			// aux: [0..2]=C̄_t, [3]=den_t, [4]=P_t, [5]=α_F, [6]=front gauss id.
			const float aF = wsr_front_alpha;
			const float den_t = fmaxf(wsr_den - wsr_front_w, 0.0f);
			const float P_t = fminf(T / fmaxf(1.0f - aF, 1e-4f), 1.0f);
			const float A_t = 1.0f - P_t;
			const float inv_den_t = (den_t > 1e-8f) ? (1.0f / den_t) : 0.0f;
			const float A_total = aF + (1.0f - aF) * A_t;
			for (int ch = 0; ch < CHANNELS; ch++)
			{
				const float cbar = fmaxf(C[ch] - wsr_front_w * wsr_front_feat[ch], 0.0f) * inv_den_t;
				out_color[ch * H * W + pix_id] =
					aF * wsr_front_feat[ch] + (1.0f - aF) * A_t * cbar;
				if (d_wsr_aux != nullptr) d_wsr_aux[ch * H * W + pix_id] = cbar;
			}
			if (d_wsr_aux != nullptr) {
				d_wsr_aux[3 * H * W + pix_id] = den_t;
				d_wsr_aux[4 * H * W + pix_id] = P_t;
				d_wsr_aux[5 * H * W + pix_id] = aF;
				d_wsr_aux[6 * H * W + pix_id] = __int_as_float(wsr_front_id);
			}
			// Alpha-map convention: final_T = 1 − A so out_others ALPHA = A.
			T = 1.0f - A_total;
#if RENDER_AXUTILITY
			{
				const float s_wsr = (wsr_den > 1e-8f) ? (A_total / wsr_den) : 0.0f;
				D *= s_wsr;
				for (int ch = 0; ch < 3; ch++) N[ch] *= s_wsr;
			}
#endif
		} else if (d_wsr_mode) {
			// WSR composite: out = (1-P)·C̄, C̄ = Σwc/Σw. Same premultiplied-
			// without-bg convention as the sorted path (Python composites bg via
			// the alpha map), so downstream code is unchanged. C̄ and den are
			// stashed in d_wsr_aux for the order-independent backward. Aux D/N
			// get the same (1-P)/den normalization so their alpha-map-relative
			// semantics match the sorted path.
			const float A_wsr = 1.0f - T;
			const float inv_den = (wsr_den > 1e-8f) ? (1.0f / wsr_den) : 0.0f;
			const float s_wsr = A_wsr * inv_den;
			for (int ch = 0; ch < CHANNELS; ch++)
			{
				const float cbar = C[ch] * inv_den;
				out_color[ch * H * W + pix_id] = A_wsr * cbar;
				if (d_wsr_aux != nullptr) d_wsr_aux[ch * H * W + pix_id] = cbar;
			}
			if (d_wsr_aux != nullptr) d_wsr_aux[3 * H * W + pix_id] = wsr_den;
#if RENDER_AXUTILITY
			D *= s_wsr;
			for (int ch = 0; ch < 3; ch++) N[ch] *= s_wsr;
#endif
...
// WSR mode + per-render pointers (occ [N], aux [4*H*W]). Call before every
// forward while WSR is active; call with (0, nullptr, nullptr) to disable.
__global__ void setWsrFwdKernel(int mode, const float* occ, float* aux) {
	d_wsr_mode = mode; d_wsr_occ = occ; d_wsr_aux = aux;
}
void FORWARD::setWsr(int mode, const float* occ, float* aux) {
	h_wsr_mode_fw = mode;
	setWsrFwdKernel<<<1, 1>>>(mode, occ, aux);
}

// WSR transmittance gate: tau ≤ 0 disables. tbin is the per-render
// [bins*H*W] fp32 buffer the pre-pass fills (Python allocates + keeps alive
// through backward). zmin/zmax parameterize the log-depth bin mapping — the
// SAME constants must be fed to the viewer's WGSL uniforms.
__global__ void setWsrGateFwdKernel(float tau, int bins, float lo, float invr, float* tbin) {
	d_wsr_gate_tau = tau; d_wsr_gate_bins = bins;
	d_wsr_gate_lo = lo; d_wsr_gate_invr = invr; d_wsr_tbin = tbin;
}
void FORWARD::setWsrGate(float tau, int bins, float zmin, float zmax, float* tbin) {
	bins = min(max(bins, 1), WSR_GATE_MAX_BINS);
	const float lo = log2f(1.0f + zmin);
	const float invr = 1.0f / fmaxf(log2f(1.0f + zmax) - lo, 1e-6f);
	h_wsr_gate_tau = tau; h_wsr_tbin = tbin;
	setWsrGateFwdKernel<<<1, 1>>>(tau, bins, lo, invr, tbin);
}

// WSR mean-depth gate ("D-gate", ?wsr=3): margin ≤ 0 disables. dbuf is the
// per-render [2*H*W] fp32 buffer the sorted pre-pass fills (D̄, A).
__global__ void setWsrDGateFwdKernel(float margin, float* dbuf) {
	d_wsr_dgate_margin = margin; d_wsr_dbuf = dbuf;
}
void FORWARD::setWsrDGate(float margin, float* dbuf) {
	h_wsr_dgate_margin = margin; h_wsr_dbuf = dbuf;
	setWsrDGateFwdKernel<<<1, 1>>>(margin, dbuf);
}
	...
	int* metric_counts)
{
	...
	// WSR gate pre-pass: fills d_wsr_tbin with per-pixel per-bin exclusive
	// transmittance before the main render reads it. Same grid — the tbin
	// write and read are cleanly ordered by kernel launch order in-stream.
	if (h_wsr_mode_fw != 0 && h_wsr_gate_tau > 0.0f && h_wsr_tbin != nullptr) {
		wsrGatePrepassCUDA<<<grid, block>>>(
			ranges, point_list, beta, W, H,
			means2D, transMats, normal_opacity, shapes, kernel_type, render_mode);
	}
	// WSR mean-depth gate pre-pass (?wsr=3): sorted walk → (D̄, A) per pixel.
	if (h_wsr_mode_fw != 0 && h_wsr_dgate_margin > 0.0f && h_wsr_dbuf != nullptr) {
		wsrDGatePrepassCUDA<<<grid, block>>>(
			ranges, point_list, beta, W, H,
			means2D, transMats, normal_opacity, shapes, kernel_type, render_mode);
	}
