...
namespace BACKWARD
{
	void render(
		...
		float* dL_dtex = nullptr,  // PROBERES: [Ht*Wt*3] texture image grad
	...
	// WSR transmittance gate — mirrors FORWARD::setWsrGate; the backward
	// re-derives each fragment's gate decision from the forward's tbin buffer.
	void setWsrGate(float tau, int bins, float zmin, float zmax, const float* tbin);

	// WSR mean-depth gate — mirrors FORWARD::setWsrDGate.
	void setWsrDGate(float margin, const float* dbuf);

	// WSR (sort-free weighted-sum) mode. Mirrors FORWARD::setWsr; occ_grad is
	// the [N] dL/docc accumulator retrieved by the Python side after backward.
	void setWsr(int mode, const float* occ, float* occ_grad, const float* aux);
