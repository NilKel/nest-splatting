	...
	const bool detach_hash_grad)
{
  ...
  // PROBERES (render_mode & 0x1000): the gridrange_diffuse slot carries the shared
  // texture image [Ht, Wt, 3] and features_diffuse carries probes [N, 6]; allocate
  // the texture gradient (empty otherwise so other modes are untouched).
  torch::Tensor dL_dtex = (render_mode & 0x1000)
      ? torch::zeros_like(gridrange_diffuse)
      : torch::zeros({0}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  (render_mode & 0x1000) ? dL_dtex.contiguous().data<float>() : nullptr,
	  render_mode,
	  shapes_ptr_bw,
	  kernel_type,
  ...
  return std::make_tuple(dL_dfeatures, dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, dL_gradsum, dL_dfeatures_diffuse, dL_dshapes,
                         dL_dmlp_W1, dL_dmlp_W2, dL_dmlp_W3, dL_dtex);
}
...
// WSR (sort-free weighted-sum) mode. Installs mode + raw pointers into BOTH
// TUs' device globals. Tensors must be contiguous fp32 CUDA tensors and stay
// alive through forward+backward (the Python side stashes them on the model).
// Call with mode=0 and empty tensors to disable.
void SetWsrCUDA(int mode, torch::Tensor occ, torch::Tensor occ_grad, torch::Tensor aux) {
    const float* occ_ptr = occ.numel() ? occ.data_ptr<float>() : nullptr;
    float* grad_ptr = occ_grad.numel() ? occ_grad.data_ptr<float>() : nullptr;
    float* aux_ptr = aux.numel() ? aux.data_ptr<float>() : nullptr;
    if (occ.numel()) {
        TORCH_CHECK(occ.is_cuda() && occ.is_contiguous() && occ.dtype() == torch::kFloat32,
                    "wsr occ must be contiguous fp32 CUDA");
    }
    if (occ_grad.numel()) {
        TORCH_CHECK(occ_grad.is_cuda() && occ_grad.is_contiguous() && occ_grad.dtype() == torch::kFloat32,
                    "wsr occ_grad must be contiguous fp32 CUDA");
    }
    if (aux.numel()) {
        TORCH_CHECK(aux.is_cuda() && aux.is_contiguous() && aux.dtype() == torch::kFloat32,
                    "wsr aux must be contiguous fp32 CUDA");
    }
    FORWARD::setWsr(mode, occ_ptr, aux_ptr);
    BACKWARD::setWsr(mode, occ_ptr, grad_ptr, aux_ptr);
}

// WSR transmittance gate. tbin = per-render [bins*H*W] fp32 scratch the
// forward's pre-pass fills and both passes' gate checks read; must stay alive
// through backward (Python stashes it next to aux). tau <= 0 disables.
void SetWsrGateCUDA(float tau, int bins, float zmin, float zmax, torch::Tensor tbin) {
    float* tbin_ptr = tbin.numel() ? tbin.data_ptr<float>() : nullptr;
    if (tbin.numel()) {
        TORCH_CHECK(tbin.is_cuda() && tbin.is_contiguous() && tbin.dtype() == torch::kFloat32,
                    "wsr gate tbin must be contiguous fp32 CUDA");
    }
    FORWARD::setWsrGate(tau, bins, zmin, zmax, tbin_ptr);
    BACKWARD::setWsrGate(tau, bins, zmin, zmax, tbin_ptr);
}

// WSR mean-depth gate (?wsr=3). dbuf = per-render [2*H*W] fp32 buffer the
// sorted pre-pass fills (D̄, A); keep alive through backward. margin<=0 off.
void SetWsrDGateCUDA(float margin, torch::Tensor dbuf) {
    float* dbuf_ptr = dbuf.numel() ? dbuf.data_ptr<float>() : nullptr;
    if (dbuf.numel()) {
        TORCH_CHECK(dbuf.is_cuda() && dbuf.is_contiguous() && dbuf.dtype() == torch::kFloat32,
                    "wsr dgate dbuf must be contiguous fp32 CUDA");
    }
    FORWARD::setWsrDGate(margin, dbuf_ptr);
    BACKWARD::setWsrDGate(margin, dbuf_ptr);
}
