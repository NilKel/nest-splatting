	...
	const torch::Tensor& viewdirs_enc)
{
  ...
  // MLP gradient buffers for 3D_direct_fused mode (render_mode=5, bias-free, WMMA-padded)
  // Architecture: 48D input (40D + 1.0 bias + 7 zero pad) -> 32 hidden -> 32 hidden -> 16 output (first 3 = RGB)
  torch::Tensor dL_dmlp_W1 = torch::zeros({32, 48}, means3D.options());  // Layer 1 weights (WMMA-padded from 32x41)
  torch::Tensor dL_dmlp_W2 = torch::zeros({32, 32}, means3D.options());   // Layer 2 weights
  torch::Tensor dL_dmlp_W3 = torch::zeros({16, 32}, means3D.options());   // Layer 3 weights (WMMA-padded from 3x32)
...
void SetMlpWeightsCUDA(
    const torch::Tensor& W1,      // [32, 48] - Layer 1 weights (WMMA-padded, implicit bias at col 40)
    const torch::Tensor& W2,      // [32, 32] - Layer 2 weights
    const torch::Tensor& W3,      // [OUT_DIM, 32] - Layer 3 weights (3 for RGB)
