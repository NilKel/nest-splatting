...
renderCUDAsurfelBackward(
	...
	float* __restrict__ dL_dmlp_W1 = nullptr,    // [32 * 48] = W1_SIZE
	float* __restrict__ dL_dmlp_W2 = nullptr,    // [32 * 32] = W2_SIZE
	float* __restrict__ dL_dmlp_W3 = nullptr,    // [16 * 32] = W3_SIZE
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	__shared__ float tile_dL_dW1[W1_SIZE];   // 32*48 = 6KB
	__shared__ float tile_dL_dW2[32 * 32];   // 4KB
	__shared__ float tile_dL_dW3[W3_SIZE];   // 16*32 = 2KB

	// Shared memory cache for MLP weights (FP16, ~8KB total)
	// Loaded once per tile, used for both MLP forward recomputation and per-pixel backward
	__shared__ __half smem_mlp_W1[W1_SIZE];   // 32*48 = 3,072 bytes
	__shared__ __half smem_mlp_W2[32 * 32];   // 2,048 bytes
	__shared__ __half smem_mlp_W3[W3_SIZE];   // 16*32 = 1,024 bytes
