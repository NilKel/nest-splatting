...
// L1: W1[32×48] (input[40]=1.0 for implicit bias, positions 41-47 = 0 padding)
// L2: W2[32×32] (no bias)
// L3: W3_rgb[16×32] (only first 3 rows used for RGB, rows 3-15 = 0 padding)
__device__ __half* d_mlp_W1 = nullptr;      // 48D input → 32D hidden1 (W1_SIZE×2 = 3KB)
...
__device__ __half* d_mlp_W3_rgb = nullptr;  // For 3D_direct_fused: 32D → 16 RGB-padded (W3_SIZE×2 = 1KB)
...
float* d_dL_dW1 = nullptr;   // [32, 48] = W1_SIZE = 1536 floats
float* d_dL_dW2 = nullptr;   // [32, 32] = W2_SIZE = 1024 floats
float* d_dL_dW3 = nullptr;   // [16, 32] = W3_SIZE = 512 floats (or [48, 32] for SH)
...
// WMMA-padded: IN_DIM=48 (positions 41-47 = 0 padding), OUT_DIM=3 (only 3 outputs used for RGB)
// Input must have input[40] = 1.0f for implicit L1 bias (last column before padding)
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	// Shared memory cache for MLP weights (FP16, WMMA-padded, ~6.1KB total)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		case 5: {
			...
			// 4. Build MLP input: [gauss(20) | hash(4) | view(16) | 1.0 | zeros(7)] = 48D (WMMA-padded)
			float mlp_input[TC_INPUT_DIM];
			// Zero-initialize the padding (positions 41-47)
			for (int i = ORIG_INPUT_DIM; i < TC_INPUT_DIM; i++) mlp_input[i] = 0.0f;
			...
			mlp_forward_fused<TC_INPUT_DIM, TC_HIDDEN_DIM, ORIG_OUTPUT_DIM>(mlp_input, feat, h1, h2, true,
				smem_mlp_W1, smem_mlp_W2, smem_mlp_W3);
	...
	bool is_sh_mode)
{
	...
	if (!mlp_weights_allocated) {
		cudaMalloc(&h_mlp_W1, W1_SIZE * sizeof(__half));    // W1[32×48] WMMA-padded
		cudaMalloc(&h_mlp_W2, W2_SIZE * sizeof(__half));    // W2[32×32]
		cudaMalloc(&h_mlp_W3_sh, 32 * 48 * sizeof(__half)); // W3_sh[48×32] (SH mode, unchanged)
		cudaMalloc(&h_mlp_W3_rgb, W3_SIZE * sizeof(__half)); // W3_rgb[16×32] WMMA-padded
