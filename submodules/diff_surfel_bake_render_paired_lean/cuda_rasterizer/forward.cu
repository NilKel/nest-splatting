	...
	const float* __restrict__ conic_uv = nullptr)
{
	...
	// res_3d_paired: per-Gauss textured flag + EWA conic (2D screen conic
	// (a, b, c) + opacity in .w). Nullptr signals from Python collapse both
	// to sentinels → pure 2DGS single-cascade (byte-identical to plain lean).
	// FP16 pack for the conic to stay within the 48KB static-shared cap.
	__shared__ bool   collected_is_textured[BLOCK_SIZE];
	__shared__ __half collected_ewa_conic_abc[BLOCK_SIZE * 3];   // (a, b, c) — Mahalanobis coefs
	__shared__ __half collected_ewa_opa[BLOCK_SIZE];             // opacity (con_o.w)
#else
	#ifdef LEAN_T2
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			// res_3d_paired: stage is_textured + EWA conic. When ewa_conic is
			// null (non-paired scenes), stamp is_textured=true so the
			// untextured branch is skipped (byte-identical to non-paired path).
			if (ewa_conic != nullptr && is_textured != nullptr) {
				collected_is_textured[t] = is_textured[coll_id];
				float4 con_o = ewa_conic[coll_id];
				collected_ewa_conic_abc[t * 3 + 0] = __float2half(con_o.x);  // a
				collected_ewa_conic_abc[t * 3 + 1] = __float2half(con_o.y);  // b
				collected_ewa_conic_abc[t * 3 + 2] = __float2half(con_o.z);  // c
				collected_ewa_opa[t] = __float2half(con_o.w);
			} else {
				collected_is_textured[t] = true;
				collected_ewa_opa[t] = __float2half(0.0f);
			}
#else
	#ifdef LEAN_T2
		...
		{
			// ============================================================
			// res_3d_paired: UNTEXTURED EWA 3D-ellipsoid splat.
			// Handled BEFORE the textured LEAN_CONIC path. Untex Gauss
			// use screen-space conic falloff + shared SV/SH base color.
			// No atlas fetch (zero-rect skip-texture invariant). Signed
			// per-Gauss color under mode 2 → post-blend LeakyReLU applied
			// by Python (matches diff_surfel_mixed_3d training forward).
			// FP16 conic decode matches CLAUDE.md's stated FP16 tolerance
			// for a per-Gauss iso value in [-3σ, 3σ].
			// ============================================================
#ifdef LEAN_CONIC
			if (ewa_conic != nullptr && !collected_is_textured[j]) {
				const float opa_e = __half2float(collected_ewa_opa[j]);
				if (opa_e <= 0.0f) continue;   // culled in preprocess
				const float a_c = __half2float(collected_ewa_conic_abc[j * 3 + 0]);
				const float b_c = __half2float(collected_ewa_conic_abc[j * 3 + 1]);
				const float c_c = __half2float(collected_ewa_conic_abc[j * 3 + 2]);
				const float2 xy_e = collected_xy[j];
				const float dxe = xy_e.x - pixf.x;
				const float dye = xy_e.y - pixf.y;
				// FMA-fused Mahalanobis: me = a·dx² + 2b·dx·dy + c·dy² —
				// matches base's forward.cu:951 exactly.
				const float me = fmaf(fmaf(2.0f * b_c, dye, a_c * dxe), dxe,
				                      c_c * dye * dye);
				// Untextured kernel: --kernel2 override (d_untex_kernel) if set,
				// else the run's --kernel. Mirrors training-forward branch.
				const int ut_kt = (d_untex_kernel >= 0) ? d_untex_kernel : kernel_type;
				float G_e;
				if (ut_kt == 1 || ut_kt == 4) {
					const float k_sq = (ut_kt == 4) ? 9.0f : 1.0f;
					if (me >= k_sq + 1e-6f) continue;
					const float shape_e = __half2float(collected_shapes_h[j]);
					const float base_e = fmaxf(0.0f, 1.0f - me / k_sq);
					G_e = powf(base_e, shape_e);
				} else {
					const float power = -0.5f * me;
					if (power > 0.0f) continue;
					G_e = expf(power);
				}
				const float alpha_e = fminf(0.99f, opa_e * G_e);
				if (alpha_e < 1.0f / 255.0f) continue;
				const float test_T_e = T * (1.0f - alpha_e);
				...
				// Activation dispatch mirrors the textured path.
				if (d_residual_mode == 2) {
					for (int ch = 0; ch < 3; ch++)
						feat_e[ch] = feat_e[ch] + d_res_bias;
				} else {
					for (int ch = 0; ch < 3; ch++)
						feat_e[ch] = fmaxf(0.0f, feat_e[ch] + d_res_bias);
				}
				for (int ch = 0; ch < 3; ch++)
					C[ch] += feat_e[ch] * w_e;
				T = test_T_e;
			...
			// LEAN: mixed_3d EWA branch REMOVED (now handled above for paired mode).
			...
			// Activation dispatch: mode 0 → per-Gauss ReLU (3D_SH_res).
			// Mode 2 → signed passthrough (res_3d_paired / mixed_3d), Python
			// applies post-blend LeakyReLU.
			if (d_residual_mode == 2) {
				for (int ch = 0; ch < 3; ch++)
