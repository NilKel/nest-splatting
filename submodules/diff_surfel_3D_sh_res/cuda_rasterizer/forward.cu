...
// ============================================================================
// Bias-free MLP weights: [hash(4) | bias(1) | pad(11)] = 16D input
...
// Input: [hash(4) | bias(1) | pad(11)] = 16D. input[4] = 1.0f for implicit L1 bias.
...
__device__ void mlp_forward_fused(
	const float* input,       // [IN_DIM] = 16D (hash(4) + bias(1) + pad(11))
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		case 5: {
			 ...
			 *   4. Build MLP input: [hash(4) | bias(1) | pad(11)] = 16D
			...
			// 3. Build MLP input: [hash(4) | bias(1) | pad(11)] = 16D
			...
			mlp_input[4] = 1.0f;  // Implicit bias
			// Positions 5-15 are zero (padding for WMMA alignment)
