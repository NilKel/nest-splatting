...
namespace BACKWARD
{
	...
	// Per-pixel Z-cull proxy occluder (backward mirror). Same signature +
	// semantics as FORWARD::setOccluderDepth. SetOccluderDepthCUDA in
	// rasterize_points.cu fires both together so fwd and bwd read the same
	// map (identical contributor set).
