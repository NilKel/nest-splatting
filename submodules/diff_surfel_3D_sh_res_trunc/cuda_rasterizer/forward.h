...
namespace FORWARD
{
	...
	// TRUNC clone: post-blend truncation exit threshold (default 1e-4 = off).
	void setExitT(float val);
	// TRUNC clone: runtime T-crossing level for the median-depth aux output.
	void setTCrossing(float val);
	// TRUNC clone: cluster-stats analysis mode (median slot repurposed).
	void setClusterMode(int mode, float alpha_min, float gap, float void_mass);
	// TRUNC clone: macro-gap truncation for --gap_noise training.
	void setGapTrunc(int mode, float thresh, float margin, float void_mass, float arm_T, float T_lo, float sat_T);
