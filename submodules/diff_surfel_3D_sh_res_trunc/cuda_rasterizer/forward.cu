...
// --- TRUNC CLONE ---
// Truncation exit threshold (--trunc training): once the POST-blend
// transmittance T drops below this value, the walk stops. Unlike the stock
// pre-blend `test_T < 1e-4` exit (which SKIPS the crossing fragment — fine
// when the skipped weight is ≤1e-4, fatal at 0.5 where it would pin T just
// above the threshold forever), the truncation exit fires AFTER the crossing
// fragment has blended, so an opaque terminator can drive T → ~0 and kill the
// Python-side `(1−rend_alpha)·noise` term. At the default 1e-4 the stock exit
// always fires first → byte-identical to the base rasterizer. The backward is
// automatically consistent: it only walks contributors the forward recorded
// (last_contributor) and starts from the recorded final_Ts.
__device__ float d_exit_T = 0.0001f;

// --- TRUNC CLONE ---
// Runtime T-crossing level for the median-depth aux output (default = the
// compile-time T_CROSSING, i.e. 0.5 unless the build overrode it). Lets
// analysis scripts sample the per-ray inverse transmittance profile: render
// N times with set_T_crossing(tau) for a ladder of tau values and read the
// median slot as depth(T=tau). No effect on color/alpha/blending.
__device__ float d_t_crossing = T_CROSSING;

// --- TRUNC CLONE: per-ray cluster statistics (analysis only) ---
// set_cluster_mode: 0 = off (median slot behaves normally);
//   1 = the median-depth aux slot carries the per-pixel count of DISJOINT
//       surfel clusters along the ray (1D single-linkage: a new cluster
//       starts when the next significant fragment is more than
//       d_cluster_gap behind the deepest fragment of the current cluster);
//   2 = the slot carries the depth gap between cluster 1 and cluster 2
//       (0 if fewer than 2 clusters) — the first-void size.
// Fragments join the stream when alpha >= d_cluster_alpha_min (ignores
// kernel-fringe dust). Fragments arrive in tile-sorted (center-depth) order;
// per-pixel intersection depths can be locally out of order, so the cluster
// tracks its DEEPEST member (cl_prev = max) and gaps are measured from that.
__device__ int   d_cluster_mode = 0;
__device__ float d_cluster_alpha_min = 0.05f;
__device__ float d_cluster_gap = 0.05f;
// Modes 3/4: MASS-aware clustering — a split requires a gap longer than
// d_cluster_gap containing less than d_cluster_void_mass total opacity.
// Distinguishes "two faces with an empty void between" (splits) from "one
// fuzzy thick face" (continuous mass merges). 3 = count, 4 = gap12.
__device__ float d_cluster_void_mass = 0.05f;

// --- TRUNC CLONE: macro-gap truncation (--gap_noise training) ---
// When active (d_gap_trunc != 0), the forward walk ENDS IN THE VOID: the
// first significant fragment lying more than d_gap_trunc_thresh behind the
// front manifold's deepest tracked member stops the walk BEFORE blending.
// The remaining transmittance is recorded in final_Ts as usual, so the
// Python-side (1−rend_alpha)·noise composite fills the void — same audited
// truncation semantics as d_exit_T (backward walks recorded contributors).
// The wall is GEOMETRY-anchored: opacity cannot move it, and post-reset
// alphas fall below d_gap_trunc_alpha_min so the wall goes dormant instead
// of shocking. Margin (NGS erosion-buffer analog): FAINT fragments within
// d_gap_trunc_margin of the manifold's deepest member still EXTEND the
// manifold — the wall never presses on a manifold's own trailing fringe.
__device__ int   d_gap_trunc = 0;
__device__ float d_gap_trunc_thresh = 0.2f;     // min void LENGTH
__device__ float d_gap_trunc_margin = 0.05f;    // cluster-extend zone
__device__ float d_gap_trunc_void_mass = 0.1f;  // max opacity mass inside a void
__device__ float d_gap_trunc_T_lo = 0.02f;      // min leaked T worth pressuring
// Arming threshold: the wall may only fire when the transmittance at the
// candidate fragment is BELOW this value, i.e. the front manifold has
// already absorbed at least (1 - arm_T) of the ray. Prevents floater
// amplification: a lone low-alpha floater leaves T high, so it can never
// truncate the real scene behind it (and never earn noise-kill reward).
// 1.0 = guard off.
__device__ float d_gap_trunc_arm_T = 1.0f;
// Saturation gate (deferred commit). <= 0 → legacy immediate truncation.
// > 0 → the wall only COMMITS on rays whose full (untruncated) march ends
// with final T < sat_T, i.e. rays that fully saturate anyway. Rays that end
// semi-transparent (background blends, floaters over bg) render as if no
// wall existed, so the leftover-T noise never pressures them.
__device__ float d_gap_trunc_sat_T = 0.0f;
	...
	float* __restrict__ out_others)
{
	...
	// TRUNC clone cluster-stats locals (see d_cluster_mode).
	int cl_count = 0;
	float cl_prev = 0.0f, cl_gap12 = 0.0f, cl_pend = 0.0f;
	// TRUNC clone gap-truncation locals (see d_gap_trunc). v2 mass automaton:
	// gt_prev = deepest member of the current cluster; gt_pend = opacity mass
	// accumulated beyond the margin (a void must stay under void_mass).
	int gt_seen = 0;
	float gt_prev = 0.0f, gt_pend = 0.0f;
	// Saturation-gated deferred commit (d_gap_trunc_sat_T > 0): the wall's
	// pre-void state is snapshotted when it fires and committed at write-out
	// ONLY if the full ray saturates. gt_maxi is used by the surfel kernel.
	int gt_fired = 0; uint32_t gt_lc = 0; int gt_maxi = -1;
	float gt_T = 1.0f, gt_C[CHANNELS] = { 0 };
	float gt_N[3] = {0}, gt_D = 0.0f, gt_M1 = 0.0f, gt_M2 = 0.0f;
	float gt_dist = 0.0f, gt_med_d = 0.0f, gt_med_c = -1.0f, gt_maxd = 0.0f;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (alpha < 1.0f / 255.0f)
				continue;
			// TRUNC clone: macro-gap truncation, v2 MASS-AND-DISTANCE automaton
			// (see d_gap_trunc). A void must be LONGER than thresh AND contain
			// less than void_mass total opacity; the wall fires only when the
			// front has absorbed enough (T < arm_T; floater guard) and the leak
			// is visible (T > T_lo; explicit self-annealing). Dense material in
			// the gap merges the clusters — continuous media never truncate.
			if (d_gap_trunc != 0 && !gt_fired) {
				if (!gt_seen) {
					gt_seen = 1;
					gt_prev = depth;
				} else {
					float _gap = depth - gt_prev;
					if (_gap <= d_gap_trunc_margin) {
						if (depth > gt_prev) gt_prev = depth;
						gt_pend = 0.0f;
					} else if (_gap > d_gap_trunc_thresh
							&& gt_pend < d_gap_trunc_void_mass
							&& T < d_gap_trunc_arm_T && T > d_gap_trunc_T_lo) {
						// Legacy immediate wall when sat gate is off.
						if (d_gap_trunc_sat_T <= 0.0f) { done = true; continue; }
						// Deferred commit: snapshot pre-void state, keep
						// marching to learn whether the ray saturates.
						gt_fired = 1;
						gt_T = T; gt_lc = last_contributor;
						for (int ch = 0; ch < CHANNELS; ch++) gt_C[ch] = C[ch];
						for (int ch = 0; ch < 3; ch++) gt_N[ch] = N[ch];
						gt_D = D; gt_M1 = M1; gt_M2 = M2; gt_dist = distortion;
						gt_med_d = median_depth; gt_med_c = median_contributor;
						gt_maxd = max_depth;
					} else {
						gt_pend += alpha;
						if (gt_pend >= d_gap_trunc_void_mass) {
							gt_prev = depth;   // gap occupied: merge into cluster
							gt_pend = 0.0f;
						}
					}
				}
			...
			if (test_T < 0.0001f && d_cluster_mode == 0)
			{
				done = true;
				continue;
			}
...
#endif
			// TRUNC clone: per-ray disjoint-cluster tracking (analysis only,
			// see d_cluster_mode). Cluster tracks its deepest member so the
			// locally out-of-order intersection depths of tile-sorted
			// fragments don't split a contiguous cluster.
			if ((d_cluster_mode == 1 || d_cluster_mode == 2)
					&& alpha >= d_cluster_alpha_min) {
				if (cl_count == 0) { cl_count = 1; cl_prev = depth; }
				else if (depth - cl_prev > d_cluster_gap) {
					cl_count++;
					if (cl_count == 2) cl_gap12 = depth - cl_prev;
					cl_prev = depth;
				} else if (depth > cl_prev) {
					cl_prev = depth;
				}
			} else if (d_cluster_mode >= 3) {
				// mass-aware: only SIGNIFICANT frags (alpha >= alpha_min) can
				// seed/extend/split clusters. Sub-threshold frags never spawn
				// clusters; their mass can only BRIDGE a void (>= void_mass of
				// faint material between clusters => continuous, extend).
				if (alpha >= d_cluster_alpha_min) {
					if (cl_count == 0) { cl_count = 1; cl_prev = depth; }
					else {
						float _g = depth - cl_prev;
						if (_g > d_cluster_gap && cl_pend < d_cluster_void_mass) {
							cl_count++;
							if (cl_count == 2) cl_gap12 = _g;
						}
						if (depth > cl_prev) cl_prev = depth;
					}
					cl_pend = 0.0f;
				} else if (cl_count > 0) {
					cl_pend += alpha;
					if (cl_pend >= d_cluster_void_mass) {
						if (depth > cl_prev) cl_prev = depth;
						cl_pend = 0.0f;
					}
				}
...
#ifndef DISABLE_EARLY_EXIT
			// TRUNC: post-blend truncation stop (see d_exit_T). Dead code at
			// the default threshold (stock pre-blend exit fires first).
			if (T < d_exit_T && d_cluster_mode == 0)
				done = true;
#endif
		}
	}
	...
	if (inside)
	{
		// TRUNC clone: commit the gap-wall snapshot ONLY if the full ray
		// saturated (final T < sat_T). Otherwise render as if no wall fired.
		// Backward stays consistent automatically: it walks from the
		// committed n_contrib with the committed final_T/M1/M2.
		if (gt_fired && T < d_gap_trunc_sat_T) {
			T = gt_T; last_contributor = gt_lc;
			for (int ch = 0; ch < CHANNELS; ch++) C[ch] = gt_C[ch];
			for (int ch = 0; ch < 3; ch++) N[ch] = gt_N[ch];
			D = gt_D; M1 = gt_M1; M2 = gt_M2; distortion = gt_dist;
			median_depth = gt_med_d; median_contributor = gt_med_c;
			max_depth = gt_maxd;
		}
		final_T[pix_id] = T;
		...
		// TRUNC clone: in cluster-stats mode the median slot carries the
		// cluster count (mode 1) or the first-void gap (mode 2) instead.
		out_others[pix_id + MIDDEPTH_OFFSET * H * W] =
			(d_cluster_mode == 1 || d_cluster_mode == 3) ? (float)cl_count :
			(d_cluster_mode == 2 || d_cluster_mode == 4) ? cl_gap12 : median_depth;
	...
	int* __restrict__ metric_counts = nullptr)
{
...
#ifndef DISABLE_EARLY_EXIT
				if (test_T < 0.0001f && d_cluster_mode == 0) { done = true; active = false; break; }
#endif
...
#ifndef DISABLE_EARLY_EXIT
				// TRUNC: post-blend truncation stop (see d_exit_T). Safe here:
				// the phase-1 staging for later j is gated on `active = !done`,
				// and control flow stays block-uniform (no break).
				if (T < d_exit_T && d_cluster_mode == 0)
					done = true;
#endif
			}
		...
		if (d_gap_trunc != 0 && !gt_fired) {
			if (!gt_seen) {
				...
				if (_gap <= d_gap_trunc_margin) {
					...
					gt_D = D; gt_M1 = M1; gt_M2 = M2; gt_dist = distortion;
					gt_med_d = median_depth; gt_med_c = median_contributor;
					gt_maxd = max_depth; gt_maxi = max_idx;
				} else {
					gt_pend += alpha;
					if (gt_pend >= d_gap_trunc_void_mass) {
						gt_prev = depth;   // gap occupied: merge into cluster
	...
	if (inside)
	{
		// TRUNC clone: commit the gap-wall snapshot ONLY if the full ray
		// saturated (final T < sat_T). Otherwise render as if no wall fired.
		// pos/vis/overdraw/w_square/beta_sum/render_number are NOT restored
		// (debug + reg-loss channels; regs are off in gap-noise runs).
		if (gt_fired && T < d_gap_trunc_sat_T) {
			T = gt_T; last_contributor = gt_lc;
			for (int ch = 0; ch < CHANNELS; ch++) C[ch] = gt_C[ch];
			for (int ch = 0; ch < 3; ch++) N[ch] = gt_N[ch];
			D = gt_D; M1 = gt_M1; M2 = gt_M2; distortion = gt_dist;
			median_depth = gt_med_d; median_contributor = gt_med_c;
			max_depth = gt_maxd; max_idx = gt_maxi;
		}
		final_T[pix_id] = T;
		n_contrib[pix_id] = last_contributor;
...
__global__ void setExitTKernel(float val) { d_exit_T = val; }
void FORWARD::setExitT(float val) {
	setExitTKernel<<<1, 1>>>(val);
	cudaDeviceSynchronize();
}

__global__ void setTCrossingKernel(float val) { d_t_crossing = val; }
void FORWARD::setTCrossing(float val) {
	setTCrossingKernel<<<1, 1>>>(val);
	cudaDeviceSynchronize();
}

__global__ void setGapTruncKernel(int mode, float thresh, float margin, float void_mass, float arm_T, float T_lo, float sat_T) {
	d_gap_trunc = mode;
	d_gap_trunc_thresh = thresh;
	d_gap_trunc_margin = margin;
	d_gap_trunc_void_mass = void_mass;
	d_gap_trunc_arm_T = arm_T;
	d_gap_trunc_T_lo = T_lo;
	d_gap_trunc_sat_T = sat_T;
}
void FORWARD::setGapTrunc(int mode, float thresh, float margin, float void_mass, float arm_T, float T_lo, float sat_T) {
	setGapTruncKernel<<<1, 1>>>(mode, thresh, margin, void_mass, arm_T, T_lo, sat_T);
	cudaDeviceSynchronize();
}

__global__ void setClusterModeKernel(int mode, float alpha_min, float gap, float void_mass) {
	d_cluster_mode = mode;
	d_cluster_alpha_min = alpha_min;
	d_cluster_gap = gap;
	d_cluster_void_mass = void_mass;
}
void FORWARD::setClusterMode(int mode, float alpha_min, float gap, float void_mass) {
	setClusterModeKernel<<<1, 1>>>(mode, alpha_min, gap, void_mass);
	cudaDeviceSynchronize();
}
