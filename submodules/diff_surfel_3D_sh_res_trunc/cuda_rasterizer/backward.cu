...
// Per-pixel Z-cull proxy occluder — backward mirror of forward.cu's copy. The
// backward render kernel checks this against the per-fragment ray-splat depth
// and skips fragments behind the mesh (both standard + MODE-5 collab-GEMM
// paths), matching the forward's `continue`. `d_occluder_depth == nullptr`
// (default) → the check is a no-op → backward is byte-identical to before.
// Host-side setter pair: FORWARD::setOccluderDepth + BACKWARD::setOccluderDepth
// (called together from SetOccluderDepthCUDA in rasterize_points.cu).
	...
	const float* __restrict__ dc_features = nullptr)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (participates) {
					...
					if (p.z != 0.0f) {
						...
						// Per-pixel Z-cull (matches forward): if this fragment sits
						// behind the proxy occluder mesh at this pixel, mark it as
						// non-participating so all grad accumulation is skipped. The
						// alpha-cascade recurrence state (T, accum_rec, last_color…)
						// stays untouched, mirroring forward which never blended it.
						if (d_occluder_depth != nullptr && pix.x < d_occluder_W && pix.y < d_occluder_H) {
							float occ = d_occluder_depth[pix.y * d_occluder_W + pix.x];
							if (isfinite(occ) && c_d > occ) {
								participates = false;
							}
						}

						if (participates && c_d >= near_n) {
		...
		float c_d = (rho3d <= rho2d) ? (s.x * Tw.x + s.y * Tw.y) + Tw.z : Tw.z;
		if (c_d < near_n) continue;
		// Per-pixel Z-cull (matches forward): skip fragments behind the proxy
		// occluder so backward's contributor set matches forward exactly. The
		// alpha-cascade recurrence state (T, accum_rec, last_color…) skips
		// this fragment too, mirroring the forward's `continue`.
		if (d_occluder_depth != nullptr && pix.x < d_occluder_W && pix.y < d_occluder_H) {
			float occ = d_occluder_depth[pix.y * d_occluder_W + pix.x];
			if (isfinite(occ) && c_d > occ) continue;
		}
		float4 nor_o = collected_normal_opacity[j];
		float normal[3] = {nor_o.x, nor_o.y, nor_o.z};  // Already normalized in preprocessing
		float opa = nor_o.w;
...
// Per-pixel Z-cull proxy occluder (backward). Mirror of the forward-side
// setter — SetOccluderDepthCUDA in rasterize_points.cu fires both together.
__global__ void setOccluderDepthBwKernel(const float* ptr, int W, int H) {
	d_occluder_depth = ptr;
	d_occluder_W = W;
	d_occluder_H = H;
}
void BACKWARD::setOccluderDepth(const float* ptr, int W, int H) {
	setOccluderDepthBwKernel<<<1, 1>>>(ptr, W, H);
}
void BACKWARD::clearOccluderDepth() {
	setOccluderDepthBwKernel<<<1, 1>>>(nullptr, 0, 0);
}
