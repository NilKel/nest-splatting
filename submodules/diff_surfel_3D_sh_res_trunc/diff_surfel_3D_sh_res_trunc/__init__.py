...
def set_exit_T(val):
    """TRUNC clone: post-blend truncation exit threshold. The forward walk
    stops once transmittance T (AFTER blending the crossing fragment) drops
    below `val`. Default 1e-4 = byte-identical to the base rasterizer; 0.5 =
    ray terminates at the median crossing, remaining T carries the Python-side
    noise composite. Backward needs no mirror (it walks recorded contributors)."""
    _C.set_exit_t(float(val))

def set_T_crossing(val):
    """TRUNC clone: runtime T-level for the median-depth aux slot. Render with
    a ladder of values (0.9 ... 0.05) and read render_pkg's median depth each
    time to sample the per-ray inverse transmittance profile depth(T=tau).
    Default 0.5 (the compile-time T_CROSSING). No effect on color/alpha."""
    _C.set_t_crossing(float(val))

def set_cluster_mode(mode, alpha_min=0.05, gap=0.05, void_mass=0.05):
    """TRUNC clone cluster-stats analysis (median-depth slot repurposed):
    mode 0 = off (normal median depth); mode 1 = per-pixel count of disjoint
    surfel clusters along the ray (new cluster when the next fragment with
    alpha >= alpha_min is more than `gap` scene units behind the current
    cluster's deepest member); mode 2 = depth gap between cluster 1 and
    cluster 2 (0 if <2 clusters). Analysis only — remember to reset mode 0."""
    _C.set_cluster_mode(int(mode), float(alpha_min), float(gap), float(void_mass))

def set_gap_trunc(mode, thresh=0.2, margin=0.05, void_mass=0.1, arm_T=0.6, T_lo=0.02,
                  sat_T=0.05):
    """TRUNC clone macro-gap truncation (--gap_noise training). mode 1: the
    forward walk stops BEFORE blending the first fragment with
    alpha >= alpha_min lying more than `thresh` behind the front manifold's
    deepest tracked member; the remaining transmittance (rend_alpha channel)
    carries the Python noise composite — noise fills the void. Faint
    fragments within `margin` of the manifold extend it (the wall never
    presses on a manifold's trailing fringe). mode 0: off (byte-identical).
    Geometry-anchored: opacity changes cannot move the wall.
    sat_T > 0 (default 0.05): SATURATION GATE — the wall only commits on rays
    whose full untruncated march ends with final T < sat_T (rays that fully
    saturate anyway). Rays blending with the background (semi-transparent
    floaters over bg, silhouettes) render as if no wall existed and take no
    noise pressure. sat_T <= 0 restores the legacy immediate wall."""
    _C.set_gap_trunc(int(mode), float(thresh), float(margin), float(void_mass),
                     float(arm_T), float(T_lo), float(sat_T))
