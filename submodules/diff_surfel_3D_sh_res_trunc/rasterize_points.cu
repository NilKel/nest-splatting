...
// TRUNC clone: post-blend truncation exit threshold. Forward-only — the
// backward walks only the contributors the forward recorded, so it needs no
// mirror of the threshold.
void SetExitTCUDA(float val) {
    FORWARD::setExitT(val);
}

// TRUNC clone: runtime T-crossing for the median-depth slot (analysis only —
// forward-only; backward reads the recorded median_contributor).
void SetTCrossingCUDA(float val) {
    FORWARD::setTCrossing(val);
}

void SetClusterModeCUDA(int mode, float alpha_min, float gap, float void_mass) {
    FORWARD::setClusterMode(mode, alpha_min, gap, void_mass);
}

void SetGapTruncCUDA(int mode, float thresh, float margin, float void_mass, float arm_T, float T_lo, float sat_T) {
    FORWARD::setGapTrunc(mode, thresh, margin, void_mass, arm_T, T_lo, sat_T);
}
