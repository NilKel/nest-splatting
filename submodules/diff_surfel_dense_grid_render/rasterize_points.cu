/*
 * Dense grid rendering submodule — PyTorch/CUDA binding (forward-only).
 */
	...
	const float grid_vmax)
{
	...
	if (P != 0)
	{
		...
		const __half* dense_grid_ptr = (dense_grid.numel() > 0)
			? (const __half*)dense_grid.contiguous().data_ptr<at::Half>() : nullptr;
