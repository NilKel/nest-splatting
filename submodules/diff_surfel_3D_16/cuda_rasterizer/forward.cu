...
// ============================================================================
// Bias-free MLP weights in global device memory
// L1: W1[32×41] (last column = implicit bias via input[40]=1.0)
// L2: W2[32×32] (no bias)
// L3: W3_rgb[3×32] (no bias)
__device__ __half* d_mlp_W1 = nullptr;      // 41D input → 32D hidden1 (32×41×2 = 2.6KB)
__device__ __half* d_mlp_W2 = nullptr;      // 32D hidden1 → 32D hidden2 (2KB)
__device__ __half* d_mlp_W3_sh = nullptr;   // For 3D_fused: 32D → 48 SH (3KB)
__device__ __half* d_mlp_W3_rgb = nullptr;  // For 3D_direct_fused: 32D → 3 RGB (192B)
...
float* d_dL_dW1 = nullptr;   // [32, 41] = 1312 floats
float* d_dL_dW2 = nullptr;   // [32, 32]
float* d_dL_dW3 = nullptr;   // [3, 32] or [48, 32]
...
// MLP forward pass (bias-free): 41D input → 32D hidden1 → 32D hidden2 → OUT_DIM output
// Input must have input[40] = 1.0f for implicit L1 bias (last column of W1)
...
__device__ void mlp_forward_fused(
	const float* input,       // [IN_DIM] = 41D (24 features + 16 view + 1 bias pad)
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	if (render_mode == 5 && mlp_W1 != nullptr) {
		...
		for (int idx = tid; idx < 3 * 32; idx += BLOCK_SIZE)
			smem_mlp_W3[idx] = mlp_W3_rgb[idx];
		block.sync();
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		case 5: {
			...
			if (has_cached_view_enc) {
				...
				// Fallback: compute per-intersection (legacy behavior, won't match trained models)
				glm::vec3 cp = *cam_pos;
			...
			// 5. Run MLP → RGB (with sigmoid activation, weights from shared memory)
			float h1[32], h2[32];  // Hidden layer activations
			mlp_forward_fused<41, 32, 3>(mlp_input, feat, h1, h2, true,
	...
	bool is_sh_mode)
{
	...
	if (!mlp_weights_allocated) {
		cudaMalloc(&h_mlp_W1, 41 * 32 * sizeof(__half));   // W1[32×41] with implicit bias
		cudaMalloc(&h_mlp_W2, 32 * 32 * sizeof(__half));
		cudaMalloc(&h_mlp_W3_sh, 32 * 48 * sizeof(__half));
		cudaMalloc(&h_mlp_W3_rgb, 32 * 3 * sizeof(__half));
	...
	const int block = 256;
	float2half_kernel<<<(32*41 + block-1)/block, block>>>(W1, h_mlp_W1, 32*41);
	float2half_kernel<<<(32*32 + block-1)/block, block>>>(W2, h_mlp_W2, 32*32);

	if (is_sh_mode) {
		float2half_kernel<<<(48*32 + block-1)/block, block>>>(W3, h_mlp_W3_sh, 48*32);
	} else {
		float2half_kernel<<<(3*32 + block-1)/block, block>>>(W3, h_mlp_W3_rgb, 3*32);
	}
}
