...
renderCUDAsurfelBackward(
	...
	float* __restrict__ dL_dmlp_W1 = nullptr,    // [32 * 41] = 1312
	float* __restrict__ dL_dmlp_W2 = nullptr,    // [32 * 32] = 1024
	float* __restrict__ dL_dmlp_W3 = nullptr,    // [3 * 32] = 96
	...
	const __half* __restrict__ mlp_W3_rgb_ptr = nullptr,
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	__shared__ float tile_dL_dW1[32 * 41];   // 5.25KB (32×41, last column = implicit bias grad)
	__shared__ float tile_dL_dW2[32 * 32];   // 4KB
	__shared__ float tile_dL_dW3[3 * 32];    // 384B

	// Shared memory cache for MLP weights (FP16, ~4.9KB total)
	// Loaded once per tile, used for both MLP forward recomputation and per-pixel backward
	__shared__ __half smem_mlp_W1[32 * 41];   // 2,624 bytes
	__shared__ __half smem_mlp_W2[32 * 32];   // 2,048 bytes
	__shared__ __half smem_mlp_W3[3 * 32];    // 192 bytes
	...
	if (inside && viewdirs_enc != nullptr && (render_mode & 0xFF) == 5) {
		has_cached_view_enc = true;
		for (int i = 0; i < 16; i++)
			cached_view_enc[i] = viewdirs_enc[pix_id * 16 + i];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if ((render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				// --- Phase 1: Forward recomputation + sigmoid backward → dL_dz3 ---
				...
				if (participates) {
					...
					// 3. Load pre-encoded view direction (16D) - cached before Gaussian loop
					if (has_cached_view_enc) {
						for (int i = 0; i < 16; i++) my_input[24 + i] = cached_view_enc[i];
					} else {
						glm::vec3 cp = *cam_pos;
						...
						encode_view_direction_bw(view_dir, my_input + 24);
					}
					my_input[40] = 1.0f;  // Implicit bias padding

					// 4. Recompute MLP forward (from shared memory weights)
					MlpWeights smem_mlp = {smem_mlp_W1, smem_mlp_W2, smem_mlp_W3};
					mlp_forward_inline(my_input, my_output, my_h1_post, my_h2_post, true, smem_mlp);

					// 5. Sigmoid backward → dL_dz3
					#pragma unroll
					for (int o = 0; o < 3; o++) {
						float sig = my_output[o];
						my_dL_dz3[o] = (dL_dpixel[o] * w) * sig * (1.0f - sig);
					}
				}

				// Note: no __syncthreads needed here - ballot check above provides sync
				extern __shared__ __half dynamic_smem[];
				collaborative_gemm_layer3(my_dL_dz3, my_h2_post, tile_dL_dW3, dynamic_smem);
				...
				if (participates) {
					...
					for (int h = 0; h < 32; h++) {
						...
						#pragma unroll
						for (int o = 0; o < 3; o++) {
							dL_dh2 += my_dL_dz3[o] * __half2float(smem_mlp_W3[o * 32 + h]);
						}
				...
				collaborative_gemm_layer2(my_dL_dz2, my_h1_post, tile_dL_dW2, dynamic_smem);
				...
				collaborative_gemm_layer1(my_dL_dz1, my_input, tile_dL_dW1, dynamic_smem);

				// ======== Phase 4: dL_dinput + feature & geometry gradients ========
				if (participates) {
					// dL_dinput = W1^T @ dL_dz1 (only first 40 elements, skip bias column)
					float my_dL_dinput[40];
					#pragma unroll
					for (int i = 0; i < 40; i++) {
						float sum = 0;
						#pragma unroll
						for (int h = 0; h < 32; h++) {
							sum += my_dL_dz1[h] * __half2float(smem_mlp_W1[h * 41 + i]);
						}
						my_dL_dinput[i] = sum;
					}
					...
					// Using FULL geometry gradient computation from working 2DGS backward
					if (rho3d <= rho2d) {
						// Update gradients w.r.t. covariance of Gaussian 3x3 (T)
						float2 dL_ds = {
							dL_dG * -G * s.x + dL_dz * Tw.x,
						...
						// Add hash->xyz gradient contribution to dL_ds
						dL_ds.x += dL_duv.x;
						dL_ds.y += dL_duv.y;
						...
						// Update gradients w.r.t. 3D covariance (3x3 matrix) - FULL Tu, Tv, Tw
						atomicAdd(&dL_dtransMat[global_id * 9 + 0],  dL_dTu.x);
						atomicAdd(&dL_dtransMat[global_id * 9 + 1],  dL_dTu.y);
						atomicAdd(&dL_dtransMat[global_id * 9 + 2],  dL_dTu.z);
						atomicAdd(&dL_dtransMat[global_id * 9 + 3],  dL_dTv.x);
						...
						// 2D fallback case - Update gradients w.r.t. center of Gaussian 2D mean position
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 5: {
				...
				// 3. Load pre-encoded view direction (16D) - matches forward pass
				float view_enc[16];
				if (viewdirs_enc != nullptr) {
					for (int i = 0; i < 16; i++) {
						view_enc[i] = viewdirs_enc[pix_id * 16 + i];
					}
				} else {
					glm::vec3 cp = *cam_pos;
					float3 view_dir = {xyz.x - cp.x, xyz.y - cp.y, xyz.z - cp.z};
				...
				// 4. Build MLP input: [gauss(20) | hash(4) | view(16) | 1.0] = 41D
				float mlp_input[41];
				...
				mlp_input[40] = 1.0f;  // Implicit bias padding

				// 5. Recompute MLP forward (from shared memory weights)
				float h1_post[32], h2_post[32];
				float mlp_output[3];
				MlpWeights smem_mlp2 = {smem_mlp_W1, smem_mlp_W2, smem_mlp_W3};
				mlp_forward_inline(mlp_input, mlp_output, h1_post, h2_post, true, smem_mlp2);
				...
				// 7. MLP backward (legacy per-pixel atomic path, bias-free)
				float dL_dinput[40];
				mlp_backward<41, 32, 3>(
	...
	const float* viewdirs_enc)
{
	...
	// Dynamic shared memory for mode 5 (3D_direct_fused) collaborative GEMM (FP16 buffers)
	// Layer 1 requires: 256*32*2 + 256*41*2 = 16KB + 21KB = 37KB (FP16, single batch)
	size_t smem_size = 0;
	...
	if (render_mode == 5 && dL_dmlp_W1 != nullptr) {
		...
		if (!force_disable) {
			// Try to enable collaborative GEMM with ~37KB FP16 shared memory (single batch of 256)
			// Layer 1 needs 256*(32+41)*2 = 37376 bytes
	...
	renderCUDAsurfelBackward<3, 0> <<<grid, block, smem_size>>>(
			...
			dL_dmlp_W1, dL_dmlp_W2, dL_dmlp_W3,
			mlp_W1_ptr, mlp_W2_ptr, mlp_W3_rgb_ptr, viewdirs_enc);
