	...
	const torch::Tensor& viewdirs_enc)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		viewdirs_enc.numel() > 0 ? viewdirs_enc.contiguous().data<float>() : nullptr);
  }
	...
	const torch::Tensor& viewdirs_enc)
{
  ...
  // MLP gradient buffers for 3D_SH_TC mode (render_mode=5, bias-free, WMMA-padded)
  // Architecture: 32D input (24D + 1.0 bias + 7 zero pad) -> 32 hidden -> 32 hidden -> 48 SH output
  torch::Tensor dL_dmlp_W1 = torch::zeros({32, 32}, means3D.options());  // Layer 1 weights
  torch::Tensor dL_dmlp_W2 = torch::zeros({32, 32}, means3D.options());   // Layer 2 weights
  torch::Tensor dL_dmlp_W3 = torch::zeros({48, 32}, means3D.options());   // Layer 3 weights (48 SH coefs)
  ...
  if(P != 0)
  {
	  ...
	  detach_hash_grad,
	  // MLP gradient buffers for 3D_direct_fused mode (bias-free)
	  ...
	  // Pre-encoded view directions for 3D_direct_fused
	  viewdirs_enc.numel() > 0 ? viewdirs_enc.contiguous().data<float>() : nullptr);
  }
...
// Copy MLP weights from PyTorch tensors to CUDA global memory (bias-free)
// Call this before each render call if weights have changed
void SetMlpWeightsCUDA(
    const torch::Tensor& W1,      // [32, 32] - Layer 1 weights (WMMA-padded, implicit bias at col 24)
    const torch::Tensor& W2,      // [32, 32] - Layer 2 weights
    const torch::Tensor& W3,      // [48, 32] - Layer 3 weights (48 SH coefficients)
    const bool is_sh_mode         // true for SH mode (always true in this submodule)
) {
    CHECK_INPUT(W1);
    CHECK_INPUT(W2);
    CHECK_INPUT(W3);
...
// ============================================================================
// BAKING: MLP + HASH → SH COEFFICIENTS AT UV GRID POINTS
// ============================================================================

torch::Tensor BakeGaussiansCUDA(
    const torch::Tensor& centers,           // [N, 3]
    const torch::Tensor& quats,             // [N, 4]
    const torch::Tensor& scales,            // [N, 2]
    const torch::Tensor& gauss_features,    // [N, 20]
    const torch::Tensor& hash_features,     // Hash table FP16
    const torch::Tensor& level_offsets,     // Hash level offsets (int32)
    const float voxel_min,
    const float voxel_max,
    const float l_scale,
    const int Base,
    const bool align_corners,
    const int interp,
    const bool if_contract,
    const int active_hashgrid_levels,
    const torch::Tensor& appearance_levels, // [N] uint32
    const int grid_size,                    // 8
    const float uv_extent)                  // UV range: [-uv_extent, +uv_extent]
{
    CHECK_INPUT(centers);
    CHECK_INPUT(quats);
    CHECK_INPUT(scales);
    CHECK_INPUT(gauss_features);
    CHECK_INPUT(hash_features);
    CHECK_INPUT(level_offsets);
    CHECK_INPUT(appearance_levels);

    int N = centers.size(0);
    auto output_sh = torch::zeros({N, grid_size * grid_size, 48},
                                   centers.options());
    ...
    FORWARD::bake(
        ...
        scales.contiguous().data_ptr<float>(),
        gauss_features.contiguous().data_ptr<float>(),
        (const __half*)hash_features.contiguous().data_ptr<at::Half>(),
        level_offsets.contiguous().data_ptr<int>(),
        voxel_min, voxel_max, l_scale, Base,
        align_corners, interp, if_contract,
        active_hashgrid_levels,
        (const uint32_t*)appearance_levels.contiguous().data_ptr<int>(),
        output_sh.data_ptr<float>());

    return output_sh;  // [N, grid_size*grid_size, 48]
}
