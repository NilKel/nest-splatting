...
// ============================================================================
// BAKING KERNEL: Sample MLP at UV grid points per Gaussian → 48D SH
// 1 thread per (Gaussian, grid_point). No alpha blending, no opacity.
// ============================================================================
__global__ void bakeSHKernel(
    int N,
    int grid_size,
    float uv_extent,                                 // UV range: [-uv_extent, +uv_extent]
    const float* __restrict__ centers,         // [N, 3]
    const float* __restrict__ quats,           // [N, 4] (w,x,y,z)
    const float* __restrict__ scales,          // [N, 2]
    const float* __restrict__ gauss_features,  // [N, 20]
    const __half* __restrict__ hash_features,
    const int* __restrict__ level_offsets,
    float voxel_min, float voxel_max,
    float l_scale, int Base,
    bool align_corners, int interp, bool if_contract,
    int active_hashgrid_levels,
    const uint32_t* __restrict__ appearance_levels,  // [N]
    float* __restrict__ output_sh                    // [N, grid_size*grid_size, 48]
) {
    // Shared memory for MLP weights (same as forward kernel)
    __shared__ __half smem_W1[W1_SIZE];   // 32*32 = 2 KB
    __shared__ __half smem_W2[W2_SIZE];   // 32*32 = 2 KB
    __shared__ __half smem_W3[W3_SIZE];   // 48*32 = 3 KB

    // Cooperative weight loading (once per block)
    int tid = threadIdx.x;
    for (int i = tid; i < W1_SIZE; i += blockDim.x)
        smem_W1[i] = d_mlp_W1[i];
    for (int i = tid; i < W2_SIZE; i += blockDim.x)
        smem_W2[i] = d_mlp_W2[i];
    for (int i = tid; i < W3_SIZE; i += blockDim.x)
        smem_W3[i] = d_mlp_W3_sh[i];
    __syncthreads();

    int global_tid = blockIdx.x * blockDim.x + threadIdx.x;
    int total_pts = grid_size * grid_size;
    int gauss_id = global_tid / total_pts;
    int pt_id = global_tid % total_pts;
    if (gauss_id >= N) return;

    int vi = pt_id / grid_size, ui = pt_id % grid_size;
    // Texel-center convention: texel i at position (i + 0.5) * step - extent
    // With grid_size=8, uv_extent=4: step=1.0, positions = -3.5, -2.5, ..., 3.5
    // This matches the render kernel's lookup: tex_u = s.x + 3.5
    float step = 2.0f * uv_extent / (float)grid_size;
    float u = ((float)ui + 0.5f) * step - uv_extent;
    float v = ((float)vi + 0.5f) * step - uv_extent;

    // Quaternion → rotation matrix
    float qw = quats[gauss_id*4+0], qx = quats[gauss_id*4+1];
    float qy = quats[gauss_id*4+2], qz = quats[gauss_id*4+3];
    float norm = rsqrtf(qw*qw + qx*qx + qy*qy + qz*qz + 1e-8f);
    qw *= norm; qx *= norm; qy *= norm; qz *= norm;

    // R columns (local tangent frame)
    ...
    float sx = scales[gauss_id * 2 + 0];
    float sy = scales[gauss_id * 2 + 1];

    // Local → world: xyz = center + u*sx*R_col0 + v*sy*R_col1
    float3 xyz;
    xyz.x = centers[gauss_id*3+0] + u*sx*r00 + v*sy*r01;
    xyz.y = centers[gauss_id*3+1] + u*sx*r10 + v*sy*r11;
    xyz.z = centers[gauss_id*3+2] + u*sx*r20 + v*sy*r21;

    // Query hash features (4D)
    float hash_feat[4] = {0};
    uint32_t ap_level = appearance_levels[gauss_id];
    query_feature<false, 4, 4>(hash_feat, xyz, voxel_min, voxel_max,
                                (int*)level_offsets, ap_level, hash_features,
                                active_hashgrid_levels, l_scale, Base,
                                align_corners, interp, if_contract, false);

    // Build MLP input: [gauss(20) | hash(4) | 1.0 bias | zeros(7)] = 32D
    float mlp_input[TC_INPUT_DIM];
    for (int i = 0; i < TC_INPUT_DIM; i++) mlp_input[i] = 0.0f;
    for (int i = 0; i < 20; i++)
        mlp_input[i] = gauss_features[gauss_id * 20 + i];
    for (int i = 0; i < 4; i++)
        mlp_input[20 + i] = hash_feat[i];
    mlp_input[24] = 1.0f;  // Implicit L1 bias

    // MLP forward → 48D SH (identity activation, no sigmoid)
    float sh_out[TC_OUTPUT_DIM];
    float h1[TC_HIDDEN_DIM], h2[TC_HIDDEN_DIM];
    mlp_forward_fused<TC_INPUT_DIM, TC_HIDDEN_DIM, TC_OUTPUT_DIM>(
        mlp_input, sh_out, h1, h2, false,
        smem_W1, smem_W2, smem_W3);

    // Write output
    int out_base = (gauss_id * total_pts + pt_id) * TC_OUTPUT_DIM;
    for (int i = 0; i < TC_OUTPUT_DIM; i++)
        output_sh[out_base + i] = sh_out[i];
}


void FORWARD::bake(
    int N, int grid_size, float uv_extent,
    const float* centers, const float* quats, const float* scales,
    const float* gauss_features,
    const __half* hash_features, const int* level_offsets,
    float voxel_min, float voxel_max,
    float l_scale, int Base,
    bool align_corners, int interp, bool if_contract,
    int active_hashgrid_levels,
    const uint32_t* appearance_levels,
    float* output_sh)
{
    int total_threads = N * grid_size * grid_size;
    int block_size = 256;
    int grid = (total_threads + block_size - 1) / block_size;

    bakeSHKernel<<<grid, block_size>>>(
        N, grid_size, uv_extent,
