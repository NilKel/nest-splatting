...
def set_frontmost_first(v):
    """GES-paper frontmost-first promotion (harden stage). True = each pixel's
    FRONTMOST surfel — by exact per-pixel ray-disc intersection depth — is
    promoted to blend FIRST; all other surfels keep the tile-sort order
    ("the blending order of the other surfels is not adjusted", GES §render).
    Color-only (aux depth/normal/dist channels keep the original order).
    Patches forward + backward device globals. False = off (byte-identical)."""
    _C.set_frontmost_first(1 if v else 0)
...
def set_tile_depth_sort(val):
    """Set first-intersection (tile-depth) sort toggle. True = key each (Gauss,tile)
    on the ray-disc intersection depth at the tile-center ray (correct front-to-back
    order for tilted surfels); False = default center-depth sort. Requires rect-AABB
    coverage (renderer forces aabb_mode 0 when enabled)."""
    _C.set_tile_depth_sort(bool(val))
