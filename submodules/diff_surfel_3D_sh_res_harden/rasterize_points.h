...
// GES frontmost-first promotion (harden stage): 1 = promote each pixel's
// frontmost surfel (exact ray-disc intersection depth) to blend first,
// color-only (fwd + bwd). 0 = off (default, byte-identical).
void SetFrontmostFirstCUDA(int v);
...
// on the ray-disc intersection depth at the tile-center ray; false = default.
void SetTileDepthSortCUDA(bool val);
