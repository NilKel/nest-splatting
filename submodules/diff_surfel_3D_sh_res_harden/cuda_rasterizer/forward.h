...
namespace FORWARD
{
	...
	void render(
		...
		// GES frontmost-first promotion (d_frontmost_first): per-pixel outputs
		// consumed by the backward. fm_pos [H*W] = 1-based contributor index of
		// the promoted frontmost surfel (0 = none); c_nf [3*H*W] = the full
		// non-frontmost composite color (no bg). Both nullptr => not stored.
		uint32_t* fm_pos = nullptr,
		float* c_nf = nullptr);
	...
	// GES frontmost-first promotion: 1 = promote each pixel's frontmost surfel
	// (by exact ray-disc intersection depth) to blend first (color-only). 0 = off.
	void setFrontmostFirst(int v);
