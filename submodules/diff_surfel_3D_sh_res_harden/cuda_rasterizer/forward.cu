...
// GES-paper frontmost-first promotion (harden stage). When 1, each pixel's
// FRONTMOST surfel — by exact per-pixel ray-disc intersection depth — is
// promoted to blend FIRST; all other surfels keep the tile-sort order (GES
// "the blending order of the other surfels is not adjusted"). Implemented as
// a post-hoc algebraic correction (no reordering of the loop):
//   C_promoted = C_base − αF·C_snap + αF·(1−T_snap)·cF
// where (C_snap, T_snap) are the accumulators the moment F was about to blend
// in the ORIGINAL order, cF/αF are F's color/alpha. Identical to blending F
// first: everyone after F already carries the (1−αF) factor in the original
// order; only the pre-F prefix (scaled by 1−αF) and F itself (T→1) change.
// COLOR-ONLY: aux channels (depth/normal/dist) keep the original order.
__device__ int d_frontmost_first = 0;
...
renderCUDAsurfelForward(
	...
	// GES frontmost-first promotion outputs (see d_frontmost_first).
	uint32_t* __restrict__ fm_pos = nullptr,
	float* __restrict__ c_nf = nullptr)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	// GES frontmost-first promotion state (d_frontmost_first). The render runs as
	// a literal GES-style multi-pass over the tile range (ges_rasterization_surfel):
	//   pass 0: FULL-RANGE scan (no transmittance early-exit) for the per-pixel
	//           frontmost surfel by exact ray-disc intersection depth;
	//   pass 1: F ONLY — the unchanged loop body blends F FIRST (T = 1);
	//   pass 2: everyone EXCEPT F, in tile order ("the blending order of the
	//           other surfels is not adjusted"), early exit on the promoted T.
	// Off (d_frontmost_first == 0) → single pass 2 with fm_contrib = 0: the
	// original loop, byte-identical.
	float fm_depth = 1e30f;
	uint32_t fm_contrib = 0;   // 1-based `contributor` index of frontmost; 0 = none
	...
	// GES frontmost-first: multi-pass over the range (see fm state above). The
	// pass count is driven by the block-uniform device global so the collective
	// batch fetches + syncs stay uniform across the block.
	for (int fm_pass = (d_frontmost_first != 0 ? 0 : 2); fm_pass < 3; fm_pass++)
	{
	// Re-init per-pass walk state (T/C/aux accumulate ACROSS passes 1→2).
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// GES frontmost-first pass gating: pass 1 visits ONLY F (it blends
			// first, T = 1, via this unchanged body); pass 2 skips F (already
			// blended). fm_contrib == 0 makes both gates no-ops.
			if (fm_pass == 1 && contributor != fm_contrib)
				continue;
			if (fm_pass == 2 && fm_contrib > 0 && contributor == fm_contrib)
				continue;
		...
		// GES frontmost-first pass 0: FULL-RANGE frontmost scan — track the min
		// exact intersection depth over ALL covering candidates (no transmittance
		// early-exit; mirrors GES ges_rasterization_surfel pass 1), then stop:
		// no blending, no side effects in this pass.
		if (fm_pass == 0) {
			if (depth < fm_depth) {
				fm_depth = depth;
				fm_contrib = contributor;
			}
			continue;
		}
		...
		// Accumulate hashgrid features
			...
			// GES frontmost-first: pass 1 (the F-blend) must NOT publish F into
			// last_contributor — the backward's phase-1 exemption processes F
			// outside the last_contributor gate. If it did, and pass 2 then
			// blended NOTHING (near-opaque F: test_T trips on the first pass-2
			// candidate), n_contrib would claim [0, fm_contrib) as blended and
			// backward phase 0 would divide T by (1-0.99) per PHANTOM contributor
			// in front of F -> T overflows to inf -> NaN gradients. (The exact
			// 18-19.5k harden NaN.)
			if (fm_pass != 1)
				last_contributor = contributor;
	...
	}  // fm_pass (GES frontmost-first multi-pass)
	...
	if (inside)
	{
		...
		// GES frontmost-first: the promoted blend already happened literally
		// (pass 1 blended F first, pass 2 the rest) — nothing to correct. Store
		// F's 1-based contributor index for the backward's two-phase walk.
		// (c_nf is unused by the literal multi-pass backward; kept in the
		// signature to avoid plumbing churn.)
		if (d_frontmost_first != 0 && fm_pos != nullptr)
			fm_pos[pix_id] = fm_contrib;
...
// GES frontmost-first promotion toggle (see d_frontmost_first).
__global__ void setFrontmostFirstFwdKernel(int v) { d_frontmost_first = v; }
void FORWARD::setFrontmostFirst(int v) {
	setFrontmostFirstFwdKernel<<<1, 1>>>(v);
}
	...
	float* c_nf)
{
	...
	renderCUDAsurfelForward<3, 0> <<<grid, block>>>(
		...
		metric_map, metric_counts, fm_pos, c_nf);
