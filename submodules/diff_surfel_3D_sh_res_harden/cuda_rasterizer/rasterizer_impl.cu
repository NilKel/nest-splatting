...
// Host-side first-intersection (tile-depth) sort toggle (set via
// SetTileDepthSortCUDA). When true, the standard sort keys each (Gauss,tile)
// instance on the ray-disc intersection depth at that tile's CENTER ray
// (duplicateKeysWithTileDepth) instead of the Gaussian-center depth
// (duplicateWithKeys) — correct front-to-back order for tilted surfels.
// Requires rect-AABB coverage (renderer forces aabb_mode 0 when on) so the
// emitted (Gauss,tile) count matches preprocess tiles_touched.
bool g_tile_depth_sort = false;
	...
	if (radii[idx] > 0)
	{
		...
		for (int y = rect_min.y; y < rect_max.y; y++)
		{
			for (int x = rect_min.x; x < rect_max.x; x++)
			{
				...
				float3 l = pixy * Tw - Tv;
				float3 p = cross(k, l);
				// Ray-disc first-intersection depth at the tile-center ray.
				// Degenerate (p.z==0): fall back to the Gaussian-center depth so
				// `off` stays aligned with the preprocess-computed offsets (must
				// still emit exactly one key per touched tile — do NOT continue).
				float tile_depth;
				if (p.z == 0.0f) {
					tile_depth = depths[idx];
				} else {
					float2 s = {p.x / p.z, p.y / p.z};
					tile_depth = (s.x * Tw.x + s.y * Tw.y) + Tw.z;
				}
...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char*& chunk, size_t N)
{
	...
	// GES frontmost-first promotion buffers (forward writes, backward reads)
	obtain(chunk, img.fm_pos, N, 128);
	obtain(chunk, img.c_nf, N * 3, 128);
	return img;
}
	...
	int* metric_counts)
{
	...
	} else if (g_tile_depth_sort) {
		// First-intersection sort: key = tile_id|(ray-disc intersection depth at
		// the tile-center ray). Rect-AABB only (renderer forces aabb_mode 0), so
		// the emit count matches preprocess tiles_touched. Full radix sort over
		// [0, 32+bit) orders within-tile front-to-back by intersection depth.
		duplicateKeysWithTileDepth << <(P + 255) / 256, 256 >> > (
			P,
			geomState.means2D,
			geomState.depths,
			geomState.transMat,
	...
	CHECK_CUDA(FORWARD::render(
		...
		metric_map,
		metric_counts,
		imgState.fm_pos,   // GES frontmost-first: per-pixel frontmost contributor index
		imgState.c_nf      // GES frontmost-first: non-frontmost composite (for backward)
		), debug)
	...
	float* dL_dmlp_W3)
{
	...
	CHECK_CUDA(BACKWARD::render(
		...
		((render_mode & 0xFF) == 6) ? colors_precomp : nullptr,
		imgState.fm_pos,   // GES frontmost-first: per-pixel frontmost contributor index
		imgState.c_nf      // GES frontmost-first: non-frontmost composite
		), debug)
