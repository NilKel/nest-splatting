...
namespace BACKWARD
{
	void render(
		...
		// GES frontmost-first promotion (forward-stored): per-pixel 1-based
		// contributor index of the promoted frontmost (0 = none) + non-frontmost
		// composite color [3*H*W]. nullptr => promotion off.
		const uint32_t* fm_pos = nullptr,
		const float* c_nf = nullptr);
