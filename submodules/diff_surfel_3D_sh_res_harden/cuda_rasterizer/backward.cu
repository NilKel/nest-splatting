...
// GES frontmost-first promotion (mirror of forward.cu d_frontmost_first).
// The reverse walk skips the promoted frontmost surfel F (no T division / no
// accum_rec fold) and processes it at its slot with T=1 + accum_rec = c_nf.
__device__ int d_frontmost_first_bw = 0;
...
renderCUDAsurfelBackward(
	...
	// GES frontmost-first promotion (forward-stored; see d_frontmost_first_bw)
	const uint32_t* __restrict__ fm_pos = nullptr,
	const float* __restrict__ c_nf = nullptr)
{
	...
	// GES frontmost-first promotion (d_frontmost_first_bw): the forward blended
	// the per-pixel frontmost surfel F (1-based contributor index fm_contrib,
	// found by a FULL-RANGE intersection-depth scan) FIRST; the others kept the
	// tile order. Backward of that promoted chain = the standard reverse walk
	// run in TWO PHASES over the range (GES ges_rasterization_surfel pattern):
	//   phase 0: every contributor EXCEPT F (skip F: no T division, no folds —
	//            the un-divided (1−αF) factor applies the promotion scaling to
	//            everything in front of F automatically);
	//   phase 1: F ONLY — the UNCHANGED per-contributor code runs for F at the
	//            end of the walk, where T/(1−αF)=1 and the walk-final accum_rec
	//            (+ aux recurrences) are exactly the full non-F composites.
	// F may lie beyond last_contributor (the forward always blends F, even past
	// the pass-2 termination) — phase 1 therefore skips the last_contributor
	// gate, mirroring GES's `contributor != minGeoIdx` exemption.
	const uint32_t fm_contrib = (d_frontmost_first_bw != 0 && inside && fm_pos != nullptr)
	                            ? fm_pos[pix_id] : 0;
	...
	// GES frontmost-first: TWO-PHASE walk (phase 0 = all non-F, phase 1 = F only,
	// via the unchanged per-contributor code — see fm_contrib comment above).
	// Phase count driven by the block-uniform device global so batch fetches and
	// syncs stay uniform; threads without a frontmost skip everything in phase 1.
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		// (fm_phase 1 routes through the scalar branch and F may exceed
		// tile_max_last_contrib — gate the skip to phase 0.)
		if (fm_phase == 0 && (render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
		...
		// (fm_phase 1 — the GES frontmost-first F-sweep — always routes through the
		// scalar branch below; F's MLP backward runs scalar there.)
		// ============================================================================
		if (fm_phase == 0 && (render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				// GES frontmost-first: F is deferred to the phase-1 scalar sweep
				// (blends FIRST in the promoted chain → processed LAST in the walk).
				if (fm_contrib > 0 && current_contributor == (int)fm_contrib - 1)
					participates = false;
						...
						if (kernel_type == 5) {
							...
							if (kernel_type == 1 || kernel_type == 4) {
								if (beta_wins && base > 1e-7f) {
									dG_factor = -shape_val * alpha_beta / (fmaxf(base, 1e-3f) * k_sq);  // edge-grad clamp: base^(shape-1) diverges for shape<1 (flat-top); frontmost-first carries it at full T=1 weight -> NaN. Gradient-only; forward unchanged.
								} else {
									dG_factor = 0.0f;
								}
							} else if (kernel_type == 3) {
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			contributor--;
			if (fm_phase == 1) {
				// GES frontmost-first phase 1: F ONLY — no last_contributor gate
				// (the forward always blends F, even past the pass-2 termination).
				if (fm_contrib == 0 || contributor != fm_contrib - 1) {
					if (contributor > toDo) { done = true; }
					continue;
				}
			} else {
			...
			// GES frontmost-first: F blends FIRST in the promoted chain → it is
			// processed LAST in the reverse walk (phase 1); skip it here.
			if (fm_contrib > 0 && contributor == fm_contrib - 1) continue;
			}
	...
	}  // fm_phase (GES frontmost-first two-phase walk)
...
// GES frontmost-first promotion toggle (mirror of FORWARD::setFrontmostFirst).
__global__ void setFrontmostFirstBwKernel(int v) { d_frontmost_first_bw = v; }
void BACKWARD::setFrontmostFirst(int v) {
	setFrontmostFirstBwKernel<<<1, 1>>>(v);
}
