...
// GES frontmost-first promotion (harden stage): promote each pixel's frontmost
// surfel (by exact ray-disc intersection depth) to blend first, color-only.
void SetFrontmostFirstCUDA(int v) {
    FORWARD::setFrontmostFirst(v);
    BACKWARD::setFrontmostFirst(v);
}
...
// Defined in rasterizer_impl.cu
extern bool g_tile_depth_sort;

void SetTileDepthSortCUDA(bool val) {
    g_tile_depth_sort = val;
}
