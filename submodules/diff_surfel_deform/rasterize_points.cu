...
// `--deform`: stash the per-Gauss CANONICAL xyz pointer so the kernel samples the
// hashgrid in canonical space. Empty tensor ⇒ nullptr ⇒ byte-identical to
// 3D_SH_res. The caller MUST keep the tensor alive (and unchanged) through the
// matching forward AND backward (pass pc.get_xyz.detach() — it shares storage with
// the persistent _xyz Parameter).
void SetCanonicalXyzCUDA(torch::Tensor canonical_xyz) {
    const float* ptr = (canonical_xyz.numel() > 0) ? canonical_xyz.contiguous().data_ptr<float>() : nullptr;
    FORWARD::setCanonicalXyz(ptr);
    BACKWARD::setCanonicalXyz(ptr);
}
