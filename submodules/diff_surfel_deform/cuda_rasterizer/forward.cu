...
// `--deform`: when non-null, the in-kernel hash-query point is reconstructed from
// this per-Gauss CANONICAL xyz (3 floats/Gauss) instead of the deformed surfel
// center, so the hashgrid is sampled in canonical space (sharp, time-consistent
// textures) while geometry/SV use the deformed means. Null ⇒ byte-identical to
// 3D_SH_res. `collected_pk` is used ONLY for the hash-xyz reconstruction, so
// overwriting its load is sufficient — no use-site changes.
__device__ const float* d_canonical_xyz = nullptr;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			if(homotrans != nullptr){
				...
				// --deform: hash queried at CANONICAL center when d_canonical_xyz is set
				// (collected_pk feeds ONLY the hash-xyz recon; geometry uses Tu/Tv/Tw).
				if (d_canonical_xyz != nullptr)
					collected_pk[block.thread_rank()] = {d_canonical_xyz[3 * coll_id + 0], d_canonical_xyz[3 * coll_id + 1], d_canonical_xyz[3 * coll_id + 2]};
				else
...
// `--deform`: set the per-Gauss canonical-xyz pointer (null disables canonical hash).
__global__ void setCanonicalXyzFwdKernel(const float* p) { d_canonical_xyz = p; }
void FORWARD::setCanonicalXyz(const float* ptr) {
	setCanonicalXyzFwdKernel<<<1, 1>>>(ptr);
}
