...
// `--deform`: canonical-xyz pointer (mirrors forward.cu). When set, the hash-xyz is
// reconstructed from canonical centers AND the pk-position gradient term
// (acc_dL_dhomoMat[6..8] += dL_dxyz) is dropped — canonical positions are detached
// on the deform path (geometry still optimizes them via the deformed means).
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			if(homotrans != nullptr){
				...
				// --deform: hash reconstructed at CANONICAL center (mirrors forward).
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (participates) {
					...
					if (homotrans != nullptr && active_hashgrid_levels > 0) {
						...
						// --deform: drop the center-from-hash term (canonical hash uses the
						// detached canonical center; dL_du/dL_dv tangent terms above stay).
						if (d_canonical_xyz == nullptr) {
...
// `--deform`: set the per-Gauss canonical-xyz pointer (mirrors forward).
__global__ void setCanonicalXyzBwKernel(const float* p) { d_canonical_xyz = p; }
void BACKWARD::setCanonicalXyz(const float* ptr) {
	setCanonicalXyzBwKernel<<<1, 1>>>(ptr);
}
