...
def set_canonical_xyz(canonical_xyz=None):
    """`--deform`: point the in-kernel hash query at per-Gauss CANONICAL xyz so the
    hashgrid is sampled in canonical space (sharp, time-consistent textures) while
    geometry/SV use the deformed means passed to the rasterizer.

    Call right BEFORE the forward each iteration; the device pointer persists through
    the matching backward, so the tensor MUST stay alive (and its storage unchanged)
    until backward completes. Pass `pc.get_xyz.detach()` (shares storage with the
    persistent _xyz Parameter). Pass None / empty to disable (byte-identical to
    3D_SH_res)."""
    if canonical_xyz is None or canonical_xyz.numel() == 0:
        _C.set_canonical_xyz(torch.empty(0, device="cuda"))
    else:
        _C.set_canonical_xyz(canonical_xyz.contiguous())
