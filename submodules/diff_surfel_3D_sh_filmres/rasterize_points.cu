	...
	const torch::Tensor& film_beta)
{
  ...
  if(P != 0)
  {
	  ...
	  // FiLM modulation pointers (nullptr => plain 3D_SH_res). Held in named locals so
	  // the contiguous copy of any non-contiguous slice outlives the kernel launch.
	...
	const torch::Tensor& film_beta)
{
  ...
  // FiLM gradients (3D_SH_filmres). beta width = film_beta.size(1) (= hash_dim used,
  // <= 16); default 1 when FiLM unused so the tensor is allocatable but never written.
  ...
  if(P != 0)
  {
	  ...
	  // FiLM modulation pointers (named-local contiguous copies; nullptr => plain 3D_SH_res)
  ...
  return std::make_tuple(dL_dfeatures, dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, dL_gradsum, dL_dfeatures_diffuse, dL_dshapes,
                         dL_dmlp_W1, dL_dmlp_W2, dL_dmlp_W3, dL_dfilm_gamma, dL_dfilm_beta);
}
...
// `--film_gamma_act`: FiLM gamma activation (0=identity, 1=relu, 2=sigmoid).
void SetFilmGammaActCUDA(int v) {
    FORWARD::setFilmGammaAct(v);
    BACKWARD::setFilmGammaAct(v);
}

// `--lock_gamma X`: force gamma_eff = X (constant) + freeze gamma grad; -1e30 = off.
void SetFilmLockGammaCUDA(float v) {
    FORWARD::setFilmLockGamma(v);
    BACKWARD::setFilmLockGamma(v);
}
