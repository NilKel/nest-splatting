...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        # --method 3D_SH_filmres: per-Gauss FiLM modulation of the residual MLP's hash
        # input. Empty => nullptr in CUDA => byte-identical to 3D_SH_res.
        if film_gamma is None:
            film_gamma = torch.Tensor([]).cuda()
        if film_beta is None:
            film_beta = torch.Tensor([]).cuda()
        ...
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, homotrans, ap_level, features, offsets, gridrange, \
            ...
            depth, out_index, radii, sh, geomBuffer, binningBuffer, imgBuffer, shapes, film_gamma, film_beta)
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_intersection_buffer, grad_intersection_count, grad_geomBuffer, grad_out_index, grad_metric_counts):
        ...
        colors_precomp, means3D, scales, rotations, cov3Ds_precomp, homotrans, ap_level, features, offsets, gridrange, \
            features_diffuse, offsets_diffuse, gridrange_diffuse, \
            depth, out_index, radii, sh, geomBuffer, binningBuffer, imgBuffer, shapes, film_gamma, film_beta = ctx.saved_tensors
        ...
        if raster_settings.debug:
            ...
            (grad_features, grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D,
             ...
             grad_features_diffuse, grad_shapes,
             grad_mlp_W1, grad_mlp_W2, grad_mlp_W3,
             grad_film_gamma, grad_film_beta
            ) = _C.rasterize_gaussians_backward(*args)
        ...
        grads = (
            ...
            grad_film_gamma,  # film_gamma
            grad_film_beta,   # film_beta
        )
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None,
        ...
        metric_map = None, film_gamma = None, film_beta = None):
        ...
        # FiLM modulation params (empty => nullptr in CUDA => plain 3D_SH_res)
        if film_gamma is None:
            film_gamma = torch.Tensor([]).cuda()
        if film_beta is None:
            film_beta = torch.Tensor([]).cuda()
...
def set_film_gamma_act(mode=0):
    """`--film_act` (3D_SH_filmres): independent activation on the per-Gauss FiLM
    gamma/beta before they modulate the residual MLP hash input
    (mlp_input = gamma_act(gamma)*H + beta_act(beta)). 0 = identity (default, both
    raw), 1 = gamma_relu, 2 = beta_relu, 3 = gamma_sigmoid, 4 = beta_sigmoid,
    5 = double_relu, 6 = double_sigmoid. Patches both fwd + bwd device globals —
    call once at startup (or whenever it changes)."""
    _C.set_film_gamma_act(int(mode))

def set_film_lock_gamma(value=-1e30):
    """`--lock_gamma X` (3D_SH_filmres): force gamma_eff = X (constant, bypassing
    the per-Gauss gamma and its activation) and freeze gamma's gradient. Use 1.0 to
    pin the FiLM scale to identity so mlp_input = H + beta_act(beta) (pure additive
    latent on the full-strength hash). value = -1e30 (default) disables the lock.
    Patches both fwd + bwd device globals — call once at startup."""
    _C.set_film_lock_gamma(float(value))
