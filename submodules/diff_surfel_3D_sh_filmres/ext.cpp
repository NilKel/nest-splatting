...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_film_gamma_act", &SetFilmGammaActCUDA,
        "`--film_act` (3D_SH_filmres): independent activation on the per-Gauss FiLM "
        "gamma/beta (mlp_input = gamma_act(gamma)*H + beta_act(beta)). 0 = identity "
        "(both raw), 1 = gamma_relu, 2 = beta_relu, 3 = gamma_sigmoid, 4 = beta_sigmoid, "
        "5 = double_relu, 6 = double_sigmoid.");
  m.def("set_film_lock_gamma", &SetFilmLockGammaCUDA,
        "`--lock_gamma X` (3D_SH_filmres): force gamma_eff = X (constant, bypasses gamma "
        "+ its activation) and freeze gamma's gradient. -1e30 = off (default).");
