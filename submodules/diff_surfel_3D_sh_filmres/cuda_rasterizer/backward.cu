...
__device__ __forceinline__ float film_beta_apply(float b) {
	if (d_film_gamma_act == 2 || d_film_gamma_act == 5) return fmaxf(0.0f, b);
	if (d_film_gamma_act == 4 || d_film_gamma_act == 6) return 1.0f / (1.0f + __expf(-b));
	return b;
}
__device__ __forceinline__ float film_beta_apply_grad(float b) {
	if (d_film_gamma_act == 2 || d_film_gamma_act == 5) return (b > 0.0f) ? 1.0f : 0.0f;
	if (d_film_gamma_act == 4 || d_film_gamma_act == 6) { float s = 1.0f / (1.0f + __expf(-b)); return s * (1.0f - s); }
	return 1.0f;
}
...
renderCUDAsurfelBackward(
	...
	// FiLM (3D_SH_filmres): per-Gauss gamma/beta inputs + grad outputs. nullptr => plain 3D_SH_res.
	const float* __restrict__ film_gamma = nullptr,
	const float* __restrict__ film_beta = nullptr,
	float* __restrict__ dL_dfilm_gamma = nullptr,
	float* __restrict__ dL_dfilm_beta = nullptr)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if ((render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				float my_hash_raw[TC_INPUT_DIM] = {0};  // raw (unmodulated) hash, for FiLM dL/dgamma
				...
				if (participates) {
					...
					float my_dL_dinput[TC_INPUT_DIM] = {0};  // 16D (FiLM + 4-level safe; was 12)
					if (!d_skip_mlp_grad) {
					...
					// FiLM (3D_SH_filmres): dL/dbeta = dL/dinput; dL/dgamma = sum(rawhash*dL/dinput);
					// then gamma-scale my_dL_dinput in place so the hash backprop below yields
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 5: {
				...
				// FiLM (3D_SH_filmres): dL/dbeta = dL/dinput; dL/dgamma = sum(hash*dL/dinput).
...
__global__ void setFilmGammaActBwKernel(int v) { d_film_gamma_act = v; }
void BACKWARD::setFilmGammaAct(int v) {
	setFilmGammaActBwKernel<<<1, 1>>>(v);
}

// `--lock_gamma X`: force gamma_eff = X (constant) and freeze gamma grad; -1e30 = off.
__global__ void setFilmLockGammaBwKernel(float v) { d_film_lock_gamma = v; }
void BACKWARD::setFilmLockGamma(float v) {
	setFilmLockGammaBwKernel<<<1, 1>>>(v);
}
...
void BACKWARD::render(
	...
	const float* film_gamma,
	const float* film_beta,
	float* dL_dfilm_gamma,
	float* dL_dfilm_beta)
{
	...
	renderCUDAsurfelBackward<3, 0> <<<grid, block, smem_size>>>(
			...
			dc_features,
			film_gamma, film_beta, dL_dfilm_gamma, dL_dfilm_beta);
