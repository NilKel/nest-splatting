...
// `--film_act`: independent activation on the per-Gauss FiLM gamma and beta before they
// modulate the residual MLP hash input (mlp_input = gamma_act(gamma)*H + beta_act(beta)).
...
renderCUDAsurfelForward(
	...
	// FiLM (3D_SH_filmres): per-Gauss gamma [N,1] / beta [N,24] modulating the
	// residual MLP hash input. nullptr => plain 3D_SH_res.
	...
	const float* __restrict__ film_beta = nullptr)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		// FiLM (3D_SH_filmres): stage per-Gauss gamma/beta into shared (beta stride 24
		// in global; channel-major [16][BLOCK_SIZE] in shared). One global read each per
		// Gauss instead of one per contributing pixel — frees L1 for the hash table.
...
__global__ void setFilmGammaActFwdKernel(int v) { d_film_gamma_act = v; }
void FORWARD::setFilmGammaAct(int v) {
	setFilmGammaActFwdKernel<<<1, 1>>>(v);
}

// `--lock_gamma X`: force gamma_eff = X (constant) and freeze gamma grad; -1e30 = off.
__global__ void setFilmLockGammaFwdKernel(float v) { d_film_lock_gamma = v; }
void FORWARD::setFilmLockGamma(float v) {
	setFilmLockGammaFwdKernel<<<1, 1>>>(v);
}
	...
	const float* film_beta)
{
	...
	renderCUDAsurfelForward<3, 0> <<<grid, block>>>(
		...
		metric_map, metric_counts, film_gamma, film_beta);
