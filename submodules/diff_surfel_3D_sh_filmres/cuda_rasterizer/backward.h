...
namespace BACKWARD
{
	void render(
		...
		// FiLM (3D_SH_filmres): per-Gauss gamma/beta inputs + their grad outputs
		const float* film_gamma = nullptr,
		const float* film_beta = nullptr,
		float* dL_dfilm_gamma = nullptr,
		float* dL_dfilm_beta = nullptr);
