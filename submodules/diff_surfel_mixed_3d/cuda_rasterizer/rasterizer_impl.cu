...
{
	ImageState img;
	// 4 per-pixel slots: [T, M1, M2, T_tex]. T_tex is the textured-only
	// transmittance — Π_{textured j} (1 - α_j), used by the dist regulariser
	// so its `A = 1 - T_tex` excludes untextured EWA occlusion (mixed_3d).
