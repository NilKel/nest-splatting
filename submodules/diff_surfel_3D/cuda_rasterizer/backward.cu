...
renderCUDAsurfelBackward(
	...
	// MLP weight pointers for fused modes (bias-free)
	...
	const float* __restrict__ viewdirs_enc = nullptr)
{
	...
	if ((render_mode & 0xFF) == 5 && dL_dmlp_W1 != nullptr) {
		for (int idx = block.thread_rank(); idx < 32 * 41; idx += BLOCK_SIZE)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if ((render_mode & 0x100) && dL_dmlp_W1 != nullptr) {
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (participates) {
					...
					// 4. Recompute MLP forward (post-ReLU activations only)
					mlp_forward_inline(my_input, my_output, my_h1_post, my_h2_post, true, mlp_weights);
				...
				if (participates) {
					...
					for (int h = 0; h < 32; h++) {
						...
						#pragma unroll
						for (int o = 0; o < 3; o++) {
							dL_dh2 += my_dL_dz3[o] * mlp_weights.W3_rgb[o * 32 + h];
						}
				...
				if (participates) {
					...
					for (int h = 0; h < 32; h++) {
						...
						#pragma unroll
						for (int i = 0; i < 32; i++) {
							dL_dh1 += my_dL_dz2[i] * mlp_weights.W2[i * 32 + h];
						}
				...
				if (participates) {
					...
					for (int i = 0; i < 40; i++) {
						...
						#pragma unroll
						for (int h = 0; h < 32; h++) {
							sum += my_dL_dz1[h] * mlp_weights.W1[h * 41 + i];
						}
						my_dL_dinput[i] = sum;
					}
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 5: {
				...
				// 5. Recompute MLP forward (post-ReLU only, no pre-activation)
				float h1_post[32], h2_post[32];
				float mlp_output[3];
				mlp_forward_inline(mlp_input, mlp_output, h1_post, h2_post, true, mlp_weights);
	...
	const float* viewdirs_enc)
{
	...
	float *mlp_W1_ptr = nullptr;
	float *mlp_W2_ptr = nullptr;
	float *mlp_W3_sh_ptr = nullptr;
	float *mlp_W3_rgb_ptr = nullptr;
	if (render_mode == 5) {
	...
	if (render_mode == 5 && dL_dmlp_W1 != nullptr) {
		...
		if (!force_disable) {
			// Try to enable collaborative GEMM with ~37KB shared memory (sub-tiled)
			// Sub-tile size 128: Layer 1 needs 128*(32+41)*4 = 37376 bytes
