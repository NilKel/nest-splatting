...
__device__ float* d_mlp_W1 = nullptr;      // 41D input → 32D hidden1 (32×41×4 = 5.25KB)
...
__device__ void mlp_forward_fused(
	...
	// Layer 1: IN_DIM → HIDDEN_DIM (ReLU, no explicit bias)
	#pragma unroll
	...
	// Layer 2: HIDDEN_DIM → HIDDEN_DIM (ReLU, no bias)
	#pragma unroll
	for (int h = 0; h < HIDDEN_DIM; h++) {
		float acc = 0;
		#pragma unroll
	...
	// Layer 3: HIDDEN_DIM → OUT_DIM (Sigmoid optional, no bias)
	#pragma unroll
...
__global__ void setMlpPointersKernel(
	float* W1, float* W2, float* W3_sh, float* W3_rgb)
{
	d_mlp_W1 = W1;
	d_mlp_W2 = W2;
	d_mlp_W3_sh = W3_sh;
...
// Copy MLP weights to global device memory (bias-free)
	...
	bool is_sh_mode)
{
	...
	if (!mlp_weights_allocated) {
		cudaMalloc(&h_mlp_W1, 41 * 32 * sizeof(float));   // W1[32×41] with implicit bias
	...
	cudaMemcpy(h_mlp_W1, W1, 41 * 32 * sizeof(float), cudaMemcpyDeviceToDevice);
...
// Get MLP weight device pointers for passing to kernels (bias-free)
