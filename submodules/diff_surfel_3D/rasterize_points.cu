	...
	const torch::Tensor& viewdirs_enc)
{
  ...
  // MLP gradient buffers for 3D_direct_fused mode (render_mode=5, bias-free)
  // Architecture: 41D input (40D + 1.0 pad) -> 32 hidden -> 32 hidden -> 3 RGB
  torch::Tensor dL_dmlp_W1 = torch::zeros({32, 41}, means3D.options());  // Layer 1 weights (implicit bias via pad)
  torch::Tensor dL_dmlp_W2 = torch::zeros({32, 32}, means3D.options());   // Layer 2 weights
  torch::Tensor dL_dmlp_W3 = torch::zeros({3, 32}, means3D.options());    // Layer 3 weights (RGB)
