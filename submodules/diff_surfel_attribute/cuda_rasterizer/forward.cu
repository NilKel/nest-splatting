...
// ============================================================================
// PER-GAUSS ERROR ATTRIBUTION (forward-only side output)
// Set via SetErrorAttributionCUDA from Python. Both pointers must be non-null
// to activate; on each Gauss-pixel hit, the render kernel does
//   atomicAdd(&d_err_accum[gauss_id],  T·α · d_error_image[pix_id])
// giving a per-Gauss alpha-blend-weighted error sum across the frame.
// d_err_accum: [P] float buffer (caller-owned, zero-initialised per view).
// d_error_image: [H,W] float buffer (e.g. per-pixel L1 |render − gt|).
// ============================================================================
__device__ float* d_err_accum   = nullptr;
__device__ float* d_error_image = nullptr;
__device__ int    d_err_W       = 0;
__device__ int    d_err_H       = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (false && (render_mode & 0xFF) == 5) {
		  ...
		  for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		  {
			...
			if (active) {
				...
				// Per-Gauss alpha-weighted error attribution (forward-only side output).
				// Both device pointers non-null → attribute T·α · error_image[pix] to this
				// Gauss. Untouched when disabled (atomic skipped, register-only check).
				if (d_err_accum != nullptr && d_error_image != nullptr
					&& pix.x < (unsigned)d_err_W && pix.y < (unsigned)d_err_H) {
					atomicAdd(&d_err_accum[collected_id[j]],
						my_w * d_error_image[pix.y * d_err_W + pix.x]);
				}
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (!tex_fwd && ewa_conic != nullptr) {
				...
				// Per-Gauss alpha-weighted error attribution (untextured-EWA branch).
				if (d_err_accum != nullptr && d_error_image != nullptr
					&& pix.x < (unsigned)d_err_W && pix.y < (unsigned)d_err_H) {
					atomicAdd(&d_err_accum[gid_e],
						w_e * d_error_image[pix.y * d_err_W + pix.x]);
				}
		...
		// Accumulate hashgrid features
			...
			// Per-Gauss alpha-weighted error attribution (textured scalar branch).
			if (d_err_accum != nullptr && d_error_image != nullptr
				&& pix.x < (unsigned)d_err_W && pix.y < (unsigned)d_err_H) {
				atomicAdd(&d_err_accum[collected_id[j]],
					w * d_error_image[pix.y * d_err_W + pix.x]);
			}
...
// Per-Gauss alpha-weighted error attribution: install/clear device pointers.
// Both pointers must be non-null for the kernel to accumulate.
__global__ void setErrorAttributionKernel(float* err_accum, float* error_image, int W, int H) {
	d_err_accum   = err_accum;
	d_error_image = error_image;
	d_err_W       = W;
	d_err_H       = H;
}
void FORWARD::setErrorAttribution(float* err_accum, float* error_image, int W, int H) {
	setErrorAttributionKernel<<<1, 1>>>(err_accum, error_image, W, H);
}
