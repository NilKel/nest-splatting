...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_error_attribution", &SetErrorAttributionCUDA,
        "Install per-Gauss alpha-weighted error attribution device buffers. "
        "Both tensors empty → disabled (default). err_accum: [P] float "
        "(caller-owned, zeroed per view); error_image: [H,W] float.");
