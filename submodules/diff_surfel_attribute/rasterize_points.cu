...
// Per-Gauss error attribution (forward-only side output). Pass empty tensors
// to disable; otherwise err_accum: [P] float, error_image: [H,W] float.
void SetErrorAttributionCUDA(torch::Tensor err_accum, torch::Tensor error_image) {
    float* p_acc = err_accum.numel() > 0 ? err_accum.contiguous().data_ptr<float>() : nullptr;
    float* p_img = error_image.numel() > 0 ? error_image.contiguous().data_ptr<float>() : nullptr;
    int W = error_image.numel() > 0 ? (int)error_image.size(1) : 0;
    int H = error_image.numel() > 0 ? (int)error_image.size(0) : 0;
    FORWARD::setErrorAttribution(p_acc, p_img, W, H);
}
