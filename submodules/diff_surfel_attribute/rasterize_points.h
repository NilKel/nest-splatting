...
// Install/clear device pointers for per-Gauss alpha-weighted error attribution.
// Pass empty tensors to disable. err_accum: [P] float (caller-owned, zeroed
// per view); error_image: [H,W] float (e.g. per-pixel L1 |render − gt|).
void SetErrorAttributionCUDA(torch::Tensor err_accum, torch::Tensor error_image);
