...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& film_gamma,
	const torch::Tensor& film_beta)
{
  ...
  if(P != 0)
  {
	  ...
	  // FiLM modulation pointers (nullptr => cat behaviour in kernel).
	  // film_gamma/film_beta arrive as NON-contiguous slices of a packed [N,25]
	  // tensor (gamma=col 0, beta=cols 1:). Hold the contiguous copies in named
	  // locals so their storage outlives the kernel launch (a temporary from
	  // `.contiguous().data()` would be freed before the kernel runs → illegal access).
	  torch::Tensor film_gamma_c = (film_gamma.numel() > 0) ? film_gamma.contiguous() : film_gamma;
	  torch::Tensor film_beta_c  = (film_beta.numel() > 0)  ? film_beta.contiguous()  : film_beta;
	  const float* film_gamma_ptr = (film_gamma.numel() > 0) ? film_gamma_c.data<float>() : nullptr;
	  const float* film_beta_ptr  = (film_beta.numel() > 0)  ? film_beta_c.data<float>()  : nullptr;
	...
	const torch::Tensor& film_beta)
{
  ...
  // FiLM gradients (β width matches the input; default 1 when FiLM unused so the
  // tensor is allocatable but never written by the kernel)
  int film_beta_dim = (film_beta.numel() > 0) ? film_beta.size(1) : 1;
  torch::Tensor dL_dfilm_gamma = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dfilm_beta  = torch::zeros({P, film_beta_dim}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  // FiLM modulation pointers (nullptr => cat behaviour in kernel). Hold the
	  // contiguous copies in named locals (slices of packed [N,25] are non-contiguous;
	  // a `.contiguous().data()` temporary would dangle before the kernel runs).
	  torch::Tensor film_gamma_c_bw = (film_gamma.numel() > 0) ? film_gamma.contiguous() : film_gamma;
	  torch::Tensor film_beta_c_bw  = (film_beta.numel() > 0)  ? film_beta.contiguous()  : film_beta;
	  const float* film_gamma_ptr_bw = (film_gamma.numel() > 0) ? film_gamma_c_bw.data<float>() : nullptr;
	  const float* film_beta_ptr_bw  = (film_beta.numel() > 0)  ? film_beta_c_bw.data<float>()  : nullptr;
	  ...
	  film_gamma_ptr_bw,
	  film_beta_ptr_bw,
	  dL_dfilm_gamma.contiguous().data<float>(),
	  dL_dfilm_beta.contiguous().data<float>());
  }

  return std::make_tuple(dL_dfeatures, dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, dL_gradsum, dL_dfeatures_diffuse, dL_dshapes, dL_dfilm_gamma, dL_dfilm_beta);
}
