...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        # FiLM modulation params (empty tensor => nullptr in CUDA => cat behaviour)
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, out_index, radii, geomBuffer, binningBuffer, imgBuffer, pixels, transmittance_avg, max_weight, accum_weights = _C.rasterize_gaussians(*args)
        ...
        return color, radii, depth, transmittance_avg, pixels, max_weight, accum_weights

    @staticmethod
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_2, grad_3):
        ...
        if raster_settings.debug:
            ...
            try:
                (grad_features, grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D,
                 ...
                 grad_features_diffuse, grad_shapes, grad_film_gamma, grad_film_beta
...
class GaussianRasterizationSettings(NamedTuple):
    ...
    max_intersections_per_pixel : int = 0  # Used by other rasterizers (diff_surfel_3D, etc.)
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None,
        ...
        # FiLM modulation params (empty => nullptr in CUDA)
        if film_gamma is None:
            film_gamma = torch.Tensor([]).cuda()
        if film_beta is None:
            film_beta = torch.Tensor([]).cuda()
