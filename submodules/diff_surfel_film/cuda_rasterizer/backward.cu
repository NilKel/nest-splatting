		...
		if (alpha < 1.0f / 255.0f)
			...
			case 1: {
				...
				if (hashgrid_levels > 0) {
					...
					// FiLM: dL/dH = gamma * dL/df, so gamma-scale the hash/xyz gradient.
					const float film_g_bw = (film_gamma != nullptr) ? film_gamma[global_id] : 1.0f;
					for(int i = 0; i < hashgrid_dim; i++){
						grad_feat_hashgrid[i] = film_g_bw * grad_feat[per_gaussian_dim + i];
					}
				...
				// FiLM backward: beta grad = grad_feat (= w*dL/df); gamma grad = sum(H * grad_feat);
				// reconstruct modulated feat = gamma*H + beta for the alpha recurrence below.
				if (film_gamma != nullptr) {
					const float film_g = film_gamma[global_id];
					const float* film_b = film_beta + global_id * C;
					const int film_valid = per_gaussian_dim + hashgrid_levels * l_dim;
					float film_dgamma = 0.0f;
					for(int ch = 0; ch < C; ch++){
						const float Hch = (ch < film_valid) ? feat[ch] : 0.0f;
						film_dgamma += Hch * grad_feat[ch];
						atomicAdd(&(dL_dfilm_beta[global_id * C + ch]), grad_feat[ch]);
						feat[ch] = film_g * Hch + film_b[ch];
					}
					atomicAdd(&(dL_dfilm_gamma[global_id]), film_dgamma);
				}
	...
	switch (C) {
		case 3:
			renderCUDAsurfelBackward<3, 0> <<<grid, block>>>(
					...
					hash_features_diffuse, level_offsets_diffuse, gridrange_diffuse, dL_dfeatures_diffuse, render_mode, shapes, kernel_type, dL_dshapes, detach_hash_grad, film_gamma, film_beta, dL_dfilm_gamma, dL_dfilm_beta);
			break;
		case 8:
