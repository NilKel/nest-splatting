...
// Leaky slope for the outer per-Gauss activation. 0 = standard ReLU (default). The decomp
// viz sets it to 1.0 → identity activation, so the (possibly negative) tex residual passes
// through SIGNED and |.| reveals it. Set via set_lru_slope().
__device__ float d_lru_slope = 0.0f;
__device__ __forceinline__ float concat_outer_act(float x) {
	return (d_lru_slope != 0.0f) ? ((x > 0.0f) ? x : d_lru_slope * x) : fmaxf(0.0f, x);
}
...
renderCUDAsurfelForward(
	...
	// 3D_SH_concat: per-Gauss 16D surfel latent (cols 0..15 of _film_params beta [N,24]).
	// nullptr ⇒ pure-hash behavior (byte-identical to 3D_SH_32).
	const float* __restrict__ film_beta = nullptr)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	float max_w = 0.0f;
	float max_depth = 0.0f;
	int max_idx = -1;  // per-pixel max-contributor id (FastGS / max_contrib_idx)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		  ...
		  for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		  {
			...
			if (active) {
				for (int ch = 0; ch < ORIG_OUTPUT_DIM; ch++) {
					...
					C[ch] += concat_outer_act(my_sh_color[ch] + residual + d_res_bias) * my_w;
				}
		...
		case 5: {
			...
			float hash_feat[16];  // Max 4 levels × 4D = 16D (concat: hash half of [latent(16)|hash(16)])
			...
			// 3. Build MLP input — CONCAT [latent(16) | hash(16)] = 32D (TC_INPUT_DIM).
			//    [0..15]  = per-Gauss surfel latent (raw; cols 0..15 of _film_params beta [N,24])
			//    [16..31] = hash features (zero-padded if hash_dim < 16)
			float mlp_input[TC_INPUT_DIM];  // TC_INPUT_DIM = 32
			for (int i = 0; i < TC_INPUT_DIM; i++) mlp_input[i] = 0.0f;
			if (film_beta != nullptr)
				for (int i = 0; i < 16; i++) mlp_input[i] = film_beta[gauss_id * 24 + i];
			for (int i = 0; i < hash_dim && i < 16; i++) mlp_input[16 + i] = hash_feat[i];
			...
			for (int ch = 0; ch < 3; ch++)
				feat[ch] = concat_outer_act(sh_color[ch] + residual[ch] + d_res_bias);
		...
		case 6: {
			...
			// 6. feat = ReLU(ReLU(SH_color + sh_bias) + residual + res_bias)
			for (int ch = 0; ch < 3; ch++)
				feat[ch] = concat_outer_act(sh_color_6[ch] + residual_6[ch] + d_res_bias);
	...
	{
		final_T[pix_id] = T;
		if (out_index != nullptr) out_index[pix_id] = max_idx;  // FastGS max-contributor
		n_contrib[pix_id] = last_contributor;
...
// --lru / decomp signed-residual: leaky slope on the outer per-Gauss activation (0 = ReLU, 1 = identity).
__global__ void setLruSlopeKernel(float v) { d_lru_slope = v; }
void FORWARD::setLruSlope(float v) {
	setLruSlopeKernel<<<1, 1>>>(v);
}
	...
	const float* film_beta)
{
	...
	renderCUDAsurfelForward<3, 0> <<<grid, block>>>(
		...
		intersection_buffer, intersection_count, max_intersections_per_pixel, viewdirs_enc, film_beta);
