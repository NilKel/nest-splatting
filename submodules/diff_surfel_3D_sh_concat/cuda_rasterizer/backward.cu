...
renderCUDAsurfelBackward(
	...
	// 3D_SH_concat: per-Gauss surfel latent (in, for MLP-forward recompute) +
	// grad output [P,24] (cols 0..15 via atomicAdd)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
			...
			for (int j = 0; j < effective_toDo; j++)
			{
				...
				if (participates) {
					...
					const int hash_dim_collab = active_hashgrid_levels * l_dim;
						// concat: surfel latent in [0:16] (always present; hash goes to [16:32])
						if (film_beta != nullptr)
							for (int i = 0; i < 16; i++) my_input[i] = film_beta[global_id * 24 + i];
					if (!skip_hash && active_hashgrid_levels > 0 && l_dim == 4) {
						...
						if (hash_dim_collab == 4)
							...
							query_feature<false, 16, 4>(hash_feat, xyz, voxel_min, voxel_max, collec_offsets,
							                           appearance_level, hash_features, active_hashgrid_levels,
							                           l_scale, Base, align_corners, interp, if_contract, false);
						for (int i = 0; i < hash_dim_collab && i < 16; i++) my_input[16 + i] = hash_feat[i];
					}
				...
				if (participates) {
					...
					float my_dL_dinput[TC_INPUT_DIM];  // 32D concat input: [latent(16) | hash(16)]
					for (int i = 0; i < 16 + hash_dim && i < TC_INPUT_DIM; i++) {  // latent[0:16] + hash[16:16+hash_dim]
						float sum = 0;
						for (int h = 0; h < TC_HIDDEN_DIM; h++) {
					...
					// latent grad: dL_dinput[0:16] → dL_dfilm_beta (cols 0..15)
						if (dL_dfilm_beta != nullptr)
							for (int i = 0; i < 16; i++)
								atomicAdd(&(dL_dfilm_beta[global_id * 24 + i]), my_dL_dinput[i]);

						// Backprop to hash features - dL_dxyz flows to geometry
					float dL_dxyz[3] = {0, 0, 0};
					if (!skip_hash && active_hashgrid_levels > 0 && l_dim == 4) {
						float dL_dhash[16];
						for (int i = 0; i < hash_dim; i++) dL_dhash[i] = my_dL_dinput[16 + i];  // hash occupies input [16:32]
		...
		if (alpha < 1.0f / 255.0f)
			...
			case 5: {
				...
				// 2. Build MLP input — CONCAT [latent(16) | hash(16)] = 32D
				float mlp_input[TC_INPUT_DIM];
				for (int i = 0; i < TC_INPUT_DIM; i++) mlp_input[i] = 0.0f;
				if (film_beta != nullptr)
					for (int i = 0; i < 16; i++) mlp_input[i] = film_beta[global_id * 24 + i];
				for (int i = 0; i < hash_dim_px && i < 16; i++) mlp_input[16 + i] = hash_feat[i];
				...
				// latent grad: dL_dinput_full[0:16] → dL_dfilm_beta (cols 0..15)
				if (dL_dfilm_beta != nullptr)
					for (int i = 0; i < 16; i++)
						atomicAdd(&(dL_dfilm_beta[global_id * 24 + i]), dL_dinput_full[i]);

				// 7. Backprop to hash features (hash occupies input [16:32])
				const int hash_dim = active_hashgrid_levels * l_dim;
				if (!skip_hash && active_hashgrid_levels > 0 && l_dim == 4) {
					float dL_dhash[16];
					for (int i = 0; i < hash_dim; i++)
						dL_dhash[i] = dL_dinput_full[16 + i];  // hash occupies input [16:32]
	...
	float* dL_dfilm_beta)
{
	...
	renderCUDAsurfelBackward<3, 0> <<<grid, block, smem_size>>>(
			...
			dc_features, film_beta, dL_dfilm_beta);
