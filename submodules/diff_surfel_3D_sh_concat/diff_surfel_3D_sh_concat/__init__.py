...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        # 3D_SH_concat: per-Gauss surfel latent (None ⇒ empty ⇒ nullptr ⇒ pure-hash sh_32)
        ...
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, homotrans, ap_level, features, offsets, gridrange, \
            ...
            depth, out_index, radii, sh, geomBuffer, binningBuffer, imgBuffer, shapes, film_beta)

        # out_index is the per-pixel max-contributor id (FastGS max_contrib_idx fallback).
        ctx.mark_non_differentiable(out_index)
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_0, grad_1, grad_intersection_buffer, grad_intersection_count, grad_geomBuffer, grad_out_index):
        ...
        colors_precomp, means3D, scales, rotations, cov3Ds_precomp, homotrans, ap_level, features, offsets, gridrange, \
            features_diffuse, offsets_diffuse, gridrange_diffuse, \
            depth, out_index, radii, sh, geomBuffer, binningBuffer, imgBuffer, shapes, film_beta = ctx.saved_tensors
        ...
        if raster_settings.debug:
            ...
            try:
                (grad_features, grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D,
                 ...
                 grad_mlp_W1, grad_mlp_W2, grad_mlp_W3, grad_film_beta
        ...
        grads = (
            ...
            None,  # kernel_type
            None,  # aabb_mode
            grad_film_beta,  # film_beta (3D_SH_concat surfel latent)
        )
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None,
        ...
        render_mode = 0, shapes = None, kernel_type = 0, aabb_mode = 0, film_beta = None):
...
def set_lru_slope(v=0.0):
    """Forward-only leaky slope on the outer per-Gauss activation. 0 = ReLU (default),
    1 = identity → the (possibly negative) tex residual passes through SIGNED. The decomp
    viz sets it to 1.0 to render |residual|, then restores 0."""
    _C.set_lru_slope(float(v))
