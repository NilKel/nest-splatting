	...
	const torch::Tensor& film_beta)
{
  ...
  if(P != 0)
  {
	  ...
	  // 3D_SH_concat: per-Gauss surfel latent (empty ⇒ nullptr ⇒ pure-hash sh_32 behavior)
	...
	const torch::Tensor& film_beta)
{
  ...
  // 3D_SH_concat: grad w.r.t. the per-Gauss surfel latent [P,24] (cols 0..15 populated)
  torch::Tensor dL_dfilm_beta = torch::zeros({P, 24}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  // 3D_SH_concat: per-Gauss surfel latent (in, for MLP recompute)
	  ...
	  film_beta_ptr_bw,
	  (film_beta.numel() > 0) ? dL_dfilm_beta.contiguous().data<float>() : nullptr);
  }

  // dL_dmeans2D is a [P,3] float3* kernel buffer (sh_32 convention), but the renderer's
  // screenspace_points is [N,4]. Pad to [P,4] (4th col = 0) so the autograd grad shape
  // matches the input. The x/y screen-position gradient (cols 0,1) drives densification.
  torch::Tensor dL_dmeans2D_out = torch::zeros({P, 4}, means3D.options());
  if (P > 0) dL_dmeans2D_out.narrow(1, 0, 3).copy_(dL_dmeans2D);

  return std::make_tuple(dL_dfeatures, dL_dmeans2D_out, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, dL_gradsum, dL_dfeatures_diffuse, dL_dshapes,
                         dL_dmlp_W1, dL_dmlp_W2, dL_dmlp_W3, dL_dfilm_beta);
}
...
// Forward-only (decomp viz): leaky slope on the outer activation. 0 = ReLU, 1 = identity (signed residual).
