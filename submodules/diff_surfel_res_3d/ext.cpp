...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("set_textured_bias_gate", &SetTexturedBiasGateCUDA,
        "`--method res_3d_double`: when 1 (default for `--method res_3d`), "
        "tex carriers force sh_color = 0 → feat = residual (pure separation). "
        "When 0, tex carriers KEEP sh_color = ReLU(SV+0.5) → feat = SV + "
        "residual (full mixed_3d-style capacity per Gauss, still dual cascade).");
