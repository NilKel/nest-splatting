...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, color_sv_aux, color_tex_aux, depth, out_index, radii, geomBuffer, binningBuffer, imgBuffer, pixels, transmittance_avg, intersection_buffer, intersection_count, metric_counts = _C.rasterize_gaussians(*args)
        ...
        # `--method res_3d` single-pass dual-cascade outputs. color_sv_aux is
        # Σ T_sv·α·sv_rgb over untextured (EWA) contributors only; color_tex_aux
        # is Σ T_tex·α·(residual + res_bias) over textured contributors only.
        # Renderer composes LRU(color_sv_aux + color_tex_aux) on the Python side
        # so signed C_tex can darken SV regions. Backward still flows through
        # `color` (joint cascade) until the dual-cascade reverse recurrence is
        # mirrored in backward.cu — see TODO in submodule README.
        ...
        # `--method res_3d` SINGLE-PASS: `color` is the signed sum
        # C_sv_aux + C_tex_aux (forward writes the sum to out_color), backward
        # routes upstream dL_dout_color through both cascades inside the kernel.
        # color_sv_aux / color_tex_aux are intentionally NOT returned to keep
        # tuple arity (= 11) compatible with the renderer's unpacker; the kernel
        # still writes them to the alloc'd buffers for future introspection.
...
def set_textured_bias_gate(v=1):
    """`--method res_3d_double`: toggle the per-Gauss bias gate on textured carriers.

    v=1 (default, `--method res_3d`): textured carriers force `sh_color = 0` →
        feat = residual only (no +0.5 SH-bias floor). Pure separation design:
        tex carries residual, untex EWA carries SV.
    v=0 (`--method res_3d_double`): textured carriers KEEP `sh_color = ReLU(SV+0.5)` →
        feat = ReLU(SV+0.5) + residual + res_bias. Tex carriers carry BOTH SV
        AND residual (full mixed_3d-style per-Gauss capacity) while still
        using the dual-cascade T_tex / T_sv blending.

    Patches both forward and backward device globals — call once at split."""
