...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.ewa_conic, P, 128);  // mixed_3d untextured EWA conic
...
{
	ImageState img;
	// 6 per-pixel slots: [T, M1, M2, A_tex, T_sv_aux_final, T_tex_aux_final].
	// Slots 0-3 unchanged from mixed_3d. Slots 4-5 added for `--method res_3d`
	// single-pass dual-cascade backward: forward persists final T values of the
	// untextured (SV-aux) and textured (tex-aux) cascades, backward recovers
	// per-Gauss T_aux by reverse division.
	obtain(chunk, img.accum_alpha, N * 6, 128);
	...
	const float* scaling_z)
{
	...
	CHECK_CUDA(FORWARD::render(
		...
		metric_map,
		metric_counts,
		is_textured,  // `--method mixed` per-Gauss bool flag (nullptr → all-textured)
		// `--method mixed_3d`: only expose the EWA conic buffer to the render
		// kernel when scaling_z is present (post-texsplit). When scaling_z is
		// null the buffer is unpopulated → pass nullptr so untextured rows fall
		// through to the 2DGS ray-splat path (byte-identical to `--method mixed`).
