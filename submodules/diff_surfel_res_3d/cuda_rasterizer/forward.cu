...
// `--method res_3d` (gate ON, default 1): textured Gauss force sh_color = 0 →
// feat = residual (no +0.5 SH-bias floor on tex carriers). This is the
// pure-separation design where tex contributes residual ONLY.
// `--method res_3d_double` (gate OFF, set via set_textured_bias_gate(0)):
// textured Gauss keep `sh_color = ReLU(SV + sh_bias)` → feat = ReLU(SV+0.5)
// + residual (full mixed_3d-style capacity per Gauss) while still using the
// dual-cascade T_tex / T_sv backward.
...
renderCUDAsurfelForward(
	...
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	float* __restrict__ out_color_sv_aux,
	float* __restrict__ out_color_tex_aux,
	float* __restrict__ out_others,
	...
	const float4* __restrict__ ewa_conic = nullptr)
{
	...
	// `--method res_3d` dual-cascade auxiliary state (single-pass kernel).
	// Tracks two INDEPENDENT transmittance / color cascades alongside the
	// existing joint T / C[] — no kernel rewrite, just additive updates at
	// every contribution site. T_tex_aux / C_tex_aux see only textured
	// contributors (the 2D residual-carriers); T_sv_aux / C_sv_aux see only
	// untextured contributors (the 3D EWA SV-carriers). At the end of the
	// kernel we write C_tex_aux and C_sv_aux to two new output buffers; the
	// renderer composes the final image in Python as LRU(C_sv + C_tex). For
	// non-res_3d callers (--method mixed_3d, etc.) these auxiliary outputs
	// are simply ignored on the Python side. The joint T / C[] cascade
	// continues to drive the existing depth / dist / normal regularisers.
	float T_tex_aux = 1.0f;
	float T_sv_aux  = 1.0f;
	float C_tex_aux[CHANNELS] = { 0 };
	float C_sv_aux [CHANNELS] = { 0 };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (false && (render_mode & 0xFF) == 5) {
		  ...
		  for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		  {
			...
			do {
				...
				// `--method res_3d` dual-cascade early-out (WMMA collab path).
				// `done` is per-thread (per-pixel); the collab GEMM itself is
				// block-uniform, so we can't per-Gauss skip without breaking
				// the WMMA tile. We just terminate the THREAD when both
				// cascades have saturated; remaining threads in the block
				// continue (they may still have cascades active). When a
				// per-thread contribution is degenerate (e.g. T_tex_aux=0 for
				// a textured Gauss), the accumulation site multiplies by ~0
				// → harmless, just a tiny amount of wasted bandwidth.
				if (T_tex_aux < 0.0001f && T_sv_aux < 0.0001f) {
					done = true; active = false; break;
				}
			...
			if (active) {
				...
				// Load SH base color. `--method res_3d` per-Gauss bias gate:
				// textured carriers post-split have their SH/SV zeroed, so the
				// only contribution from `rgb` is the +sh_bias floor injected
				// by computeColorFromSH (`ReLU(0 + 0.5) = 0.5`). Forcing the
				// SH base to 0 for textured Gauss makes `feat = residual`
				// directly — no +0.5 baseline. Untex carriers keep the bias
				// (their SV is the actual signal, `ReLU(SV + 0.5)`).
				int gauss_id = collected_id[j];
				const bool tex_my = collected_is_textured[j];
				for (int ch = 0; ch < 3; ch++)
					my_sh_color[ch] = (d_textured_bias_gate && tex_my) ? 0.0f : RGB_TO_FLOAT(rgb[gauss_id * 3 + ch]);
			...
			if (active) {
				// `--method res_3d` textured-aux cascade weight (independent T_tex_aux,
				// decays ONLY from textured contributors). C_tex_aux accumulates the
				// SIGNED residual contribution (sv_bias zeroed for tex rows on the
				// Python side, so feat_ch is effectively `residual + res_bias`).
				const float my_w_tex_aux = my_alpha * T_tex_aux;
				for (int ch = 0; ch < ORIG_OUTPUT_DIM; ch++) {
					...
					C[ch] += feat_ch * my_w;
					C_tex_aux[ch] += feat_ch * my_w_tex_aux;
				}
				T_tex_aux *= (1.0f - my_alpha);
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (!tex_fwd && ewa_conic != nullptr) {
				...
				// `--method res_3d` dual-cascade early-out. Pixel terminates
				// only when BOTH cascades have saturated. No per-Gauss skip —
				// once T_sv_aux is below 1e-4, this untex Gauss still
				// accumulates `C_sv_aux += T_sv_aux·α·rgb ≈ 0` and decays
				// `T_sv_aux *= (1-α)`. Trivial work, but the reverse recurrence
				// in backward stays bit-symmetric (no fuzzy "was this Gauss
				// skipped" decision needed when reverse-dividing T_sv_back).
				// Joint T (used by depth/dist/normal aux) still decays through
				// all Gauss; it's just no longer the loop-termination gate.
				if (T_tex_aux < 0.0001f && T_sv_aux < 0.0001f) { done = true; continue; }
				...
				// `--method res_3d` SV-aux cascade (independent T_sv_aux,
				// decays ONLY from EWA contributors). Renderer reads from
				// out_color_sv_aux on the Python side.
				{
					const float w_sv_aux = alpha_e * T_sv_aux;
					for (int ch = 0; ch < CHANNELS; ch++)
						C_sv_aux[ch] += RGB_TO_FLOAT(rgb[gid_e * CHANNELS + ch]) * w_sv_aux;
					T_sv_aux *= (1 - alpha_e);
				}
		...
		// `--method res_3d` dual-cascade early-out. Pixel terminates only
		// when BOTH cascades have saturated. No per-Gauss skip (see EWA
		// branch comment above for why — keeps backward bit-symmetric).
		// Joint T (used by depth/dist/normal aux) still decays through all
		// Gauss; only the loop termination is decoupled from it.
		if (T_tex_aux < 0.0001f && T_sv_aux < 0.0001f) { done = true; continue; }
		float test_T = T * (1 - alpha);
		...
		case 5: {
			...
			// `--method res_3d` per-Gauss bias gate (scalar path mirror of the
			// WMMA collab path): textured carriers force sh_color = 0 so the
			// +sh_bias floor doesn't pollute their residual-only contribution.
			int gauss_id = collected_id[j];
			float sh_color[3];
			for (int ch = 0; ch < 3; ch++)
				sh_color[ch] = (d_textured_bias_gate && tex_fwd) ? 0.0f : RGB_TO_FLOAT(rgb[gauss_id * 3 + ch]);
		...
		// Accumulate hashgrid features
				...
				// `--method res_3d` textured-aux cascade (scalar path; independent
				// T_tex_aux decays ONLY from textured contributors). SV is zeroed
				// on tex rows so feat[] is the signed residual + res_bias only.
				{
					const float w_tex_aux = alpha * T_tex_aux;
					for (int ch = 0; ch < CHANNELS; ch++)
						C_tex_aux[ch] += feat[ch] * w_tex_aux;
					T_tex_aux *= (1.0f - alpha);
				}
	...
	if (inside)
	{
		...
		// `--method res_3d` SINGLE-PASS dual-cascade output. out_color is the
		// signed sum C_sv_aux + C_tex_aux — the renderer's per-pixel LRU
		// composes the final image from this. Autograd's dL/dout_color flows
		// linearly to BOTH dL/dC_sv_aux and dL/dC_tex_aux (since out_color is
		// their sum). The backward kernel routes this single upstream grad
		// through dual T cascades + dual accum_rec per primitive.
		//
		// Non-res_3d usage (EWA branch never fires → C_sv_aux ≡ 0, T_sv_aux ≡ 1,
		// C_tex_aux ≡ original joint C[], T_tex_aux ≡ joint T) → out_color
		// byte-identical to the pre-dual-cascade C[] write.
		for (int ch = 0; ch < CHANNELS; ch++)
		{
			out_color[ch * H * W + pix_id] = C_sv_aux[ch] + C_tex_aux[ch];
		}

		// Optional pass-through writes for visualization / introspection.
		if (out_color_sv_aux != nullptr) {
			for (int ch = 0; ch < CHANNELS; ch++)
				out_color_sv_aux[ch * H * W + pix_id] = C_sv_aux[ch];
		}
		if (out_color_tex_aux != nullptr) {
			for (int ch = 0; ch < CHANNELS; ch++)
				out_color_tex_aux[ch * H * W + pix_id] = C_tex_aux[ch];
		}
		...
		// `--method res_3d`: slots 4-5 = final values of the dual-cascade aux
		// transmittances. Backward starts the reverse recurrence from these
		// and reverse-divides by (1 - α_j) to recover T_aux_at_j per primitive.
		final_T[pix_id + 4 * H * W] = T_sv_aux;
		final_T[pix_id + 5 * H * W] = T_tex_aux;
		out_others[pix_id + DEPTH_OFFSET * H * W] = D;
		out_others[pix_id + ALPHA_OFFSET * H * W] = 1 - T;
		for (int ch=0; ch<3; ch++) out_others[pix_id + (NORMAL_OFFSET+ch) * H * W] = N[ch];
		out_others[pix_id + MIDDEPTH_OFFSET * H * W] = median_depth;
...
// `--method res_3d_double`: toggle the per-Gauss bias gate (forward side).
...
void FORWARD::render(
	...
	float* out_color,
	float* out_color_sv_aux,
	float* out_color_tex_aux,
	float* out_others,
	int* out_index,
	float* cover_pixels,
	float* trans_avg,
	...
	const float4* ewa_conic)
{
	...
	renderCUDAsurfelForward<3, 0> <<<grid, block>>>(
		...
		depths, normal_opacity, final_T, n_contrib, bg_color, out_color, out_color_sv_aux, out_color_tex_aux, out_others, out_index, cover_pixels, trans_avg, cam_pos,
