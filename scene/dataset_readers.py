...
from utils.graphics_utils import get_rays_np
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        c_y = 0.5 * image.size[1]
        c_x = 0.5 * image.size[0]
        K = np.array([[focal_length_x, 0, c_x], [0, focal_length_y, c_y], [0, 0, 1]], dtype=np.float32)
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, K=K, FovY=FovY, FovX=FovX, image=image, alpha = None,
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    ### DTU dataset:
    if 'dtu' in path or 'DTU' in path:
        test_cams = [8, 13, 16, 21, 26, 31, 34]
        if len(cam_infos) > 56:
            test_cams.append(56)
        
        print(f'DTU dataset, eval index {test_cams} .')
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", data_type = None):
    cam_infos = []

    cam_K = None
    w2c_list = []

    depth_folder = os.path.join(path, 'depth', data_type)
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            focal_y = fov2focal(fovy, image.size[0])
            focal_x = fov2focal(fovx, image.size[1])
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension, 'train')
    print("Reading Test Transforms")

    test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension, 'test')
    # test_cam_infos = readCamerasFromTransforms(path, "transforms_path.json", white_background, extension, 'test')
