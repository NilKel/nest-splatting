...
def build_H(rots, scales, xyzs):

    batch = rots.shape[0]
    s = torch.zeros(batch, 1, 3).cuda()
    s[:, 0, :2] = scales

    rs = torch.mul(s, rots)
    h = torch.zeros(batch, 4, 4).cuda()
    h[:, :3, :3] = rs
    h[:, :3, 3] = xyzs
    h[:, 3, 3] = 1.0
...
def MEM_PRINT(tag = None):

    x = 1
    # s = "" if tag is None else tag
    # print(f'{s} -------------')
    # allocated_memory = torch.cuda.memory_allocated() / (1024 ** 3)
    # print(f"Allocated memory: {allocated_memory:.4f} GB")
    # reserved_memory = torch.cuda.memory_reserved() / (1024 ** 3)
    # print(f"Reserved memory: {reserved_memory:.4f} GB")
    # print(f'-------------')
