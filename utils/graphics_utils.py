...
def get_rays(H, W, ori_K, c2w, resolution = -1):

	K = ori_K.clone()
	if resolution > 1:
		H = (H + resolution - 1) // resolution
		W = (W + resolution - 1) // resolution
		K[:2] /= resolution	
	...
	### zx2017 add  1108, rays_d seems inverse:
	# rays_d = -1. * rays_d
	# rays_d = torch.flip(rays_d, dims = [1])
