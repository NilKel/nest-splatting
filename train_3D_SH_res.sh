#!/bin/bash
# Script to train Nest-Splatting with 3D_SH_res mode
# Usage: ./train_3D_SH_res.sh <dataset> <base_name> [scene_names] [iterations] [extra_args]
#
# Example:
#   ./train_3D_SH_res.sh nerf_synthetic exp1              # Run all nerf_synthetic scenes
#   ./train_3D_SH_res.sh DTU exp1 scan24,scan37           # Run specific DTU scenes
#   ./train_3D_SH_res.sh mip_360 exp1 all 30000           # Run all mip_360 scenes with 30k iters
#   ./train_3D_SH_res.sh nerf_synthetic exp1 all 30000 "--kernel betascaled"  # With extra args
...
if [ $# -lt 2 ]; then
    ...
    echo ""
    echo "Default args per scene: --method 3D_SH_res --hybrid_levels 5 --disable_c2f --aabb rect --warmup gaussian --kernel gaussian"
    echo ""
    echo "Datasets:"
...
echo "════════════════════════════════════════════════════════════════════"
echo "  Nest-Splatting - 3D_SH_res Training"
echo "════════════════════════════════════════════════════════════════════"
...
LOG_DIR="logs"
mkdir -p $LOG_DIR
GLOBAL_LOG_FILE="${LOG_DIR}/train_3D_SH_res_${BASE_NAME}_${TIMESTAMP}.log"
...
run_training() {
    ...
    OUTPUT_PATH="outputs/${DATASET_PATH}/${scene_name}/3D_SH_res/${experiment_name}"
    ...
    CMD="python train.py -s $scene_path -m $experiment_name --yaml $scene_yaml --eval --iterations $ITERATIONS $scene_resolution --method 3D_SH_res --hybrid_levels 5 --disable_c2f --aabb rect --warmup gaussian --kernel gaussian $extra_args $EXTRA_ARGS"
